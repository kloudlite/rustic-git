...
| **CosmosRuSaturation** | `max(azure_normalizedruconsumption_maximum) >= 80` for 3 of the last 6 five-minute points | Search `MetricName:azure_normalizedruconsumption_maximum`, chart `max(Value)` in 5m buckets grouped by `azuremonitor.resource_id`, alert when 3 of the last 6 buckets are `>= 80` | Cosmos throttles with 429s past the provisioned RU/s; normalized consumption is the per-partition number, so one hot partition shows here before the account average does. |
