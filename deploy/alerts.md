...
| **CosmosThrottled** | `max(azure_throttledrequestpercentage_maximum) >= 1` (one percent of requests answered 429) for 2 of the last 6 five-minute points | Search `MetricName:azure_throttledrequestpercentage_maximum`, chart `max(Value)` in 5m buckets grouped by `azuremonitor.resource_id`, alert when 3 of the last 6 buckets are `>= 80` | Cosmos throttles with 429s past the provisioned RU/s; normalized consumption is the per-partition number, so one hot partition shows here before the account average does. |
| **CosmosUnavailable** | `avg(azure_serviceavailability_average) < 99.9` in any five-minute point of the last 15m | Search `MetricName:azure_serviceavailability_average`, chart `avg(Value)` in 5m buckets grouped by `azuremonitor.resource_id`, alert when any bucket in the last 15m is `< 99.9` (no `for` — one bad bucket is the page) | Every repo's SlateDB manifest and the ownership map live here; below the SLA the fleet is losing writes, not slowing down. |
