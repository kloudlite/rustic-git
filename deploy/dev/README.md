...
| watch a run live | `deploy/dev/tail.sh` (one line per step), `kubectl -n kloudlite logs -f deploy/dev` or k9s (raw JSON lines), HyperDX (search `k8s.pod.name:dev-*`, filter `ok:false` or an `slo_id`) — the run also streams to the pod's stdout; the console's SLO page shows it stage by stage |
