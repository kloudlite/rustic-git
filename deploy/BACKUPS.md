...
| --- | --- | --- | --- |
| Blob container `rustic-git` (the pre-rename name; data location, kept) (`KLOUDLITE_S3_URL: az://rustic-git`) | Every git repo, registry manifest and tag, PR row (SlateDB per repo/image/volume), credentials as plain keys, `index/` markers, registry blobs and manifests | storage account named in Secret `kloudlite-storage` | one (LRS unless changed) |
...
| Cosmos DB, Mongo API (`kloudlite-mongo`) | Pull-request store used by the server tier | Cosmos account | Cosmos-managed periodic backup (default: 2 copies, 8 h interval, 8 h retention) |
...
| Blob container `k3s-backup` | 24 hourly + 7 daily slots of the above, fixed names that overwrite, AES-256 encrypted with `/etc/kloudlite/k3s-backup.key` on `k3s-cp` (a copy of that key must live in the password manager) | same account as `kloudlite` | one |
...
soft-delete is the guard against `az storage container delete kloudlite` with the key that
sits in six pod specs.

```sh
ACCT=<account from Secret kloudlite-storage>
...
```sh
az cosmosdb update -g <rg> -n kloudlite-mongo --backup-policy-type Continuous --continuous-tier Continuous7Days
az cosmosdb show -g <rg> -n kloudlite-mongo --query 'backupPolicy'
```
...
`SNITCH_URL=` in `/etc/kloudlite/k3s-backup.env` on `k3s-cp`, pointing at a healthchecks.io-style
...
- Blob: `az storage blob undelete` on a soft-deleted object under `kloudlite/`; read it back.
...
- **Secrets.** The ten `kloudlite-*` Secrets on AKS and `kloudlite-agent` on k3s are created
...
| --- | --- | --- | --- |
| `kloudlite-storage` | srv | read + write + delete on `kloudlite` (SlateDB compaction deletes SSTs; `DELETE /v2/.../blobs`) | account key |
| `kloudlite-storage` | worker | read + write + delete (the GC sweep, marker reconcile) | account key |
| `kloudlite-storage` | api | read (browse, `/api/{owner}/images`, `_catalog`), write of `auth/`/`index/` keys | account key |
| `kloudlite-agent` `AZURE_*` (historical — the agent no longer holds Azure credentials at all; see Task 8) | k3s agent | read + write on the region's `wslayers*` (push uploads, restore reads); never delete | account key |
| `kloudlite-mongo` | srv, api, worker | read + write (directory, PRs) | connection string |
| `kloudlite-jwt`, `kloudlite-peer` | as `deploy/RECOVERY.md` A.2 | symmetric — the same value everywhere by design | minted value |

**Per-tier Secrets with the minimum role each**, the half that needs no code change and is
the state this file asks for: `kloudlite-storage` stays the key-holding Secret for srv
and worker; the api tier gets its own `kloudlite-storage-api`, and the agent's `AZURE_KEY`
becomes a container-scoped SAS (`racw` on `wslayers-k3s`, one-year expiry, minted by
`az storage container generate-sas`) so a leaked agent Secret cannot read `kloudlite` or
delete anything.
...
from the specs is the switch. Verify line when it is: `kubectl -n kloudlite get secret
kloudlite-storage` returns NotFound and the fleet still serves.
...
| `wslayers*` key or SAS (agent) — historical, the agent holds no Azure credential any more | same two-key dance on the region account; patch `kloudlite-agent` `AZURE_KEY`; `kubectl -n kube-system rollout restart ds/kloudlite-agent`. Only this region's own agents hold the key, so there is no second cluster to patch | in-flight pushes retry |
| Cosmos key (`kloudlite-mongo`) | `az cosmosdb keys regenerate --key-kind secondary` → patch `kloudlite-mongo` to the secondary → roll → regenerate primary | none, same reason |
| `kloudlite-jwt` **(two clusters)** | new value → `kubectl -n kloudlite patch secret kloudlite-jwt` on AKS AND `kubectl -n kloudlite-system patch secret kloudlite-jwt` on k3s → `deploy/roll.sh` → `kubectl -n kloudlite-system rollout restart deploy/kloudlite-gateway`. Every signed-in session and every `docker login` bearer is invalidated: users sign in again, clients `docker login` again | every session, once |
| `kloudlite-peer` | new value → patch → `deploy/roll.sh`. During the roll, old and new pods cannot forward to each other: 421s until the last pod is on the new value | minutes of misdirected writes |
...
| k3s api ServiceAccount token | `kubectl -n kube-system create token kloudlite-api --duration=8760h` on k3s → rebuild the kubeconfig (`deploy/RECOVERY.md` B.3) → patch `kloudlite-k3s-kubeconfig` → `kubectl -n kloudlite rollout restart deploy/kloudlite-api`. It EXPIRES — put the date somewhere that pages | none |
| k3s backup key | only if compromised: new key in `/etc/kloudlite/k3s-backup.key` AND the vault, keep the old one in the vault too (older bundles need it) | none |
| Cloudflare Origin CA cert (`gateway-tls`, optional) | 15-year cert; re-issue in the dashboard, `kubectl -n kloudlite-system create secret tls gateway-tls ... --dry-run=client -o yaml \| kubectl apply -f -`, restart the gateway | reconnects |
