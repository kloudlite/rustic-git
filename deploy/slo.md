...
| `id.cli.flow` | Identity | The kl-connect CLI's login-to-command flow completes | 99.9 % ≤ 15000 ms | fast | 1 · Identity |
...
| `env.space.bench` | Environments | The probe owner's bench follows its space's environment in its resolv.conf | 99.9 % ≤ 300000 ms | hourly | 6 · Environment |
...
| `ctl.leader` | Cluster controller | Exactly one controller pod holds the `kloudlite-controller` Lease and has renewed it within its TTL | 99.9 % | fast | 15 · Cluster controller |
| `ctl.grant.set` | Cluster controller | Choosing an environment for a space lets a probe workspace resolve and connect to one of its services | 99.9 % ≤ 30000 ms | fast | 15 · Cluster controller |
| `ctl.grant.switch` | Cluster controller | Switching the choice makes the new environment's service reachable and the old one unreachable | 99.9 % ≤ 30000 ms | fast | 15 · Cluster controller |
| `ctl.grant.cleared` | Cluster controller | Clearing the choice refuses the connect and leaves no `space-*` policy for that space | 99.9 % ≤ 30000 ms | fast | 15 · Cluster controller |
| `ctl.failover` | Cluster controller | Deleting the leader pod elects another within 20 s and a choice made during the gap converges once it is up | 99.9 % ≤ 120000 ms | hourly | 15 · Cluster controller |
| `ctl.agent.nowrite` | Cluster controller | Every `space-*` NetworkPolicy in the cluster is managed by `kloudlite-controller` and by no agent | 99.9 % | hourly | 15 · Cluster controller |
| `ctl.fanout` | Cluster controller | One change of a space's choice reconciles at most two environments | 99.9 % | hourly | 15 · Cluster controller |
...
| `ide.exec.https` | Workspaces | An HTTPS fetch from inside a wrapped exec succeeds, so a sandboxed command can still reach git, npm and cargo | 99.9 % | hourly | 14 · Experience |
...
| `team.member.paused` | Teams | A paused member's tool token, team `/v1` and bench tunnel and bench session are refused and their bench is stopped within a minute; after unpause and start the bench folder's canary is still there | 99.9 % ≤ 360000 ms | hourly | 14 · Experience |
...
| `id.cli.sshconfig` | Identity | `kl-connect ws sshconfig` writes a host block naming a running workspace | 99.9 % ≤ 15000 ms | hourly | 14 · Experience |
...
| `reg.limits` | Container registry | A manifest over its own limit is refused 413 while a blob of the same size is accepted — the two ceilings are different knobs | 99.9 % | weekly | 12 · Weekly |
...
| — | Teams | With the directory unreachable for one beat, a removed pair's objects all survive and the reconcile logs that it skipped | — | manual | manual · directory fault |
