...
#        openssl dgst -sha256 -mac HMAC -macopt "hexkey:$(od -An -tx1 < /etc/kloudlite/k3s-backup.key | tr -d ' \n')" \
...
#        openssl enc -d -aes-256-cbc -pbkdf2 -pass file:/etc/kloudlite/k3s-backup.key -in /tmp/b.tgz.enc -out /tmp/b.tgz
