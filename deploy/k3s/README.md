...
| `crds.yaml` | **Generated** — do not hand-edit. Package locks (`Workspace.spec.locks`, `status.packages.locked`) ship in it — the only thing to apply for pinned packages. `CRD_REGEN=1 cargo test -p kloudlite-workspaces --test crd_yaml`. Now also carries `Region`: apply this and `api-rbac.yaml` before rolling the api image, then register each region with `POST /v1/regions` rather than seeding it into Cosmos. |
