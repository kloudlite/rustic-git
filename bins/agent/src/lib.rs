...
pub async fn run(cfg: Config) -> Result<(), String> {
    ...
    let ctx = Arc::new(controller::Ctx::new(client.clone(), engine, cfg.node, cfg.pool, cfg.region, has_pool, cfg.registry_host, nix_client, nix::PROFILES_DIR.into(), settings.clone()));
