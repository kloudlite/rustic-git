...
mod tests {
    ...
    use kloudlite_workspaces::kube_test::{get, Route};
    ...
    fn node_decommissioning(name: &str) -> serde_json::Value {
        ...
        n["metadata"]["labels"] =
            serde_json::json!({ kloudlite_workspaces::crd::DECOMMISSION_LABEL: "true" });
        n
    }
    ...
    async fn a_drain_leaves_a_running_parent_completely_alone() {
        ...
        assert_eq!(ann["metadata"]["annotations"]["kloudlite.io/decommission-status"], "draining running=1 owned=1 copies=0 thin=1");
    }
    ...
    async fn a_releasable_volume_is_released_with_the_decommissioned_reason() {
        ...
        let routes = vec![
            ...
            Route { method: "GET", path: ENVIRONMENTS.into(), status: 200, body: list_of("Environment", vec![]) },
            get("/apis/kloudlite.io/v1alpha1/workspaces/ws-1", ws_stopped_replicated("ws-1", "node-a", "vol-1")),
            Route { method: "PUT", path: "/apis/kloudlite.io/v1alpha1/workspaces/ws-1/status".into(), status: 200, body: ws_stopped_replicated("ws-1", "", "vol-1") },
        ...
        let ws = rec.sent("PUT", "/apis/kloudlite.io/v1alpha1/workspaces/ws-1/status").remove(0);
