...
///
/// `spec.owner` is the truth; `kloudlite.io/owner` is a VIEW of it that exists because label
...
fn permanent_reason(e: &str) -> Option<&'static str> {
    use kloudlite_workspaces::engine::ops::NO_SUCH_RECORD;
...
mod tests {
    ...
    async fn a_decommissioning_owner_is_alive_so_the_loser_un_places_itself() {
        ...
        let routes = vec![
            get("/apis/kloudlite.io/v1alpha1/workspaces/vol-1", src),
            get(
            ...
            Route {
                method: "PUT",
                path: "/apis/kloudlite.io/v1alpha1/workspaces/ws-2/status".into(),
                status: 200,
    ...
    async fn the_migration_baseline_is_owned_by_its_parent() {
        ...
        let sent = rec.sent("POST", "/apis/kloudlite.io/v1alpha1/snapshots").remove(0);
