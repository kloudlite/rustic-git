...
/// `{pod ip}:8444` for the `kloudlite-agent` pod on `node` — the peer listener's own address,
...
pub(crate) async fn agent_pod_addr(client: &kube::Client, node: &str) -> Result<String, String> {
    ...
    let lp = ListParams::default().labels("app=kloudlite-agent").fields(&format!("spec.nodeName={node}"));
...
pub(crate) async fn pull_volume(ctx: &Arc<Ctx>, beat: &crate::listing::Beat, btrfs_bin: &str, http: &reqwest::Client, secret: &str, volume: &str, live: &[String]) -> bool {
    ...
    for name in order {
        ...
        for (source, addr) in &addrs {
            ...
            if let (Err(e), Some(parent)) = (&result, my_parent.as_deref()) {
                ...
                tracing::warn!(%volume, snapshot = %name, node = source, parent_requested = %parent, error = %e, reason = "incremental-failed", "pull.retried");
                ...
                tracing::info!(%volume, snapshot = %name, node = source, ok = result.is_ok(), full_ms = full.elapsed().as_millis() as u64, "pull.full.done");
            }
            match result {
                Ok(()) => {
...
pub(crate) async fn write_replica_status(
    ...
    let mut obj = match api.get_opt(&name).await? {
        ...
        None => {
            ...
            // H2: owner is unknown here (only the volume id is), so only `kloudlite.io/volume`
