...
use kloudlite_workspaces::crd::{self, VolumeSource};
use std::sync::Arc;
...
mod snapshot_tests {
    ...
    use kloudlite_workspaces::kube_test::{not_found, Recorder, Route};
    ...
    const WS_GET: &str = "/apis/kloudlite.io/v1alpha1/workspaces/ws-1";
    const WS_STATUS: &str = "/apis/kloudlite.io/v1alpha1/workspaces/ws-1/status";
    const SNAP_STATUS: &str = "/apis/kloudlite.io/v1alpha1/snapshots/vol-1-a/status";
    ...
    async fn an_unresolvable_worktree_requeues_instead_of_awaiting() {
        let tmp = tempfile::tempdir().unwrap();
        let routes = vec![not_found(WS_GET), not_found("/apis/kloudlite.io/v1alpha1/environments/ws-1")];
        let (ctx, rec) = test_ctx(tmp.path(), "node-a", routes);
        let s = snapshot("vol-1-a", "vol-1", "ws-1", "", false, crd::Phase::Working);
    ...
    async fn an_unknown_volume_still_requeues() {
        let tmp = tempfile::tempdir().unwrap();
        let routes = vec![
            not_found("/apis/kloudlite.io/v1alpha1/volumes/vol-ghost"),
            not_found("/apis/kloudlite.io/v1alpha1/workspaces/ws-1"),
            not_found("/apis/kloudlite.io/v1alpha1/environments/ws-1"),
        ];
        let (ctx, rec) = test_ctx(tmp.path(), "node-a", routes);
    ...
    async fn an_unreadable_post_cut_generation_leaves_the_annotation_alone() {
        ...
        assert_eq!(rec.sent("PATCH", SNAP_STATUS).len(), 1, "the cut still goes Ready");
        assert!(
            rec.sent("PATCH", "/apis/kloudlite.io/v1alpha1/snapshots/vol-1-a").is_empty(),
    ...
    async fn a_cut_a_seeded_clone_still_needs_survives_a_newer_one() {
        ...
        let routes = |vol: serde_json::Value| {
            vec![
                ...
                Route { method: "GET", path: "/apis/kloudlite.io/v1alpha1/volumes".into(), status: 200, body: list_of("Volume", vec![vol]) },
                Route {
                    method: "DELETE",
                    path: format!("{SNAPSHOTS_LIST}/sync-ws-1-a"),
                    status: 200,
