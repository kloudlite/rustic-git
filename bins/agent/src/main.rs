//! `kloudlite-agent`: the fleet-side process that materializes workspaces on local btrfs.
//!
...
use kloudlite_agent::{run, Config};
...
async fn main() {
    ...
    kloudlite_storage::config::install_crypto_provider();
