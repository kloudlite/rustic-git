...
fn a_git_seeded_pod_carries_an_init_container_with_the_key_and_no_token() {
    ...
    assert!(mounts.contains(&k8s::HOME_DIR), "the seeder mounts the volume as the home");
    ...
    assert!(inits[0].command.as_ref().unwrap().join(" ").contains(&format!("ls -A {}", k8s::WORKSPACE_DIR)));
