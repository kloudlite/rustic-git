...
/// (`rustic-git-tests`) this binary's `tests/` cannot reach.
async fn app() -> Arc<rustic_git_app::App> {
    std::env::set_var("RUSTIC_GIT_JWT_SECRET", JWT_SECRET);
    ...
    let app = rustic_git_app::App::new(
        Arc::new(store),
        Arc::new(ownership),
        "rustic-git-0".into(),
...
async fn revert_with_no_history_is_422() {
    let app = app().await;
    let router = rustic_git_server::router::peer_router(app.clone());
