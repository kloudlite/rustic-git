...
async fn a_panicking_stage_still_yields_a_finished_report_and_a_teardown() {
    ...
    let out = tokio::task::spawn_blocking({
        let url = url.clone();
        move || {
            std::process::Command::new(env!("CARGO_BIN_EXE_kloudlite-git-slo"))
                .args(["run", "--suite", "fast"])
                .env_clear()
                .env("PATH", std::env::var("PATH").unwrap_or_default())
                .env("KLOUDLITE_GIT_SLO_TEST_PANIC", "1")
                .env("KLOUDLITE_GIT_ADMIN_API_URL", &url)
                .env("KLOUDLITE_GIT_API_URL", &url)
                .env("KLOUDLITE_GIT_WEB_URL", &url)
                .env("KLOUDLITE_GIT_URL", &url)
                .env("KLOUDLITE_GIT_REGISTRY", "127.0.0.1:1")
                .env("KLOUDLITE_GIT_SSH_HOST", "127.0.0.1")
                .env("KLOUDLITE_GIT_REGION", "test")
                .env("KLOUDLITE_GIT_JWT_SECRET", "0123456789abcdef0123456789abcdef")
                .output()
                .expect("spawn")
        }
    })
