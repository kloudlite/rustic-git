...
pub(crate) async fn namespace_reaped(c: &mut Ctx) {
    ...
    c.step("team.namespace.reaped", QUICK, move |_| {
        async move {
            ...
            // A bench is a Workspace (`crd::is_bench`), so this one list already counts the team
            // benches that hold a namespace with no pod — `prune_namespaces` spares those for the
            // `user-key` Secret, and a keep set that missed them read an ordinary team bench as a
            // leak (four red hourly runs, 2026-09-14).
