// Shared by the two collector-outage tests (web's `test-node/collector.ts`, here for the bench):
// each runs in its own `node --test` process, because `startTracing` installs one global provider.
...
export async function requestsSurvive(collector: string) {
  ...
  startTracing("bench-test", collector);
