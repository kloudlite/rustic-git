...
const BENCH_ROUTES = new RegExp(
  `^(GET|POST) /sessions$|^POST /sessions/${SID}/(archive|restore|btw)$|^DELETE /sessions/${SID}$|^GET /sessions/${SID}/(btw|tools)$|^GET /(tasks|procs|healthz)$|^GET /exchanges\\?(session|workspace)=[\\w.-]+$|^POST /import$|^POST /workspaces/[a-z0-9-]+(/eph/[a-z0-9-]+)?/session$`,
);
