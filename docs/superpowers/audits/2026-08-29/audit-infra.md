...
What: The CRDs are the source of truth for every workspace and environment (CLAUDE.md, `crd.rs`). That truth is `/var/lib/rancher/k3s/server/db/state.db` on a single `Standard_D2s_v5` VM with no HA datastore. `backup-controlplane.sh` is correct (`VACUUM INTO`, identity tarball, `--fail`) but nothing in the repo installs it: no systemd `.service`/`.timer`, no cron line, no instruction for creating `/etc/rustic-git/k3s-backup.sas` or the `k3s-backup` container. "Hourly" is a README claim with no artefact behind it. The script hard-codes `ACCOUNT=rusticgitkolomi`.
...
### [I-2] `rustic-git-leader-0` is a single point of failure with no failover, and its PDB blocks node drains
Severity: high
Location: `deploy/rustic-git.yaml` StatefulSet `rustic-git-leader` (replicas: 1), PDB `rustic-git-leader` (`maxUnavailable: 0`), env `RUSTIC_GIT_LEADER`
...
Fix: Enable blob soft-delete (container + blob, >=14d) and blob versioning on the `rustic-git` and `wslayers*` containers; enable Cosmos continuous backup (7d) for the Mongo-API account; record the settings and a restore drill in `deploy/README` or `docs/`. These are Azure-side toggles, not code.
Effort: S
...
### [I-5] `RUSTIC_GIT_AGENT_SOURCES` IP allow-list is enforced from `X-Real-IP`/`X-Forwarded-For`, which is only trustworthy once the origin lock is applied — and that lock is a manual `kubectl patch` outside the manifests
Severity: high
Location: `bins/server/src/vol_agent.rs` `client_ip()` (reads `x-real-ip`, then first hop of `x-forwarded-for`), `deploy/ingress-nginx-config.yaml` (`use-forwarded-headers: "true"`), `deploy/ingress-nginx-origin-lock.md` (manual `loadBalancerSourceRanges` patch), `deploy/rustic-git.yaml` registry Ingress comment ("dig cr.khost.dev resolves to Cloudflare today")
...
Why it matters: A leaked agent token becomes usable from anywhere, which is exactly what the comment on `RUSTIC_GIT_AGENT_SOURCES` says cannot happen. The Cloudflare edge is the security boundary but nothing in the repo proves it is closed.
...
Severity: high
Location: `deploy/rustic-git.yaml` (`rustic-git-storage` account/key on leader, srv, api, worker; `rustic-git-mongo`; `rustic-git-cosmos` incl. `vol-agent-token`; `rustic-git-jwt`; `rustic-git-peer`), `deploy/k3s/README.md` ("agent Secret … created by hand"), `deploy/k3s/gateway.yaml` (jwt copied cluster to cluster)
...
Fix: Keep the parallel build but add a third job `needs: [test, build]` that `docker buildx imagetools create -t ghcr.io/kloudlite/rustic-git:tested-${sha}` (a tag copy, seconds) — and tell the README to pin `tested-` tags. Drop `:latest` entirely; nothing in `deploy/` uses it.
Effort: S

### [I-8] The two-image drift problem is real and only enforced by memory
Severity: medium
Location: `deploy/rustic-git.yaml` (server/api/worker pinned to `d520413`, 2026-08-28), `deploy/rustic-git-web.yaml` (web pinned to `5046c27`, 2026-08-29), `deploy/k3s/agent-daemonset.yaml` (agent `0874250`), `deploy/k3s/gateway.yaml` (gateway `59a6bd0`)
...
Severity: medium
Location: `deploy/rustic-git-web.yaml` Ingress comment ("Cloudflare's SSL mode here is Flexible"), `deploy/k3s/gateway.yaml` ("TLS terminates at the edge (the hostname's SSL mode is Flexible)"), `deploy/rustic-git.yaml` registry Ingress (`ssl-redirect: "false"`, has a cert but is proxied and fetched over HTTP)
...
Severity: medium
Location: `deploy/rustic-git.yaml` `rustic-git-srv` (no `updateStrategy`, no `podManagementPolicy`), `healthz` in `bins/server/src/router/route.rs`
...
Severity: medium
Location: `deploy/rustic-git.yaml` (`rustic-git-api` replicas 2, `rustic-git-worker` replicas 1), `deploy/rustic-git-web.yaml` (replicas 2)
...
Concerning: `secrets get/create` cluster-wide lets the agent read `rustic-git-jwt` and `rustic-git-agent` in `kube-system` (its own creds, acknowledged) and any workspace `user-key`/git token Secret; `rolebindings *` + `bind` on `rustic-git-api-secrets` (secrets create/update/patch/delete) means the agent can grant secret-write anywhere it can create a RoleBinding — every namespace.
...
Fix: A `rustic-git-system` namespace with `pod-security.kubernetes.io/enforce: baseline` for the gateway (and `privileged` only where the agent lives); the workspace NetworkPolicy selects `namespaceSelector: {kubernetes.io/metadata.name: rustic-git-system}` + `podSelector app=rustic-git-gateway`. Put `WS_REGION` in its own ConfigMap rather than reading it from the credential Secret. Have the controller set `seccompProfile: RuntimeDefault` on tenant pods so `restricted` audit stops warning.
Effort: S
...
Severity: medium
Location: `deploy/rustic-git.yaml` `rustic-git-worker` livenessProbe
...
### [I-19] Registry rate-limit whitelist and `RUSTIC_GIT_AGENT_SOURCES` disagree; both are hand-maintained node IPs
Severity: medium
Location: `deploy/rustic-git.yaml` (`limit-whitelist: 40.80.82.158/32,20.219.22.61/32,4.224.42.0/32` vs `RUSTIC_GIT_AGENT_SOURCES: centralindia-k3s=40.80.82.158/32,20.219.22.61/32`)
...
### [I-25] `kubectl apply -f deploy/rustic-git.yaml` applies both StatefulSets in one shot; the leader has no explicit roll order
Severity: low
Location: `deploy/rustic-git.yaml`, CLAUDE.md "Deploying"
