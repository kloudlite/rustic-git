# Performance and scalability audit — rustic-git
...
Location: `/Users/karthik/rustic-git/crates/git/src/protocol/upload/mod.rs:183` (`ours` on every non-empty `haves`), `:340-378` (`reachable_set` = `TreeContents` count of the whole repo), `/Users/karthik/rustic-git/crates/git/src/protocol/receive.rs:273-286` (`unexplained` → `reachable_set(old_tips)`).
...
Impact: high — a clone of a 2 GiB repo builds a 2 GiB `Vec<u8>` before the first byte leaves; three concurrent 500 MB pushes (each `Bytes` body + `RUSTIC_GIT_MAX_BODY` 512 MiB × measured 3× peak) exceed the 4 Gi srv limit and OOM-kill every repo on that pod.
Location: `/Users/karthik/rustic-git/bins/server/src/router/git.rs:219-249` (`upload_pack`: `body: Bytes`, `let mut out = Vec::new()`), `:251-282` (`receive_pack`), `:339-348` (`success` returns the `Vec`); `/Users/karthik/rustic-git/deploy/rustic-git.yaml:149-156` (limit sized for ONE push); no `Semaphore` in `bins/server`, `crates/git`, `crates/app`.
...
Effort: S + M
Measure: `container_memory_working_set_bytes{pod=~"rustic-git-srv.*"}` during 3 parallel 500 MB pushes; TTFB of `git clone` on a 1 GiB repo.
...
Location: `/Users/karthik/rustic-git/crates/git/src/gc.rs:1-8` ("runs from `admin`, i.e. with the server stopped"), `/Users/karthik/rustic-git/crates/storage/src/store.rs:146-148` (`odb()` = `gix_odb::at` per call), `:386-398` (`open_repo` fetches every pack, 8 at a time), `/Users/karthik/rustic-git/crates/git/src/protocol/upload/pack.rs:130` (no new deltas).
...
Location: `/Users/karthik/rustic-git/crates/registry/src/manifests.rs:281-283` (`bump_pulls` awaited before answering), `/Users/karthik/rustic-git/crates/registry/src/store.rs:290-305`; `Db::put` → `await_durable: true` verified in vendored `slatedb-0.15.0/src/config.rs:482-486`.
...
Location: `/Users/karthik/rustic-git/crates/registry/src/manifests.rs:169-223`, `/Users/karthik/rustic-git/crates/registry/src/store.rs:217-224,353-366,387-406`, `/Users/karthik/rustic-git/crates/storage/src/index.rs:136-149` (marker read = 2 GETs) and write (delete + put).
...
Location: `/Users/karthik/rustic-git/crates/app/src/lib.rs:589-606` (`grant_renew`), `:553-587` (`grant_claim` does `ownership.all()` when the leader asks), `:443-452` (`prune_once` full scan every 10 s).
...
Location: `/Users/karthik/rustic-git/crates/storage/src/store.rs:345-405` (`repo_exists`, `is_public`, `reconcile_marker` → `index::read` = 2 GETs, `pack_index` scan, per-pack `metadata()`), `/Users/karthik/rustic-git/bins/server/src/router/git.rs:54,64` (`repo_public` already did `exists` + gets).
...
Location: `/Users/karthik/rustic-git/crates/storage/src/index.rs:171-221`; `/Users/karthik/rustic-git/crates/registry/src/lib.rs:117-130`; callers `/Users/karthik/rustic-git/crates/api/src/repos.rs:36-53`, `teams.rs:438,494`, `feed.rs:137`.
...
Location: `/Users/karthik/rustic-git/crates/registry/src/gc.rs:37-84` (`referenced`), `:278-285`, `:313`, `:169,190` (`join_all`); `/Users/karthik/rustic-git/bins/worker/src/main.rs:434-435,482-523`.
...
Location: `/Users/karthik/rustic-git/web/apps/web/src/components/app/auto-refresh.tsx:27-44` mounted at `/Users/karthik/rustic-git/web/apps/web/src/app/(shell)/layout.tsx:26`; per-tick cost `components/app/app-shell.tsx:64`, `app/(shell)/[owner]/[repo]/guard.ts:34`, `components/repo/code.tsx:84`.
...
Location: `/Users/karthik/rustic-git/crates/workspaces/src/engine/blob.rs:53,130-142` (`get_bytes`), `/Users/karthik/rustic-git/crates/workspaces/src/engine/ops.rs:523-536`, `/Users/karthik/rustic-git/bins/agent/src/controller.rs:636-644` (`FETCH_FAILED` → permanent).
...
Location: `/Users/karthik/rustic-git/crates/workspaces/src/engine/ops.rs:523-546`, `/Users/karthik/rustic-git/crates/workspaces/src/engine/blob.rs:362-383` (`receive_into(&[u8])`).
...
Location: `/Users/karthik/rustic-git/crates/workspaces/src/engine/blob.rs:344-347` (`upload_file` → `std::fs::read` → `put_bytes`), called from `ops.rs:316`; `upload_stream` (`blob.rs:185-254`) already streams multipart but is only used by squash.
...
Location: `/Users/karthik/rustic-git/crates/api/src/browse.rs:240-257` (order: identity → `may_act_under` → visibility → cache), `/Users/karthik/rustic-git/crates/api/src/repos.rs:92-106` (`users.find_one` + `teams.find_one`), `browse.rs:293-303`.
...
Location: query `/Users/karthik/rustic-git/crates/pulls/src/directory/mod.rs:548-553`; `ensure_indexes` `mod.rs:566-616` has no `users` entry; callers `crates/api/src/credentials.rs:290-292` ← `bins/api/src/main.rs:54` ← `crates/workspaces/src/api.rs:744,633,788,710`.
...
Location: `/Users/karthik/rustic-git/bins/gateway/src/resolve.rs:24-47`, `/Users/karthik/rustic-git/bins/gateway/src/tunnel.rs:144-157` (resolve at 144, reserve at 155), `:191,217` (64 KiB buffer + `to_vec` per frame); `/Users/karthik/rustic-git/deploy/k3s/gateway.yaml:126-131`.
...
Location: `/Users/karthik/rustic-git/crates/pulls/src/merge_worker.rs:137-158` (`prune`, ponytail "no size accounting"), `:643-691` (rebase worktree), `:289-295` (full-history fetch), `/Users/karthik/rustic-git/bins/worker/src/main.rs:468` (7-day keep), `/Users/karthik/rustic-git/deploy/rustic-git.yaml:852-854`.
...
Effort: M
Measure: `du -sh /var/cache/rustic-git/merge` in the worker; `kubelet_evictions{eviction_signal="ephemeralstorage"}`.
...
Location: `/Users/karthik/rustic-git/crates/registry/src/uploads.rs:444-449` (gate `have == 0 && announced >= MIN_PART`), `:455-470` (`pour(src.chain(body))`).
...
Location: `/Users/karthik/rustic-git/crates/registry/src/uploads.rs:778-808`, ponytail at `:625-631`; `crates/registry/src/blobs.rs:22-28`.
...
Impact: medium (bounded by `RUSTIC_GIT_MAX_WARM`=64) — ~256 background S3 GETs/min/node idle plus a full PR-row scan of every warm repo every 15 s.
Location: `/Users/karthik/rustic-git/bins/server/src/lanes.rs:57-59,100-126,142-154,169-208`, `/Users/karthik/rustic-git/crates/pulls/src/pulls/check.rs:156-168`, `crates/pulls/src/pulls/jobs.rs:136-158`.
...
Location: `/Users/karthik/rustic-git/bins/server/src/browse_api/images.rs:110-143`.
...
Location: `/Users/karthik/rustic-git/crates/registry/src/manifests.rs:319-327`, `/Users/karthik/rustic-git/crates/storage/src/store.rs:39-40`.
...
Location: `/Users/karthik/rustic-git/bins/server/src/browse_api/volumes.rs:64-81`.
...
Location: `/Users/karthik/rustic-git/bins/server/src/browse_api/pulls.rs:106-133`, `/Users/karthik/rustic-git/crates/pulls/src/pulls/model.rs:227-234`.
...
Location: `/Users/karthik/rustic-git/crates/app/src/lib.rs:224-306,494-549`, `/Users/karthik/rustic-git/crates/core/src/peer.rs:47-48`.
...
Location: `/Users/karthik/rustic-git/crates/api/src/feed.rs:154-171,185-223`; `crates/storage/src/events.rs:69`.
...
Location: `/Users/karthik/rustic-git/bins/agent/src/controller.rs:254` (all StatefulSets), `:271,324` (all SnapshotRequests), `:304` (all Workspaces), `:217,243,267,334` (Volume mine ×4), `:328,370-379`.
Fix: label-select StatefulSets (`rustic-git.io/kind=environment`), `placed` selector for the bindings watch, a `stop-of` label for env SnapshotRequests, share one Volume reflector via `watches_stream`, index the snapshot store by `spec.volume`.
...
Location: `/Users/karthik/rustic-git/crates/workspaces/src/api.rs:429-440,779,812,1334,1350,1468-1486`; only `stop-*` deleted (`controller.rs:2390`).
...
Location: `/Users/karthik/rustic-git/bins/agent/src/lib.rs:158-210,247,260,313-319`.
...
Location: `/Users/karthik/rustic-git/bins/agent/src/controller.rs:1555-1594,1529,1791,1640-1644,1804-1826,1876-1880,1899-1911`.
...
Location: `/Users/karthik/rustic-git/bins/agent/src/controller.rs:40,383-388`; contrast `build_failed_backoff` `:1342-1350`.
...
Location: `/Users/karthik/rustic-git/crates/workspaces/src/api.rs:1032-1051,1758-1768,1821-1848`, `/Users/karthik/rustic-git/crates/workspaces/src/engine/ops.rs:335-346`, `/Users/karthik/rustic-git/crates/workspaces/src/upstream.rs:81-88`.
...
Location: `/Users/karthik/rustic-git/crates/workspaces/src/api.rs:657-675`.
...
Location: `/Users/karthik/rustic-git/crates/pulls/src/directory/teams.rs:287`, `mod.rs:634` (`$regex "[A-Z]"`), ponytails at `mod.rs:385,397`, `teams.rs:347`; `credentials.rs:459`.
...
Location: `/Users/karthik/rustic-git/web/apps/web/src/auth.ts:169-183`; RSC branch drops `Set-Cookie` in vendored `next-auth/lib/index.js:105-107`; `src/lib/api-token.ts:15-23` reads the old token.
...
Location: `/Users/karthik/rustic-git/web/apps/web/src/lib/browse.ts:44-51`, `/Users/karthik/rustic-git/deploy/rustic-git-web.yaml:114` (no volumes); vendored `next/dist/server/lib/incremental-cache/file-system-cache.js:303-311`.
...
Location: `/Users/karthik/rustic-git/web/apps/web/src/app/(shell)/settings/page.tsx:20-37`, `app/(shell)/[owner]/(org)/environments/page.tsx:20-51`, `app/(shell)/[owner]/[repo]/pulls/[number]/pull-data.ts:18-41`, `lib/owners.ts:14-21`, `lib/api.ts:137`.
...
Location: `/Users/karthik/rustic-git/web/apps/web/src/lib/api.ts:39-57`, `src/lib/browse.ts:48-51`.
...
Location: `/Users/karthik/rustic-git/bins/worker/src/main.rs:55,193-217,243-244`; `/Users/karthik/rustic-git/crates/pulls/src/merge_worker.rs:168-170,383-386,663-678`.
...
Location: `/Users/karthik/rustic-git/deploy/rustic-git.yaml:149-156,35,285,174-207`; `/Users/karthik/rustic-git/deploy/rustic-git-web.yaml:149-150` (600 s `proxy-read-timeout`).
...
Effort: S
Measure: 5xx on `git-receive-pack` during `kubectl rollout status sts/rustic-git-srv`.
...
Location: `/Users/karthik/rustic-git/crates/registry/src/uploads.rs:254-256,794-796`. Fix only if profiling shows it: hash 5 MiB parts in `spawn_blocking`. Effort: S. Measure: worker busy % with two 5 GiB pushes in flight.

### [P-43] `last_changes` decodes each commit's tree twice (`at(parent)` this iteration = `at(id)` next), up to 2000 commits per request
Location: `/Users/karthik/rustic-git/crates/git/src/browse.rs:254-303`, budget at `bins/server/src/browse_api/repo.rs:203`. Fix: carry `before` forward as next `now`. Effort: S. Measure: `lastmod` latency on a deep directory.

### [P-44] Redis: one `XACK` round trip per entry on a shared 250 ms-timeout connection
Location: `/Users/karthik/rustic-git/crates/storage/src/cache/disk.rs:96-129`, `cache/mod.rs:31-34`, `bins/worker/src/main.rs:205-217`. Fix: batch acks; separate `ConnectionManager` for stream commands. Effort: S. Measure: `consumer group ack failed` rate.

### [P-45] Api tier: `ssh-keygen` and PGP verification run on the async runtime; no verification cache
Location: `/Users/karthik/rustic-git/crates/api/src/credentials.rs:752-768,861`, `crates/api/src/signatures.rs:188`, `crates/api/src/gpg.rs:139-205`. Fix: `spawn_blocking`; cache `Verification` per `(repo, sha)`. Effort: S.

### [P-46] `teams_for` loads full Team documents (member arrays) to return slugs, per request
Location: `/Users/karthik/rustic-git/bins/api/src/main.rs:25-27`, `crates/pulls/src/directory/teams.rs:1016-1025`. Fix: `projection {_id:1}`. Effort: S. Measure: RU per `for_user`.

### [P-47] Browse cache miss stampede (no single-flight)
Location: `/Users/karthik/rustic-git/crates/api/src/browse.rs:264-308`. Fix: per-key `OnceCell` single-flight. Effort: S. Measure: upstream `refs` rate for one repo at 200 concurrent.

### [P-48] Every agent GETs the Volume for every new SnapshotRequest cluster-wide; `restore_gate` LISTs pods 40× per reconcile
Location: `/Users/karthik/rustic-git/bins/agent/src/snapshot.rs:49-56,109`, `controller.rs:2017-2029,1882-1889,1961-1973`. Fix: read from the shared reflector (P-28); `requeue(2 s)` instead of spinning. Effort: S.
...
Location: `/Users/karthik/rustic-git/deploy/k3s/agent-daemonset.yaml:79-189`, `deploy/rustic-git.yaml:618-737,773`, `deploy/rustic-git-web.yaml:12-115`. Fix: requests/limits on the agent (measure a push first); copy the srv anti-affinity + `PDB maxUnavailable: 1`; worker `maxSurge: 1, maxUnavailable: 0`. Effort: S.

### [P-50] GC lane re-lists three prefixes every 60 s regardless of activity
Location: `/Users/karthik/rustic-git/bins/worker/src/main.rs:434-435,482-490`. Fix: raise `GC_PASS_GAP` past ~500 owners; skip reconcile for owners with no event since last pass. Effort: S.
...
Location: `/Users/karthik/rustic-git/web/apps/web/src/lib/repo-rail.ts:18-22`, `app/layout.tsx:21-27`, `lib/api.ts:275,614,724,766,892`, `app/(shell)/page.tsx:8-14`, `lib/session.ts:23-41`. Fix: `/languages/{oid}` + capped path list; woff2; `cache()` on `getSession`/`apiToken`; paginate when an owner passes a few hundred repos. Effort: S–M.

### [P-52] kl: three API calls before the SSH handshake (`list` + `ssh_session` ×2)
Location: `/Users/karthik/rustic-git/bins/kl/src/ws.rs:16-32`, `bins/kl/src/proxy.rs:11`. Fix: pass the session via env to the ProxyCommand child. Effort: S. Measure: `time kl ws ssh <ws> true`.

### [P-53] Unbounded `recovery_asked` map; `Gateway::spend` O(n) retain per connect
Location: `/Users/karthik/rustic-git/crates/app/src/lib.rs:51-53`, `bins/gateway/src/tunnel.rs:60-65`. Both bounded in practice; no action.
...
Deploy
- No CPU limits (deliberate), `TOKIO_WORKER_THREADS=4`, startup/liveness split prevents WAL-replay crash loops, srv/leader PDBs and anti-affinity, `RUSTIC_GIT_MAX_WARM=16` × 64 MB bounded under 4 Gi, emptyDir cache (measured Multi-Attach outage removed), ingress `proxy-body-size: 0` / 600 s timeouts / request buffering off, per-IP rate limits: `deploy/rustic-git.yaml:59-79,174-207,224-231,293-301,499-521,919-926`, `deploy/rustic-git-web.yaml:37-38,95-111,147-159`.
