# Sharded multi-node rustic-git

Date: 2026-08-16 (follows the multi-node fencing test)

## Problem

SlateDB allows one writer per database. Two `rustic-git serve` processes on one bucket fence each
...
- `RUSTIC_GIT_SHARDS` — total shards, fixed for the deployment (default 1).
- `RUSTIC_GIT_OWNED_SHARDS` — this node's shards, e.g. `0,1` (default: all).
- `RUSTIC_GIT_SHARD_URLS` — `0=https://a.example,1=https://b.example`, used to redirect.
...
`RUSTIC_GIT_SERVE_FOREIGN_READS=1` lets a node serve clone/fetch for shards it does not own, from
...
With `RUSTIC_GIT_SHARDS=1` (the default) the database path stays `slatedb`, exactly where existing
