...
Everything under `/home/kl` except `~/workspace` is the same in every workspace a person
...
- The workspace subvolume is mounted at `/home/kl/workspace` (`k8s::WORKSPACE_DIR`).
...
person's namespace, mounted at `/home/kl` **before** the workspace claim at `/home/kl/workspace`
...
The prelude (`k8s::prelude`) keeps writing `~/.zshrc`, `~/.config/fish/config.fish` — but only
**if absent** — and chowns them. The person's edits therefore persist; ours seed once.
`~/workspace` is a mount point inside the home; the prelude no longer needs to create it.
