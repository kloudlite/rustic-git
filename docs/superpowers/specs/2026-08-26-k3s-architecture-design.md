...
session node         32 OCPU / 128 GB, btrfs pool   kloudlite.io/role=session
env node             16 OCPU / 128 GB, btrfs pool   kloudlite.io/role=env
```

Listing labels are a reconciled view, not a record. `kloudlite.io/owner` and `kloudlite.io/kind`
...
- Label `kloudlite.io/role=session|env` — the *positive* half; every workspace pod carries a
  `nodeSelector` for it.
- Taint `kloudlite.io/role=session:NoSchedule` (and `=env`) — the *negative* half; nothing
...
|---|---|---|
| `Volume` (`volumes.kloudlite.io`) | `owner`, `nodeName`, `quotaGb`, `source` (empty / clone-of / restore-of `{volume, snapshotId}`) | `phase`, `subvolumePresent`, `lineageTip`, `lastPush{commit,sha,at}`, `conditions` |
| `Workspace` (`workspaces.kloudlite.io`) | `owner`, `name`, `region`, `image`, `volumeRef`, `nodeName`, `desiredState` (Running/Stopped), `resources` | `phase`, `podRef`, `conditions` |
| `Environment` (`environments.kloudlite.io`) | `owner`, `name`, `region`, `services[]` (the existing `Service` struct verbatim), `volumeRef`, `nodeName`, `desiredState` | `phase`, `serviceStatus[]`, `conditions` |
...
```yaml
# deploy/kloudlite-agent.yaml (sketch — the parts that are load-bearing)
kind: DaemonSet
spec:
  template:
    spec:
      ...
      tolerations:
        - { key: kloudlite.io/role, operator: Exists, effect: NoSchedule }
      containers:
        - name: agent
          securityContext:
          ...
          env:
            ...
            - { name: WS_REGISTRY_URL, value: http://kloudlite-srv:8081 }
...
|---|---|---|
| get, list, **watch** | `volumes`, `workspaces`, `environments` (`*.kloudlite.io`) | cluster |
...
- The btrfs half cannot cascade — the GC does not know about subvolumes — so `Volume` carries a
  **finalizer** (`kloudlite.io/subvolume`). Deleting a `Volume` blocks until the node controller
...
scope.** `crates/pulls`' merge jobs run in `kloudlite-worker` against SlateDB and have nothing
...
No dual-run compatibility shim. The replaced design needed one ("match either `kloudlite.id`
...
- The label schema (`kloudlite.owner`/`.kind`/`.id`/`.service`/`.spec`) and spec-hashing —
