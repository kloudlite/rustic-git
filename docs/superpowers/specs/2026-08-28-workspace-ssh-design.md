...
label `app=rustic-git-gateway` in `kube-system`. Nothing else in the cluster can reach a
...
`/v1/(.*) → rustic-git-api` on `dev.kloudlite.io` (regex already enabled), with the same
