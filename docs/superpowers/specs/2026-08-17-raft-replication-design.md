...
`Role::Writer`/`Role::Reader` and `RUSTIC_GIT_ROLE` go away. A node's role for a repo is its Raft
...
- `Role::Writer`/`Role::Reader`, `RUSTIC_GIT_ROLE`, and `DbReader` follower reads.
