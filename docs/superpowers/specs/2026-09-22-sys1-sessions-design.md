...
3. On the child's `turn.end`, push from the clone pod into main's pod: `exec` in the clone of
   `git push ssh://kl@{main podIP}/home/kl/workspace sub/{seq}:{main's working branch}`. The
   path is fixed, because `/home/kl` is the workspace's own volume and the tree lives at
   `/home/kl/workspace`. Two platform changes, both small:
