...
- The API socket is a systemd unit (`podman.socket` → `/run/podman/podman.sock`) and must be
  ...
  (`RUSTIC_GIT_CONTAINER_SOCKET`) and default to Podman's, so a Docker host still works by
...
| Label | Value | Purpose |
|---|---|---|
| `rustic-git.owner` | `{owner}` | Reclamation and audit |
| `rustic-git.kind` | `env` \| `ws` | Distinguishes the two container shapes |
| `rustic-git.id` | `env-{id}` \| `ws-{id}` | Groups an environment's containers |
| `rustic-git.service` | `{service name}` | Identifies one service within an environment |
| `rustic-git.spec` | `{sha256 of the rendered service spec}` | Drives recreate-vs-leave-alone |
...
1. Ensure the network `rustic-git-{env.id}` exists (idempotent; `net.rs`).
2. Build the desired `Vec<ContainerSpec>` from `env.services`, hashing each.
3. List existing containers by label `rustic-git.id={env.id}`.
4. For each desired service: if a container exists with a matching `rustic-git.spec`, ensure
...
The network is named `rustic-git-{env.id}` rather than `env-{id}` so it cannot collide with a
...
straightforward sweep: list all `rustic-git.kind` containers, ask the store which ids still
...
`rustic_git_storage::store::valid_segment` (already rejects `.`, `..`, `/`, and anything
...
- `down`/`delete` match **either** `rustic-git.id={id}` **or**
...
- Losing `docker compose logs`/`ps` as debugging affordances. Both are daemon API calls we
  should expose in the product regardless; until then `docker ps --filter label=rustic-git.id=...`
