...
- **The storage registry is a kloudlite namespace.** Volumes live at `vol/{owner}/{name}`
...
- **Audience-split surfaces.** `kloudlite-api` is the FRONTEND's door only (user CRUD,
...
- **New crate + new binary**: `crates/workspaces` (models, scheduler, snapshot engine) and
  `bins/agent` (`kloudlite-agent`). The central API mounts as new routes in the existing
  `kloudlite-api` binary.
...
## API (new routes in kloudlite-api)
...
## Agent (bins/agent → kloudlite-agent)
