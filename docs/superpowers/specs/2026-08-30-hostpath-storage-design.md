...
pool. Kubernetes contributes nothing to their lifecycle: `kloudlite-local` is
...
`kloudlite.io/session` selector and toleration the pods already carry.
...
- the `kloudlite-local` StorageClass manifest
