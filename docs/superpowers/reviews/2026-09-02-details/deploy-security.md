...
hostPath pod, but its binding is scoped by `namespaceSelector` on `rustic-git.io/kind`
...
node, mounting `rustic-git-jwt` and the agent Secret. That turns root-on-one-node into
...
`rustic-git-workspace-pod-fence` a second time with a `matchConditions` on the agent's username
...
`deploy/k3s/` applies a NetworkPolicy in `rustic-git-system`, and the one process in that
...
Fix: a default-deny NetworkPolicy in `rustic-git-system` plus one ingress rule admitting 2049
from `app=rustic-git-agent` in kube-system only — the same shape as `agent-peer.yaml`.
...
`rustic-git.io/owner` label the builder already stamps) — CEL can compare the two.
...
### M4 — `rustic-git-api-secrets` grants full CRUD on every Secret in a workspace namespace
...
- **L6** `deploy/rustic-git.yaml:452-456,483-488` — the api tier reaches the k3s cluster with a
  long-lived ServiceAccount kubeconfig in the `rustic-git-k3s-kubeconfig` Secret, in a different
