...
Run: `cargo test -p kloudlite-workspaces --test crd_yaml the_attached_environment -- --nocapture`
...
Run: `cargo test -p kloudlite-workspaces --test crd_yaml the_attached_environment`
Expected: PASS.
...
Then: `cargo test -p kloudlite-workspaces --test crd_yaml` (must pass with no `CRD_REGEN`)
...
Run: `cargo test -p kloudlite-workspaces resolv_conf`
...
Run: `cargo test -p kloudlite-workspaces attach_claim_is_one_per_namespace mounts_its_own_resolv`
...
Run: `cargo test -p kloudlite-workspaces attachment_egress attachment_ingress`
...
Run: `cargo test -p kloudlite-agent-bin attached`
...
Run: `cargo test -p kloudlite-agent-bin`
...
Run: `cargo test -p kloudlite-agent-bin deleting_an_attached_workspace`
...
async fn attaching_sets_the_spec_field() {
    ...
    let patch = rec.last_patch("/apis/kloudlite.io/v1alpha1/workspaces/ws-1");
...
Run: `cargo test -p kloudlite-workspaces --test api_user attach`
...
Run: `cargo test -p kloudlite-workspaces --test api_user`
Expected: PASS.
