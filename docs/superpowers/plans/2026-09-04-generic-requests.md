...
- **`spec.owner` is truth; labels are a view, never authorization.** `kloudlite.io/owner` is stamped only so a list can be selector-narrowed, and every list re-filters on `spec.owner`.
...
- **RBAC mirrors the split:** `kloudlite-api` gets `get, list, create` on `requests`; `kloudlite-admin` gets `get, list, create, patch, delete` on `requests` and `patch, update` on `requests/status`.
- **`deploy/k3s/crds.yaml` is GENERATED** — never hand-edit; regenerate with `CRD_REGEN=1 cargo test -p kloudlite-workspaces --test crd_yaml`.
...
Run: `cargo test -p kloudlite-workspaces --lib request_tests`
...
cargo test -p kloudlite-workspaces --test crd_yaml
```
...
In `deploy/k3s/api-rbac.yaml`, in the `kloudlite-api` ClusterRole, directly after the `quotarequests` rule:
...
And in the `kloudlite-admin` ClusterRole, directly after the `quotarequests/status` rule:
...
Run: `cargo test -p kloudlite-workspaces --test api_requests`
...
Run: `cargo test -p kloudlite-workspaces --test api_requests --test api_quota`
...
Run: `cargo test -p kloudlite-workspaces --test api_admin_requests`
...
Run: `cargo test -p kloudlite-workspaces --test api_admin_requests --test api_admin_overview`
Expected: PASS
...
- Consumes (in `bins/api`): `kloudlite_pulls::directory::Directory::{add_member, set_role}`, `AddMember`, `Membership`, `Role`.
...
Run: `cargo test -p kloudlite-workspaces --lib a_directory_without_granting`
...
In `bins/api/src/main.rs`, inside `impl kloudlite_workspaces::api::Directory for Dir`:
...
```rust
    async fn grant_access(
        ...
        team: &str,
        user: &str,
        role: kloudlite_workspaces::api::TeamRole,
    ) -> kloudlite_workspaces::api::GrantAccess {
        use kloudlite_pulls::directory::{AddMember, Membership, Role};
        use kloudlite_workspaces::api::{GrantAccess, TeamRole};
        let role = match role {
...
Run: `cargo test -p kloudlite-workspaces --lib a_directory_without_granting && cargo build -p kloudlite-api`
...
Run: `cargo test -p kloudlite-workspaces && cargo clippy --workspace -- -D warnings`
Expected: PASS
...
Run: `cargo test -p kloudlite-workspaces --test api_admin_requests the_migration_is_idempotent`
