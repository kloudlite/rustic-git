...
- [ ] **Step 5: Add `WS_HOMES_EXPORT` env** to `deploy/k3s/agent-daemonset.yaml` (value `zerofs.kloudlite-system.svc:/`), with a comment pointing at zerofs.yaml. Do NOT apply to the cluster in this task.
...
- [ ] **Step 2: Run to verify they fail** — `cargo test -p kloudlite-workspaces the_home_is_the_shared` — FAIL (old paths / missing vars).
...
- [ ] **Step 5: Run agent tests locally; run `engine_ops` on build-0 (20.219.21.58): rsync tree, `cargo test --no-run -p kloudlite-workspaces --test engine_ops`, `sudo <binary> ensure_homecache`. Both green.**
...
- [ ] **Step 1: Record the test COUNT** of `cargo test -p kloudlite-agent-bin --all-targets` (the deleted-8-tests lesson: compare counts, not colors).
...
- [ ] **Step 5: crd.rs doc-comment updates + `CRD_REGEN=1 cargo test -p kloudlite-workspaces --test crd_yaml`. Full tests + clippy. Commit** `Retire the owner-to-node pin; bindings ensure namespaces from every node`.
...
- [ ] **Step 2: Update `ws_e2e.sh`**: prerequisite check `kubectl -n kloudlite-system get deploy zerofs` (missing → `exit 77`); delete home-push wait sections; add one assertion — write a file in `~/` in workspace A, stop A, start a workspace on the OTHER node (now legal), read the file back over NFS.
