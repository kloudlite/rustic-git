...
- **Ops:** `cargo fmt --check` NOT gated (938 hunks today — repo-wide reformat is the owner's call). `kloudlite-lb` http port replaced by a ClusterIP `kloudlite-http` (both Ingresses back onto it). `compat-matrix.sh` kept (covers things `http_e2e.rs` doesn't). Routing-test sleeps replaced by a per-`App` skewable clock, not `MockSystemClock` (wrong layer). `browse_http.rs:193` sleep left (not a flake).
