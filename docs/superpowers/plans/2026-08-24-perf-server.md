...
```rust
fn spawn_lease_tasks(app: Arc<kloudlite::App>) {
    use kloudlite::ownership::{LEASE_TTL, RENEW_EVERY};
...
- [ ] Run `cargo clippy --lib -- -D warnings && cargo clippy --bin kloudlite -- -D warnings` (main.rs is the bin) and `cargo test`.
