...
### Gaps found on review of the revision (2026-09-03)

- **Existing records on the cluster.** Pushes already stored have `transient: false` and become snapshots under the new rule — intended. Existing migration baselines are ALSO `transient: false` and would keep a volume alive after its workspace is deleted. Task 4 must make `is_snapshot()` exclude a baseline written by the old build (`spec.parent == ""` and message `"migration baseline"`) and the byte/record sweeps treat it as a sync point; no one-off migration script.
- **Restore from a deleted working copy's snapshot in the web** lives on the archived listing (Task 6), not on the per-workspace page (that page is gone with the workspace). Task 6 must wire the existing restore dialog there, pre-filled from the snapshot's definition.
- **A detached volume whose owner node dies** takes the dead-node sweep's Release arm (no parents ⇒ pin cleared); nothing re-claims it until a restore. Its replicas keep the bytes. Task 7's docs state this; no code.
- **Per-owner limits** on snapshots/detached volumes: deferred by the owner ("later"). The Snapshots page shows counts (Task 6).
- **Live verification** of the whole flow (push, delete, list, restore, delete last snapshot, bytes reclaimed on every node, for both kinds) happens on the cluster after merge, in addition to Task 7's e2e script.
