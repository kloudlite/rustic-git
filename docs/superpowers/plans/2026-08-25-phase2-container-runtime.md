...
| `bins/agent/src/runtime/net.rs` | `ensure_network` / `remove_network` for `rustic-git-{env_id}`. |
...
pub const L_OWNER: &str = "rustic-git.owner";
pub const L_KIND: &str = "rustic-git.kind";
pub const L_ID: &str = "rustic-git.id";
pub const L_SERVICE: &str = "rustic-git.service";
pub const L_SPEC: &str = "rustic-git.spec";
...
- [ ] **Step 1:** Add deps. `bollard = "0.17"` (default features; the unix-socket transport is
      ...
      Run `cargo build -p rustic-git-agent-bin`.
...
- [ ] **Step 2 (failing test):** In `bins/agent/src/runtime/mod.rs` write the module with the
      ...
      mod tests {
          ...
          fn label_names_are_the_documented_contract() {
              assert_eq!(L_OWNER, "rustic-git.owner");
              assert_eq!(L_KIND, "rustic-git.kind");
              assert_eq!(L_ID, "rustic-git.id");
              assert_eq!(L_SERVICE, "rustic-git.service");
              assert_eq!(L_SPEC, "rustic-git.spec");
          }
      }
      ```

      Add `mod runtime;` to `bins/agent/src/lib.rs`.
      Run: `cargo test -p rustic-git-agent-bin runtime::` — fails to compile (no module yet).
...
- [ ] **Step 4:** Run `cargo test -p rustic-git-agent-bin runtime::` — passes.
...
`rustic_git_workspaces::model::{Environment, Service, Mount, PortMap}` (Task 5 adds `PortMap`;
...
- [ ] **Step 2:** Run `cargo test -p rustic-git-agent-bin spec::` — fails to compile.
...
- [ ] **Step 3 (implement `mount_source`):**
      ...
      pub fn mount_source(live: &Path, folder: &str) -> Result<PathBuf, EngErr> {
          if !rustic_git_storage::store::valid_segment(folder) {
      ...
      `bins/agent/Cargo.toml` needs `rustic-git-storage = { path = "../../crates/storage" }`
      (it reaches it transitively today; make it direct).

- [ ] **Step 4:** Run `cargo test -p rustic-git-agent-bin spec::` — passes.
...
- [ ] **Step 5 (failing test — hashing):**
      ...
      fn spec_hash_is_stable_and_field_sensitive() {
          ...
          let mut e = a.clone();
          e.labels.insert("rustic-git.spec".into(), "whatever".into());
          assert_eq!(spec_hash(&a), spec_hash(&e));
      }
      ```

- [ ] **Step 6:** Run `cargo test -p rustic-git-agent-bin spec::` — fails.
...
- [ ] **Step 9 (failing test — projection and naming):**
      ...
      fn env_specs_name_containers_exactly_like_compose_did() {
          ...
          assert_eq!(db.labels["rustic-git.id"], "env-e1");
          assert_eq!(db.labels["rustic-git.kind"], "env");
          assert_eq!(db.labels["rustic-git.service"], "db");
          assert_eq!(db.labels["rustic-git.owner"], env.owner);
          assert_eq!(db.labels["rustic-git.spec"], spec_hash(db));
      }
      ...
      fn ws_spec_keeps_the_double_bind_and_the_ws_name() {
          ...
          assert_eq!(s.labels["rustic-git.kind"], "ws");
          assert_eq!(s.binds, [
...
```rust
/// `rustic-git-{env_id}`, deliberately NOT `env-{id}`: a compose project network from the
...
- [ ] **Step 1 (failing test):**
      ...
      fn net_name_cannot_collide_with_a_compose_project_network() {
          assert_eq!(net_name("e1"), "rustic-git-e1");
          assert_ne!(net_name("e1"), "env-e1");
      }
      ```
      Run: `cargo test -p rustic-git-agent-bin net::` — fails.
...
- [ ] **Step 1 (failing test — the label filter is the whole design, so pin it in a unit test
      ...
      fn ours_filter_matches_our_label_or_the_legacy_compose_project() {
          ...
          assert_eq!(f["label"], ["rustic-git.id=env-e1"]);
          let f = legacy_filter("env-e1");
          assert_eq!(f["label"], ["com.docker.compose.project=env-e1"]);
      }
      ```
      Run: `cargo test -p rustic-git-agent-bin reconcile::` — fails.
...
- [ ] **Step 4 (failing daemon test — down needs no local state):** In
      ...
      //! `cargo test -p rustic-git-agent-bin --test runtime_daemon -- --ignored --test-threads=1`.
      ...
      ```
      Run: `cargo test -p rustic-git-agent-bin --test runtime_daemon -- --ignored` — fails.
...
`rustic_git_storage::store::valid_segment` directly, the same predicate).
Produces:
...
- [ ] **Step 1 (failing test):** in `crates/workspaces/src/api.rs`'s test module:
      ...
      ```
      Run: `cargo test -p rustic-git-workspaces api::` — fails.
...
- [ ] **Step 2 (implement):** add the model fields, then in `api.rs`:
      ...
      fn validate_services(svcs: &[Service]) -> Result<(), String> {
          for s in svcs {
              if !rustic_git_storage::store::valid_segment(&s.name) {
...
- [ ] **Step 3:** Run `cargo test -p rustic-git-workspaces` — passes.
...
- [ ] **Step 1 (failing test):** the decision is pure, so test it without a daemon:
      ...
      ```
      Run: `cargo test -p rustic-git-agent-bin janitor` — fails.

- [ ] **Step 2 (implement):** `orphan_ids` is a set difference. In `spawn_janitor`'s tick, call
      `runtime::reconcile::list_ours(&d)`, group by the `rustic-git.id` label, ask the meta store
...
- [ ] **Step 1:** The `docker compose version` preflight (line 40) becomes a daemon check —
      ...
      `docker rm -f $(docker ps -aq --filter "label=rustic-git.id=env-$ENV_ID")`, which needs no
...
- [ ] **Step 4:** Add the delete case that M1 was about — teardown with no local state:
      ...
      [ -z "$(docker ps -aq --filter "label=rustic-git.id=env-$ENV_ID")" ] \
