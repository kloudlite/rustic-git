...
- No new binary target. `kloudlite api-serve` is a subcommand of the existing binary.
...
```rust
        ["admin", "set-visibility", path, vis] => {
            ...
            let public = match *vis {
                "public" => true,
                "private" => false,
                _ => return Err(kloudlite::err("visibility must be public or private")),
            };
...
Expected: FAIL — `unresolved import kloudlite::browse`.
...
async fn refs_then_tree_then_blob() {
    ...
    let router = kloudlite::http::peer_router(app);
...
async fn private_repo_is_404_to_a_stranger() {
    ...
    let req = Request::builder().uri("/api/alice/web/refs")
        .header(kloudlite::proxy::PEER_HEADER, "test-peer-secret")
        .header(kloudlite::proxy::OWNER_HEADER, "bob")
        .body(axum::body::Body::empty()).unwrap();
    let r = kloudlite::http::peer_router(app).oneshot(req).await.unwrap();
...
async fn a_cache_hit_does_not_touch_upstream() {
    ...
    let cache = std::sync::Arc::new(kloudlite::cache::Cache::connect(None).await);
    let al = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let aaddr = al.local_addr().unwrap();
    let store = e.store.clone();
    tokio::spawn(async move {
        kloudlite::api::serve(store, cache, format!("http://{uaddr}"), "s".into(), al).await.unwrap()
    });
...
Expected: FAIL — `unresolved import kloudlite::api`.
...
async fn handle(State(api): State<Arc<Api>>, req: Request) -> Response {
    ...
    if let Some(v) = api.visibility(&repo).await {
        ...
        if !kloudlite::auth::authorize(caller.as_deref(), owner, v) {
    ...
    let mut up = api.client.get(format!("{}{}", api.upstream, full_path(&path, req.uri().query())))
        .header(kloudlite::proxy::PEER_HEADER, &api.secret);
    if let Some(c) = &caller {
        up = up.header(kloudlite::proxy::OWNER_HEADER, c);
    }
...
```rust
    if a.first() == Some(&"api-serve") {
        ...
        let cache = Arc::new(kloudlite::cache::Cache::connect(std::env::var("KLOUDLITE_REDIS_URL").ok().as_deref()).await);
        let upstream = env("KLOUDLITE_UPSTREAM", "http://kloudlite:8081");
        let secret = std::env::var("KLOUDLITE_PEER_SECRET").map_err(|_| kloudlite::err("KLOUDLITE_PEER_SECRET required"))?;
        let l = tokio::net::TcpListener::bind(env("KLOUDLITE_API_ADDR", "0.0.0.0:8090")).await?;
        return kloudlite::api::serve(store, cache, upstream, secret, l).await;
    }
```
...
async fn a_push_drops_the_ref_entry_and_a_flip_bumps_the_generation() {
    ...
    let before = kloudlite::cache::key(1, "alice/web", "refs");
...
- `App::new` takes and stores `cache: Arc<Cache>`; `serve()` in `main.rs` builds one from
  `KLOUDLITE_REDIS_URL` and passes it; `tests/common/mod.rs::app` passes a disabled one.
...
**Files:**
- Modify: `deploy/kloudlite.yaml` (api Deployment + Service)
...
Append to `deploy/kloudlite.yaml`. It is a Deployment, not a StatefulSet: this tier owns no repo
...
apiVersion: apps/v1
kind: Deployment
metadata: { name: kloudlite-api, namespace: kloudlite }
spec:
  replicas: 2
  selector: { matchLabels: { app: kloudlite-api } }
  template:
    metadata: { labels: { app: kloudlite-api } }
    spec:
      containers:
        - name: api
          image: kloudlite:latest
          args: ["api-serve"]
          ports: [{ name: http, containerPort: 8090 }]
          env:
            # The peer Service, not the public LB: this tier speaks to the fleet as a peer.
            - { name: KLOUDLITE_UPSTREAM, value: "http://kloudlite:8081" }
            - { name: KLOUDLITE_API_ADDR, value: "0.0.0.0:8090" }
            - name: KLOUDLITE_PEER_SECRET
              valueFrom: { secretKeyRef: { name: kloudlite-peer, key: secret } }
            - name: KLOUDLITE_REDIS_URL
              valueFrom: { secretKeyRef: { name: kloudlite-redis, key: url } }
            - { name: KLOUDLITE_S3_URL, value: "s3://REPLACE_ME" }
...
Run: `kubectl apply --dry-run=client -f deploy/kloudlite.yaml`
...
```
kloudlite api-serve                                          # read API; needs KLOUDLITE_UPSTREAM
kloudlite admin set-visibility <owner>/<name> public|private
kloudlite admin purge-cache <owner>/<name>
```

Add to the environment variable list: `KLOUDLITE_REDIS_URL` (optional; without it the api serves
every request from the git nodes), `KLOUDLITE_UPSTREAM`, `KLOUDLITE_API_ADDR`.
...
```bash
git add deploy/kloudlite.yaml README.md src/api.rs
