...
`done` SnapshotRequests by the `kloudlite.io/volume` label, and the snapshot reconciler writes
...
`kloudlite.io/session=true`, the unplaced-Environment watch iff `kloudlite.io/env=true`. Those
...
Run: `cargo test -p kloudlite-workspaces --test crd_yaml`
...
```rust
    ...
    // `SnapshotRequest`s by the `kloudlite.io/volume` label. A second controller writing this
...
pub const SNAPSHOT_FINALIZER: &str = "kloudlite.io/snapshot";
```
...
Run: `CRD_REGEN=1 cargo test -p kloudlite-workspaces --test crd_yaml`
Then: `cargo test -p kloudlite-workspaces --test crd_yaml`
...
async fn a_clone_is_claimed_only_where_its_source_lives() {
    ...
    let (ctx, rec) = ctx(
        tmp.path(),
        vec![kloudlite_workspaces::kube_test::get("/apis/kloudlite.io/v1alpha1/workspaces/ws-src", src)],
    );
...
Run: `cargo test -p kloudlite-agent --test reconcile`
Expected: FAIL to compile — `kloudlite_agent::claim` does not exist.
...
`use kloudlite_workspaces::crd::{binding_name, OwnerBinding, OwnerBindingSpec};`, and in its
`#[cfg(test)] mod tests` change line 164 to
`use kloudlite_workspaces::kube_test::{conflict, get, mock_client, not_found, post, Route};`.
Nothing else in the file changes — same algorithm, new caller. Add `kloudlite-workspaces` with
...
Run: `cargo test -p kloudlite-agent --test reconcile` and
`cargo test -p kloudlite-agent placement`
...
```rust
const BINDING_STATUS: &str = "/apis/kloudlite.io/v1alpha1/ownerbindings/r1-alice/status";
...
Run: `cargo test -p kloudlite-agent --test reconcile a_binding_ensures`
Expected: FAIL to compile — `kloudlite_agent::binding` does not exist.
...
fn test_pod_ctx() -> kloudlite_workspaces::k8s::PodContext<'static> {
...
```rust
const SNAP_STATUS: &str = "/apis/kloudlite.io/v1alpha1/snapshotrequests/snap-1/status";
const VOL_GET: &str = "/apis/kloudlite.io/v1alpha1/volumes/ws-1";
...
fn snap_json(status: serde_json::Value) -> serde_json::Value {
    serde_json::json!({
        ...
        "metadata": {"name": "snap-1", "uid": "snap-uid-1", "generation": 1,
                     "finalizers": ["kloudlite.io/snapshot"],
                     "labels": {"kloudlite.io/owner": "alice", "kloudlite.io/volume": "ws-1"}},
...
async fn a_snapshot_request_runs_the_push_once_and_writes_done() {
    ...
    let (ctx, rec) = ctx(
        tmp.path(),
        vec![
            kloudlite_workspaces::kube_test::get(VOL_GET, vol_on("node-a")),
    ...
    ctx.running.lock().unwrap().insert(
        ...
        (1, tokio::task::spawn_blocking(|| {
            Ok(kloudlite_agent::controller::Done {
    ...
    let action = kloudlite_agent::snapshot::apply_snapshot(&snapshot(serde_json::json!({"phase": "working"})), &ctx)
        .await
        .unwrap();
...
async fn a_request_for_another_nodes_volume_is_left_alone() {
    let tmp = tempfile::tempdir().unwrap();
    let (ctx, rec) = ctx(tmp.path(), vec![kloudlite_workspaces::kube_test::get(VOL_GET, vol_on("node-b"))]);

    let action = kloudlite_agent::snapshot::apply_snapshot(&snapshot(serde_json::json!({})), &ctx).await.unwrap();
    assert_eq!(action, kube::runtime::controller::Action::await_change());
    assert!(
        !rec.calls().iter().any(|c| c.starts_with("PATCH")),
...
async fn a_done_snapshot_request_does_nothing_on_a_second_reconcile() {
    ...
    let action = kloudlite_agent::snapshot::apply_snapshot(&r, &ctx).await.unwrap();
    assert_eq!(action, kube::runtime::controller::Action::await_change());
...
async fn deleting_a_working_request_waits_for_the_handle() {
    ...
    ctx.running.lock().unwrap().insert(
        ...
        (1, tokio::task::spawn_blocking(|| {
            ...
            Ok(kloudlite_agent::controller::Done { phase: crd::Phase::Done, lineage_tip: None })
        })),
    );
    let r = snapshot(serde_json::json!({"phase": "working"}));

    let action = kloudlite_agent::snapshot::cleanup_snapshot(&r, &ctx).await.unwrap();
    assert_eq!(
        action,
    ...
    wait_idle(&ctx).await;
    kloudlite_agent::snapshot::cleanup_snapshot(&r, &ctx).await.unwrap();
...
Run: `cargo test -p kloudlite-agent --test reconcile`
Expected: FAIL to compile — `kloudlite_agent::snapshot` does not exist, `Done` still has
`last_push`.
...
pub async fn apply_snapshot(r: &crd::SnapshotRequest, ctx: &Arc<Ctx>) -> Result<Action, ReconcileErr> {
    ...
    if let Some((_, handle)) = finished {
        ...
        return match outcome {
            Ok(done) => {
                ...
                // over these objects by the `kloudlite.io/volume` label — a second controller
    ...
    // `spec.owner` on the Volume is the truth; the request's `kloudlite.io/owner` label is a view
...
fn env_json(status: serde_json::Value, desired: &str) -> serde_json::Value {
    serde_json::json!({
        ...
        "metadata": {"name": "env-1", "uid": "env-uid-1", "generation": 1,
                     "labels": {"kloudlite.io/owner": "alice", "kloudlite.io/kind": "environment", "kloudlite.io/team": ""}},
...
async fn stopping_an_environment_creates_one_snapshot_request_and_deletes_nothing_yet() {
    ...
    let (ctx, rec) = ctx(
        ...
        vec![
            ...
            kloudlite_workspaces::kube_test::not_found("/apis/kloudlite.io/v1alpha1/snapshotrequests/stop-env-1"),
            kloudlite_workspaces::kube_test::post(
                "/apis/kloudlite.io/v1alpha1/snapshotrequests",
                serde_json::json!({"apiVersion": "kloudlite.io/v1alpha1", "kind": "SnapshotRequest",
                                   "metadata": {"name": "stop-env-1"},
                                   "spec": {"volume": "env-1", "message": "stop"}}),
            ),
            Route { method: "PATCH", path: "/apis/kloudlite.io/v1alpha1/environments/env-1/status".into(),
    ...
    assert_eq!(action, kube::runtime::controller::Action::requeue(std::time::Duration::from_secs(15)));
    let sent = rec.sent("POST", "/apis/kloudlite.io/v1alpha1/snapshotrequests");
...
Run: `cargo test -p kloudlite-agent --test reconcile stopping_an_environment`
Expected: FAIL — the current code patches a `kloudlite.io/push-requested` annotation on the
...
async fn the_migration_adopts_the_volume_and_backfills_one_request_per_commit() {
    ...
    let (ctx, rec) = ctx(
        tmp.path(),
        vec![
            kloudlite_workspaces::kube_test::get("/apis/kloudlite.io/v1alpha1/workspaces", legacy),
            kloudlite_workspaces::kube_test::get("/apis/kloudlite.io/v1alpha1/volumes/ws-old", vol.clone()),
            Route { method: "PATCH", path: "/apis/kloudlite.io/v1alpha1/volumes/ws-old".into(), status: 200, body: vol },
            Route { method: "PATCH", path: "/apis/kloudlite.io/v1alpha1/workspaces/ws-old/status".into(),
                    status: 200, body: serde_json::json!({"apiVersion": "kloudlite.io/v1alpha1", "kind": "Workspace", "metadata": {"name": "ws-old"}}) },
            kloudlite_workspaces::kube_test::get(
                "/apis/kloudlite.io/v1alpha1/environments",
                serde_json::json!({"apiVersion": "kloudlite.io/v1alpha1", "kind": "EnvironmentList", "metadata": {}, "items": []}),
            ),
        ],
    );

    kloudlite_agent::migrate::once(&ctx).await.unwrap();

    let adopted = rec.sent("PATCH", "/apis/kloudlite.io/v1alpha1/volumes/ws-old");
    let refs = adopted[0]["metadata"]["ownerReferences"].as_array().expect("the volume is adopted");
    assert_eq!(refs[0]["kind"], "Workspace");
    assert_eq!(refs[0]["name"], "ws-old");
    let st = rec.sent("PATCH", "/apis/kloudlite.io/v1alpha1/workspaces/ws-old/status");
...
Run: `cargo test -p kloudlite-agent --test reconcile the_migration_adopts`
Expected: FAIL to compile — `kloudlite_agent::migrate` does not exist.
...
async fn push_creates_a_snapshot_request_on_the_volumes_node() {
    ...
    assert_eq!(r["metadata"]["labels"]["kloudlite.io/volume"], "ws-1");
...
fn snap_obj(name: &str, volume: &str, id: &str, at: &str, message: Option<&str>) -> Value {
    let mut v = json!({
        "apiVersion": "kloudlite.io/v1alpha1", "kind": "SnapshotRequest",
        "metadata": {"name": name, "labels": {"kloudlite.io/owner": "karthik", "kloudlite.io/volume": volume}},
        "spec": {"volume": volume},
...
async fn history_lists_done_snapshot_requests_newest_first() {
    let list = json!({
        "apiVersion": "kloudlite.io/v1alpha1", "kind": "SnapshotRequestList", "metadata": {},
        "items": [
            ...
            json!({"apiVersion": "kloudlite.io/v1alpha1", "kind": "SnapshotRequest",
                   "metadata": {"name": "snap-c", "labels": {"kloudlite.io/volume": "ws-1"}},
...
Run: `cargo test -p kloudlite-workspaces` then `cargo test --workspace`
Expected: PASS.
...
`deploy/kloudlite.yaml` and verifying a fresh "Open in a workspace" clone. The image pins in both
...
kubectl -n kube-system get pods -l app=kloudlite-agent \
...
Run: `cargo test -p kloudlite-workspaces --test crd_yaml release_two`
...
| Objects: `SnapshotRequest` finalizer `kloudlite.io/snapshot` | 1, 5 |
...
| Watches label-selected on `kloudlite.io/kind` | 4 (Pod), 6 (Deployment) |
...
single spelling of `kloudlite.io/volume`, used by Tasks 1, 4, 5, 6, 7 and 8;
