...
**Tech Stack:** Rust, `kube` 0.99-era (`Api`, `Controller`, `watcher::Config`), `json_patch`, `axum` for the peer listener, `tokio` (`spawn_blocking` for all btrfs work), `tempfile` + `kloudlite_workspaces::kube_test` (`mock_client`/`Recorder`/`Route`) for tests.
...
- **Gate for every task** (run unpiped, exactly this, both commands, before the commit step):
  ```
  cargo test -p kloudlite-agent-bin -p kloudlite-workspaces -- --test-threads=1; echo exit=$?
...
Run: `cargo test -p kloudlite-agent-bin a_push_that_landed_during_the_pass -- --test-threads=1; echo exit=$?`
Expected: FAIL — no `GET /apis/kloudlite.io/v1alpha1/snapshots/push-late` in `rec.calls()`, and the directory is gone.
...
Run: `cargo test -p kloudlite-agent-bin -p kloudlite-workspaces -- --test-threads=1; echo exit=$?`
Expected: `exit=0`.
...
Run: `cargo test -p kloudlite-agent-bin --test reconcile deleting_a_parent_keeps_a_sync_point -- --test-threads=1; echo exit=$?`
...
Run: `cargo test -p kloudlite-agent-bin an_empty_pinned_volume -- --test-threads=1; echo exit=$?`
...
Run: `cargo test -p kloudlite-agent-bin the_retire_pass_does_not_walk -- --test-threads=1; echo exit=$?`
...
Run: `cargo test -p kloudlite-agent-bin a_wake_arriving_inside_the_floor -- --test-threads=1; echo exit=$?`
...
Run: `cargo test -p kloudlite-agent-bin --test peer a_stalled_puller -- --test-threads=1; echo exit=$?`
...
Run: `cargo test -p kloudlite-agent-bin the_receive_ceiling_follows -- --test-threads=1; echo exit=$?`
...
Run: `cargo test -p kloudlite-agent-bin the_newest_sync_point_wins -- --test-threads=1; echo exit=$?`
...
async fn a_snapshot_on_another_nodes_volume_makes_no_api_calls() {
    ...
    let action = kloudlite_agent::snapshot::reconcile_commit(s, ctx.clone()).await.expect("no error");
...
async fn a_snapshot_on_my_volume_still_resolves_its_worktree() {
    ...
    assert_eq!(action, kube::runtime::controller::Action::requeue(kloudlite_agent::controller::TICK));
}
...
async fn an_unknown_volume_falls_through_to_the_worktree_lookup() {
    ...
    let _ = kloudlite_agent::snapshot::reconcile_commit(Arc::new(snapshot_cr("push-1", "vol-x", "ws-1")), ctx.clone()).await;
...
Run: `cargo test -p kloudlite-agent-bin --test reconcile a_snapshot_on_another_nodes_volume -- --test-threads=1; echo exit=$?`
...
fn kept_conditions_preserves_replicated_and_decommissioning() {
    ...
    let kept = kloudlite_agent::controller::workspace::kept_conditions(&prev, cond("Ready", true));
...
Run: `cargo test -p kloudlite-agent-bin kept_conditions_preserves -- --test-threads=1; echo exit=$?`
...
Run: `cargo test -p kloudlite-agent-bin the_dead_node_floor_defaults -- --test-threads=1; echo exit=$?`
...
**Why (M7):** the peer address for a node is whichever pod in `kube-system` carries `app=kloudlite-agent` and `spec.nodeName={node}`. Anyone who can create a pod in `kube-system` can redirect a pull. That is already a cluster-admin-adjacent capability, so this is hardening rather than a hole — and a `spec.serviceAccountName` check closes it for one line.
...
Run: `cargo test -p kloudlite-agent-bin a_pod_wearing_the_label -- --test-threads=1; echo exit=$?`
...
```rust
    let ip = pods
        ...
        .filter(|p| p.spec.as_ref().and_then(|s| s.service_account_name.as_deref()) == Some("kloudlite-agent"))
        .find_map(|p| p.status.and_then(|s| s.pod_ip))
        .ok_or_else(|| format!("no ready kloudlite-agent pod on {node}"))?;
```
...
Run: `cargo test -p kloudlite-agent-bin cas_reads_a_conflict -- --test-threads=1; echo exit=$?`
...
Run: `cargo test -p kloudlite-agent-bin -- --test-threads=1 2>&1 | tail -5; echo exit=$?`
...
Run: `cargo test -p kloudlite-agent-bin --test reconcile -- --test-threads=1 2>&1 | tail -5; echo exit=$?`
...
**Why (K8):** six near-identical copies of the same ~30 lines (`NoopNix` + `test_ctx`). One shared fixture deletes ~150 lines. Note the review's suggestion of `kloudlite_workspaces::kube_test` is **not** taken: `Ctx` and `Nix` are the agent binary's own types, and a workspaces-crate helper cannot name them without the crate depending on the binary. A `#[cfg(test)]` module in the agent crate is the right home.
