...
async fn a_malformed_author_line_degrades_one_row_not_the_page() {
    ...
    let mut staging = rustic_git::objects::Staging::default();
...
fn common_tree_of(repo: &rustic_git::store::Repo, oid: gix_hash::ObjectId) -> String {
    let odb = repo.odb().unwrap();
...
Run: `cargo build --release --bin rustic-git`
