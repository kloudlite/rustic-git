...
Run: `cargo test -p rustic-git-workspaces packages`
...
Run: `cargo test -p rustic-git-workspaces packages`
Expected: 7 passed. Also `cargo clippy -p rustic-git-workspaces -- -D warnings` clean.
...
- [ ] **Step 2: Run to verify it fails** — `cargo test -p rustic-git-workspaces workspace_status_carries_packages` → compile error.
...
Run: `CRD_REGEN=1 cargo test -p rustic-git-workspaces --test crd_yaml && cargo test -p rustic-git-workspaces`
...
- [ ] **Step 2: Run to verify they fail** — `cargo test -p rustic-git-workspaces k8s::tests` → compile errors (`nix_pv` missing).
...
- [ ] **Step 4: Run** — `cargo test -p rustic-git-workspaces k8s && cargo clippy -p rustic-git-workspaces -- -D warnings`. Expected: all pass, including `a_workspace_pod_double_mounts_its_volume` (it counts claims named `live`; the new mounts are named `nix`).
...
- [ ] **Step 2: Run to verify they fail** — `cargo test -p rustic-git-agent-bin nix::` → compile error.
...
- [ ] **Step 4: Run** — `cargo test -p rustic-git-agent-bin nix:: && cargo clippy -p rustic-git-agent-bin -- -D warnings`. Expected: 4 pass. (The `reconcile.rs` tests fail to compile until Task 5 adds `nix` to the test `Ctx` — do that minimal edit here with a `FakeNix` that returns `Ok` for everything, and let Task 5 grow it.)
...
kubectl -n kube-system rollout status ds/rustic-git-agent --timeout=300s
P=$(kubectl -n kube-system get pods -l app=rustic-git-agent -o name | head -1)
kubectl -n kube-system exec $P -c nix-daemon -- /nix/var/nix/profiles/default/bin/nix store ping
kubectl -n kube-system exec $P -c agent -- /nix/var/nix/profiles/default/bin/nix build --impure --expr 'let pkgs = import (builtins.getFlake "github:NixOS/nixpkgs/<rev>") { }; in pkgs.hello' -o /nix/var/rustic/profiles/smoke && kubectl -n kube-system exec $P -c agent -- /nix/var/rustic/profiles/smoke/bin/hello
kubectl -n kube-system exec $P -c agent -- rm /nix/var/rustic/profiles/smoke
```
...
- [ ] **Step 2: Run to verify it fails** — `cargo test -p rustic-git-agent-bin nix_gc_high` → compile error.
...
- [ ] **Step 2** — push; wait for `image.yml`; pin `deploy/k3s/agent-daemonset.yaml` to the SHA; `kubectl apply -f deploy/k3s/nix-conf.yaml -f deploy/k3s/agent-rbac.yaml -f deploy/k3s/agent-daemonset.yaml`; `rollout status`. Watch `kubectl -n kube-system logs ds/rustic-git-agent -c agent` for `PackagesReady`; existing workspaces get an empty profile on their next pass and keep their current pods (`// ponytail:` in the spec: a pod created before this roll has no `/nix` until it is recreated).
- [ ] **Step 3** — push the web/api change; pin `deploy/rustic-git.yaml` + `deploy/rustic-git-web.yaml`; apply.
