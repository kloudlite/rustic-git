...
**Tech Stack:** Rust, existing `redis` crate (already a dependency, already wired via `KLOUDLITE_REDIS_URL`), Azure Managed Redis (already deployed).
...
**Files:**
- Modify: `deploy/kloudlite.yaml` (worker Deployment comment), `CLAUDE.md` (one load-bearing line)
...
- [ ] **Step 3:** `kubectl apply --dry-run=client -f deploy/kloudlite.yaml` OK; `cargo test` green.
...
- [ ] Redis-down drill: with `KLOUDLITE_REDIS_URL` unset, PR writes still succeed, the worker still finds work via the fallback sweep, the feed still renders via `pulls_across`
