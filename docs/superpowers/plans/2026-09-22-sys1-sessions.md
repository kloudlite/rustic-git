...
```
git push -o receive.denyCurrentBranch=updateInstead ssh://kl@{mainIP}/home/kl/{mainWs} HEAD:{branch}
```

(`-o` is a push option and does not set receiver config; the receiver-side config is Task 11. Keep only `git push ssh://kl@{mainIP}/home/kl/{mainWs} HEAD:{branch}` — drop `-o`.) `{branch}` = main's working branch, read once at spawn via `remote(main, "exec", {cmd: "git rev-parse --abbrev-ref HEAD"})` and stored on the sub's row as `target` (the existing field). `{mainIP}` = `platform.tools(main.workspace)` minus `:7788`.
...
- Fields: replace `children: Map<string, RpcChild>` and `turning` with `sched: Scheduler` and `subs: Subs`. `Session` hooks: `delegate: (s,t,i) => this.subs.delegate(s,t,i)`, `tell: (s, m) => this.emit({type:"row", session: s.row.id, row: {kind:"tell", text: m}})`, `askPerson`: append a `turn.step` `ask:{q}` row and resolve with the next `user` row from the person (store a pending resolver per session; `send()` resolves it if present instead of appending a user row).
...
Ship with `deploy/dev/dev-push.sh --aks` (memory: fast dev loop), then from a clone pod's `exec`: `git push ssh://kl@<main pod IP>/home/kl/<ws> HEAD:<branch>` must succeed and `git -C /home/kl/<ws> log -1` on main must show the commit. Paste both lines into the commit body.
...
- **Spec coverage.** §1 rows/derivation/two-write/boot: Tasks 2, 6, 7. §2 actor/scheduler/tiers/abort/cap: 6, 7. §3 engine copy, HTTP tools, notes in workspace: 1, 5, 10. §4 lifecycle, prelude, authorized_keys, non-ff retry: 8, 11. §5 top→main by name, `children` route, errors verbatim, restart, tests, probe: 8, 9, 12. `recall` reads the session log (§3 says sections move to the bench log): Task 6. `ask_user` from a child goes to the parent: Task 6's `askPerson` hook — **gap**: Task 9 routes `askPerson` to the person for every session; for a sub or main with a parent it must instead `receive` into the parent and resolve on the parent's next `delegate` to that child. Added to Task 9 Step 3: "askPerson: if `row.parent` is set, append the question as a parent `user` row `from: seq` and return `"asked the parent; its answer arrives as a message"` immediately (the child's turn ends; the parent's `delegate(childSeq, …)` is the answer)". Restart notice to the parent for an interrupted sub (§5): added to Task 7 `boot()` — after appending `interrupted` on a row with a parent, also `receive` into the parent: `"child {seq} was interrupted by a bench restart; delegate to it again to resume on its clone, or leave it"`.
