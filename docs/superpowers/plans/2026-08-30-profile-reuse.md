...
Run: `CARGO_INCREMENTAL=0 cargo test -p kloudlite-workspaces the_expression_does_not_depend`
...
Run: `CARGO_INCREMENTAL=0 cargo test -p kloudlite-workspaces -p kloudlite-agent-bin`
Expected: PASS.
...
Run: `CARGO_INCREMENTAL=0 cargo test -p kloudlite-agent-bin index_entry`
...
async fn a_workspace_whose_inputs_are_already_built_does_not_invoke_nix() {
    ...
    std::fs::create_dir_all(&store).unwrap();
    let hash = kloudlite_workspaces::packages::hash(&nixpkgs_pin_for_test(), &base_plus(&["hello"]));
...
Run: `CARGO_INCREMENTAL=0 cargo test -p kloudlite-agent-bin already_built`
...
```rust
    ...
    fn the_profile_sweep_removes_old_unreferenced_entries() {
        ...
        std::fs::create_dir_all(&store).unwrap();
        kloudlite_agent::nix::record_index(root, "orphan", &store).unwrap();
        assert_eq!(janitor_sweep_profiles(root, std::time::Duration::ZERO), 1);
        assert!(kloudlite_agent::nix::indexed(root, "orphan").is_none());
    }
    ...
    fn the_profile_sweep_keeps_entries_a_workspace_uses() {
        ...
        std::fs::create_dir_all(&store).unwrap();
        kloudlite_agent::nix::record_index(root, "used", &store).unwrap();
        kloudlite_agent::nix::link_profile(root, "ws-1", &store).unwrap();
        assert_eq!(janitor_sweep_profiles(root, std::time::Duration::ZERO), 0);
        assert!(kloudlite_agent::nix::indexed(root, "used").is_some());
    }
...
Run: `CARGO_INCREMENTAL=0 cargo test -p kloudlite-agent-bin profile_sweep`
