...
- btrfs-touching tests stay gated on `kloudlite_workspaces::engine::have_btrfs()` and skip cleanly (this Mac and non-root CI are not root-capable btrfs hosts). Any test that must run everywhere must not shell out to `btrfs`.
...
- [ ] **Step 2: Run it, watch it fail.** `cargo test -p kloudlite-workspaces --test engine_pool set_lineage` — expect a compile error (`set_lineage` returns `()`, `.unwrap()` invalid), which is the failure.
...
- [ ] **Step 4: Run it, watch it pass.** `cargo test -p kloudlite-workspaces --test engine_pool` then `cargo test` and `cargo clippy --workspace -- -D warnings`.
...
```rust
    fn bare_engine(pool_root: std::path::PathBuf) -> Engine {
        Engine::new(
            ...
            kloudlite_workspaces::registry_client::RegistryClient::new("http://127.0.0.1:1", "unused"),
        )
    }
...
`filetime` is needed as a dev-dependency of `bins/agent`. Add to `bins/agent/Cargo.toml` under `[dev-dependencies]`: `filetime = "0.2"`. (Check `cargo tree -p kloudlite-agent-bin | grep filetime` first — if it is already in the lock graph, pin the same version.)

- [ ] **Step 2: Run it, watch it fail.** `cargo test -p kloudlite-agent-bin --lib janitor_tests` — `stage_sweep_spares_a_file_staged_seconds_ago_with_no_lineage_entry_yet` and `an_empty_lineage_file_...` fail (sweep returns 1, files gone).
...
- [ ] **Step 4: Run it, watch it pass.** `cargo test -p kloudlite-agent-bin --lib janitor_tests` then `cargo clippy --workspace -- -D warnings`.
...
- [ ] **Step 2: Run it, watch it fail.** On a btrfs-capable Linux box, as root: `cargo test -p kloudlite-workspaces --test engine_ops idempotent`. On this Mac the gate skips — implement against the code review, and note in the commit that the assertion ran in the VM. If no VM is available, the fallback verification is the shell one-liner in Step 4.
...
- [ ] **Step 4: Run it, watch it pass.** `cargo test -p kloudlite-workspaces` plus, on a btrfs host, `cargo test -p kloudlite-workspaces --test engine_ops idempotent`. Cross-check the guard by hand where btrfs is unavailable:
```sh
...
```rust
    use kloudlite_workspaces::store::MemStore;
...
- [ ] **Step 2: Run it, watch it fail.** `cargo test -p kloudlite-server --lib vol_agent::tests::renew` — fails to compile (`renew_lease` does not exist).
...
- [ ] **Step 4: Run the server tests, watch them pass.** `cargo test -p kloudlite-server --lib vol_agent`.
...
fn spawn_lease_renewal(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            match client.post(&url).header(kloudlite_workspaces::api::WS_AGENT_HEADER, &token).send().await {
...
- [ ] **Step 6: Run everything.** `cargo test` and `cargo clippy --workspace -- -D warnings`. On a btrfs host also `cargo test -p kloudlite-agent-bin --test loop`.
...
- [ ] **Step 2: Run it, watch it fail.** `cargo test -p kloudlite-agent-bin --test loop env_up_` — fails to compile (`recover_env_volume` does not exist). On a non-btrfs machine the gate skips; run this one on the Linux VM.
...
- [ ] **Step 4: Run it, watch it pass.** On the btrfs VM: `cargo test -p kloudlite-agent-bin --test loop`. Everywhere: `cargo test && cargo clippy --workspace -- -D warnings`.
...
`bins/server/src/vol_agent.rs`'s `job_failed` then calls `kloudlite_workspaces::lease::mark_ws_error(&**store, job.kind, &job.payload).await` and the `mark_env_error` twin. `mark_ws_ready`, `mark_ws_stopped` and `mark_env_state` stay in `vol_agent.rs` — only the sweep needs the error pair.
...
- [ ] **Step 2: Run it, watch it fail.** `cargo test -p kloudlite-workspaces --lib lease::` — `an_exhausted_job_marks_its_workspace_error` fails on the last assert (`Creating` != `Error`).
...
```rust
    if exhausted {
        kloudlite_workspaces::lease::mark_ws_error(&**store, job.kind, &job.payload).await;
        kloudlite_workspaces::lease::mark_env_error(&**store, job.kind, &job.payload).await;
    }
```

- [ ] **Step 4: Run it, watch it pass.** `cargo test -p kloudlite-workspaces --lib lease::` then `cargo test && cargo clippy --workspace -- -D warnings`.
...
- [ ] **Step 2: Run them, watch them fail.** `cargo test -p kloudlite-workspaces --lib job_key_tests` and `cargo test -p kloudlite-server --lib vol_agent::tests::a_volume` — both fail to compile.
...
fn pick_leasable(
    queued: Vec<(Job, kloudlite_workspaces::store::Etag)>,
    agent: &str,
    busy: &std::collections::HashSet<String>,
) -> Option<(Job, kloudlite_workspaces::store::Etag)> {
...
- [ ] **Step 4: Run them, watch them pass.** `cargo test -p kloudlite-workspaces --lib job_key_tests && cargo test -p kloudlite-server --lib vol_agent` then `cargo test && cargo clippy --workspace -- -D warnings`.
...
- [ ] `cargo test`
- [ ] On a root-capable btrfs Linux VM (not this Mac): `cargo test -p kloudlite-workspaces --test engine_ops`, `cargo test -p kloudlite-agent-bin --test loop`, and `./tests/ws_e2e.sh` (exit 77 means a prerequisite was missing — that is not a pass).
