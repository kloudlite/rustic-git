...
- [ ] **Step 2:** Run it and watch it fail (on a non-btrfs box it self-skips — say so in the
      ...
      `cargo test -p kloudlite-workspaces --test engine_ops squash_deletes_its_build_image`
...
- [ ] **Step 5:** Add a failing unit test in `bins/agent/src/lib.rs`'s `janitor_tests` —
      ...
      Run: `cargo test -p kloudlite-agent-bin janitor_tests::sweeps_old_unattached_images` (fails).
...
- [ ] **Step 7:** Run green: `cargo test -p kloudlite-agent-bin janitor_tests` and
...
- [ ] **Step 2:** Run it fail: `cargo test -p kloudlite-workspaces latch_tests`
...
- [ ] **Step 4:** Run green: `cargo test -p kloudlite-workspaces latch_tests`
...
async fn squash(ws_id: Option<&String>) -> Result<(), String> {
    let ws_id = ws_id.ok_or("usage: kloudlite-agent squash <ws-id>")?;
...
- [ ] **Step 6:** Run: `cargo test -p kloudlite-agent-bin && cargo clippy --workspace -- -D warnings`
- [ ] **Step 7:** Commit:
...
- [ ] **Step 2:** Run it fail: `cargo test -p kloudlite-agent-bin --test loop re_registers_after_work_404`
...
```
- [ ] **Step 4:** Run green: `cargo test -p kloudlite-agent-bin --test loop`
- [ ] **Step 5:** Commit:
...
- [ ] **Step 2:** Run them fail:
      `cargo test -p kloudlite-workspaces --test api_user rejects_`
...
- [ ] **Step 4:** Run green: `cargo test -p kloudlite-workspaces --test api_user`
...
fn enforce_quota(engine: &Engine, w: &kloudlite_workspaces::model::Workspace);
```
...
- [ ] **Step 2:** Run it fail:
      `cargo test -p kloudlite-workspaces --test engine_ops quota_limit_is_visible`
...
- [ ] **Step 2:** Run them fail:
      `cargo test -p kloudlite-workspaces --test api_user deleted`
- [ ] **Step 3:** Implement in `api.rs`:
...
- [ ] **Step 2:** Run it fail:
      `cargo test -p kloudlite-workspaces --test api_volumes team_env_history`
...
- [ ] **Step 4:** Run green: `cargo test -p kloudlite-workspaces --test api_volumes`
- [ ] **Step 5:** Commit:
...
- [ ] **Step 1:** Add a failing unit test in `model.rs`:
      ...
      Run: `cargo test -p kloudlite-workspaces terminal_job_serializes` (fails to compile — the
...
- [ ] **Step 3:** Run green: `cargo test -p kloudlite-workspaces terminal_job_serializes`
...
- [ ] **Step 8:** Add a failing test in `store.rs`'s `mod tests`:
      ...
      Run: `cargo test -p kloudlite-workspaces queued_jobs_respects_the_limit`
...
- [ ] **Step 11:** Run green: `cargo test -p kloudlite-workspaces && cargo test -p kloudlite-server`
- [ ] **Step 12:** Commit:
...
- [ ] **Step 13:** Add failing tests to `crates/workspaces/tests/api_volumes.rs`:
      ...
      Run: `cargo test -p kloudlite-workspaces --test api_volumes history_limit`
...
```
      ...
      // enough to matter.`
- [ ] **Step 15:** Run green: `cargo test -p kloudlite-workspaces --test api_volumes && cargo test -p kloudlite-workspaces --test api_user`
- [ ] **Step 16:** Commit:
...
- [ ] **Step 1:** Add the four failing unit tests:
      ...
      `/mnt/pool/vol/ws one`.
- [ ] **Step 2:** Run them fail:
      `cargo test -p kloudlite-workspaces parse_survives_a_truncated_line mountpoint_matches`
...
- [ ] **Step 4:** Run green and check every `LineageEntry::parse` caller compiles against the
      ...
      `cargo test -p kloudlite-workspaces && cargo test -p kloudlite-agent-bin && cargo clippy --workspace -- -D warnings`
- [ ] **Step 5:** Commit:
