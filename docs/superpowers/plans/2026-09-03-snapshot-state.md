...
- `deploy/k3s/crds.yaml` is regenerated with `CRD_REGEN=1 cargo test -p rustic-git-workspaces --test crd_yaml` and committed; never hand-edited.
...
- Gates for every task: `cargo test -p rustic-git-agent-bin -p rustic-git-workspaces -- --test-threads=1; echo exit=$?` (unpiped) and `cargo clippy --workspace --all-targets --locked -- -D warnings`; web tasks add `cd web && bun run lint && bunx tsc --noEmit -p apps/web/tsconfig.json && bun test`.
...
- [ ] **Step 2: Run them, expect failure** — `cargo test -p rustic-git-workspaces snapshot_state of_workspace a_snapshot_spec_without` fails: `SnapshotState` not found.
...
- [ ] **Step 4: Regenerate the CRD and run the gates** — `CRD_REGEN=1 cargo test -p rustic-git-workspaces --test crd_yaml`, then `cargo test -p rustic-git-agent-bin -p rustic-git-workspaces -- --test-threads=1; echo exit=$?` and clippy. Confirm `deploy/k3s/crds.yaml` now carries `state` with a `kind` discriminator under the Snapshot schema (`grep -c attachedEnvironment deploy/k3s/crds.yaml` ≥ 1).
...
- [ ] **Step 1: Write the failing tests** — in `crates/workspaces/tests/api_commit_model.rs`, following the file's existing recorder pattern (`rec.sent("POST", "/apis/rustic-git.io/v1alpha1/snapshots")`):
...
async fn a_clone_cut_records_the_source_definition() {
    ...
    assert_eq!(r.status(), 202);
    let cut = rec.sent("POST", "/apis/rustic-git.io/v1alpha1/snapshots").into_iter()
...
async fn restoring_a_workspace_whose_source_is_gone_takes_the_snapshot_state() {
    ...
    assert_eq!(r.status(), 202);
    let body = rec.sent("POST", "/apis/rustic-git.io/v1alpha1/workspaces").pop().unwrap();
...
async fn a_pre_change_snapshot_restores_as_before() {
    ...
    assert_eq!(r.status(), 202);
    let body = rec.sent("POST", "/apis/rustic-git.io/v1alpha1/workspaces").pop().unwrap();
    assert_eq!(body["spec"]["image"], rustic_git_workspaces::model::default_ws_image());
...
async fn restoring_an_environment_without_services_takes_the_snapshots() {
    ...
    assert_eq!(r.status(), 202);
    let body = rec.sent("POST", "/apis/rustic-git.io/v1alpha1/environments").pop().unwrap();
