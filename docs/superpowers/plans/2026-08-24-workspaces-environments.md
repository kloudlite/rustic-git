...
**Architecture:** New `crates/workspaces` (models, store trait with mem+Cosmos impls, scheduler, snapshot engine ported from the POC) + new `bins/agent` (`rustic-git-agent`: long-polls the API, executes jobs). User+agent routes mount in the existing `rustic-git-api` binary.
...
- [ ] **Step 4:** `cargo test -p rustic-git-workspaces && cargo clippy --workspace -- -D warnings`
...
- [ ] **Step 3:** `cargo test -p rustic-git-workspaces` (skips cosmos without env) + clippy. Run once WITH real Cosmos env against the account the operator provides; paste results in the report.
...
- [ ] **Step 4:** `cargo test -p rustic-git-workspaces --test engine_pool` on a Linux VM with btrfs (operator provides; skips locally) + clippy everywhere.
...
```
The detached squash child is `rustic-git-agent squash <ws-id>` (Task 7 wires the subcommand; here spawn `std::env::current_exe()` with those args exactly like the POC).
...
- [ ] **Step 3:** `cargo test -p rustic-git-workspaces --test api_user`, clippy. **Commit** `feat: add the workspaces and environments user API`
...
**Files:**
- Create: `bins/agent/Cargo.toml` (bin name `rustic-git-agent`), `bins/agent/src/main.rs`
...
- Produces: `rustic-git-agent run` with env config `WS_API_URL, WS_REGION, WS_AGENT_TOKEN, WS_POOL, AZURE_ACCOUNT/KEY/CONTAINER`; also the hidden `rustic-git-agent squash <ws-id>` subcommand the engine's detached spawn uses (constructs Engine from the same env and calls `squash`).
...
- [ ] **Step 3:** btrfs VM run + clippy. **Commit** `feat: add the rustic-git-agent binary`
