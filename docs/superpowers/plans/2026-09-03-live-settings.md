...
- **Secrets and addresses are never settings.** Nothing here touches `RUSTIC_GIT_PEER_SECRET`,
  `RUSTIC_GIT_JWT_SECRET`, any `AWS_*`/`AZURE_*`/`RUSTIC_GIT_S3_URL`, `RUSTIC_GIT_CACHE_DIR`,
  `RUSTIC_GIT_PEER_ADDR`/`_SVC`/`RUSTIC_GIT_SELF`, `WS_POOL`/`WS_REGION`/`NODE_NAME`/
  `WS_HOMES_EXPORT`/`WS_PEER_ADDR`, `WS_DEFAULT_IMAGE`/`WS_RUNTIME_CLASS`/`WS_GIT_INIT_IMAGE`, or
  any of the web's `AUTH_*`/`RESEND_*` vars.
...
**`ClusterSettings.spec` (per-region agent tunables):**

| field | env var | default | range | file:line today |
|---|---|---|---|---|
| `sync_secs` | `WS_SYNC_SECS` | 60 | 10..=3600 | `bins/agent/src/sync.rs:38` |
| `replica_secs` | `WS_REPLICA_SECS` | 300 | 30..=3600 | `bins/agent/src/peer/pull.rs:51` |
| `decommission_secs` | `WS_DECOMMISSION_SECS` | 30 | 5..=600 | `bins/agent/src/decommission.rs:27` |
| `node_dead_secs` | `WS_NODE_DEAD_SECS` | 180 | 60..=3600 | `bins/agent/src/peer/placement.rs:35` |
| `peer_send_timeout_secs` | `WS_PEER_SEND_TIMEOUT_SECS` | 3600 | 60..=21600 | `bins/agent/src/peer/pull.rs:77` |
| `peer_serve_timeout_secs` | `WS_PEER_SERVE_TIMEOUT_SECS` | 900 | 60..=21600 | `bins/agent/src/peer/mod.rs:280` |
| `peer_receive_slack` | `WS_PEER_RECEIVE_SLACK` | 3 | 0..=60 | `bins/agent/src/peer/pull.rs:39` |
| `stop_flush_timeout_secs` | *(new — currently uncapturable, see note)* | 30 | 5..=300 | n/a — spec names it; no current env var was found reading a stop-flush timeout in the inventory, so this field ships with the built-in default only until a caller reads it (see Task 3 Step 4) |
| `nix_timeout_secs` | `WS_NIX_TIMEOUT` | `DEFAULT_TIMEOUT_SECS` (1800, confirm at `bins/agent/src/nix.rs`'s const) | 60..=7200 | `bins/agent/src/nix.rs:72` |
| `nixpkgs` | `WS_NIXPKGS` | `""` | none (string pin, no numeric range — kept anyway per spec §2, which lists it explicitly as a cluster setting despite being unbounded; validated only for non-emptiness when set) | `bins/agent/src/nix.rs:68` |
| `base_packages` | `WS_BASE_PACKAGES` | `nix::DEFAULT_BASE_PACKAGES` | none (space-separated string) | `bins/agent/src/nix.rs:63` |
| `default_replicas` | *(new field — today `Volume.spec.replicas` default is wherever `/v1`'s create path hardcodes it)* | 2 | 1..=5 | `crates/workspaces/src/api/*` — grep at Task 2 Step 3 for the literal |
| `max_per_owner` | `WS_MAX_PER_OWNER` | 20 | 1..=1000 | `crates/workspaces/src/model.rs:34` — **api tier, not agent**, but the spec's table places it under cluster scope because it is per-region policy; see Task 2 note |
| `home_cache_gb` | *(new field — no current env var; the homecache subvolume has no quota today)* | 10 | 1..=500 | n/a |
| `quota_gb_ceiling` | *(the `clamp_quota` 500 constant)* | 500 | 10..=5000 | grep `clamp_quota` at Task 2 Step 3 (lands in `crates/workspaces`, quotas plan) |

**`CentralSettings` (server/worker/gateway/api tunables), object-store document:**

| field | env var | default | range |
|---|---|---|---|
| `max_body` | `RUSTIC_GIT_MAX_BODY` | 2147483648 | 1048576..=8589934592 |
| `max_layer` | `RUSTIC_GIT_MAX_LAYER` | 5368709120 | 1048576..=21474836480 |
| `max_manifest` | *(new — no current env var found for a manifest size cap; spec §2 names a third body limit)* | 4194304 | 65536..=67108864 |
| `upload_grace_secs` | `RUSTIC_GIT_UPLOAD_GRACE_SECS` | 86400 | 3600..=604800 |
| `gc_interval_secs` | *(new — GC's own interval was not found as an env var in the inventory; grep `crates/registry/src/gc.rs` for its scheduling at Task 4 Step 2)* | 3600 | 300..=86400 |
| `merge_lease_secs` | *(new — the merge-worker lease TTL; grep `crates/pulls/src/merge_worker.rs`)* | 300 | 30..=3600 |
| `announce_stranded_secs` | *(new — `announce_stranded_merges`'s 15s beat in `bins/server/src/lanes.rs`)* | 15 | 5..=300 |
| `feed_retention_secs` | *(new — `crates/storage/src/events.rs`'s feed retention, if any is enforced; else document as "not currently enforced, field is a no-op until a caller reads it")* | 604800 | 3600..=2592000 |
| `clone_host` | `RUSTIC_GIT_CLONE_HOST` | `""` (prod requires) | none |
| `ssh_host` | `RUSTIC_GIT_SSH_HOST` | `""` | none |
| `ssh_port` | `RUSTIC_GIT_SSH_PORT` | 22 | 1..=65535 |
| `registry_host` | `RUSTIC_GIT_REGISTRY_HOST` | `""` | none |
| `signup_open` | *(new — no such flag exists today; ships `true` and is a no-op until a caller reads it, OR is dropped from scope if brainstorming with the owner finds no signup gate exists — flagged as an ambiguity below)* | true | bool, no range |
...
any beat; a `// ponytail: unread until a caller needs it` marks it in the struct).
...
- [ ] **Step 3: `AgentSettings`.** In `crates/workspaces/src/settings.rs`, the mirror of
  ...
  home to read from until this struct exists.
...
- [ ] **Step 1: Boot-time load + reflector.** In `bins/agent/src/lib.rs`, after `Ctx` is
  constructed and before any beat spawns: `AgentSettings::from_env()` seeds
  `LiveSettings::new(..)`; then a `kube::runtime::reflector` (or a plain `watcher::watcher` loop —
...
- [ ] **Step 2: `PUT /api/admin/settings`.** Peer-listener-only (mounted the same way
  ...
  separate route; Task 5's admin UI constructs that body.
...
## Task 5: Admin server routes — `GET/PUT /admin/settings/central`, `/admin/settings/clusters/{region}`
...
- [ ] **Step 1: `GET/PUT /admin/settings/central`.** `GET` proxies a read of `cluster/settings`
  ...
  `PUT` forwards the validated body to the server tier's peer route (`PUT
  /api/admin/settings`, Task 4 Step 2) with the peer secret and re-mints/forwards the caller's
  superadmin JWT — this is the ONE place `api::admin` calls out to the git-tier peer listener,
  matching the pattern `bins/worker`'s merge jobs already use for peer calls (`local()` vs
  `networked()` split — never format the peer secret into an error message here either).
...
- [ ] **Step 2: `GET/PUT /admin/settings/clusters/{region}`.** `GET` reads
  ...
  copies that could drift), stamps `rustic-git.io/updated-by`/`updated-at` annotations (per the
  spec's §4), server-side-applies the merged spec with the admin's field manager, and returns the
  new spec plus `status.observedGeneration` (likely stale immediately after the write — that
  staleness IS the pending marker Task 3 Step 5/Task 6 render).
...
- **Interfaces:** `SettingsTable` (props: `rows: SettingRow[]`, `onSave: (changed: Partial<T>) =>
  ...
  lastChangedBy, lastChangedAt, pending: boolean }`
...
- [ ] **Step 1: `SettingsTable`.** One row per field: name, description (from the Rust struct's
  ...
  admin GET routes (Task 5) should return the schema alongside the values so the web tab never
  hand-duplicates field descriptions; if schemars' description export needs a derive attribute
  tweak, that is a one-line addition at Task 1/2's struct definitions, not a new task), current
  ...
  is rarely hit), last change (who/when, from the row's own metadata), and a "pending" dot that
  shows while the row's value differs from what the last-known `observedGeneration`/health-version
  poll reports as applied (poll every few seconds after a save, same idea as any optimistic-UI
  spinner, clear it once the polled value matches).

- [ ] **Step 2: Central tab.** Fetches `GET /admin/settings/central`, renders one `SettingsTable`,
  Save posts only the rows the user touched (a plain object diff against the fetched snapshot) to
  `PUT /admin/settings/central`.

- [ ] **Step 3: Clusters tab.** One region selector (reusing whatever region-list component the
  quotas plan's admin area already has for its own per-region panels) plus one `SettingsTable` per
  selected region, same fetch/diff/save shape against `/admin/settings/clusters/{region}`.

- [ ] **Step 4: `lib/clone.ts` migration.** Replace the three `process.env.RUSTIC_GIT_*` reads
...
- [ ] **Step 5: `bun test`.** `SettingsTable`'s diff-computation (only changed rows are sent),
  the pending-marker's poll-and-clear logic with a fake timer.
...
## Task 7: Read-only infrastructure view
...
- [ ] **Step 1: The route.** Lists Deployments in the cluster namespace (image, replica count),
  reads Ingress hosts from the two Ingress objects CLAUDE.md's Deploying section calls out
  (registry vs app hostname), and Nodes for the `decommission-status` annotation. Entirely
  read-only — no write handler exists for this route, by design (spec §6: "No writes").
...
- **Files:**
  - Modify: `CLAUDE.md` — a short new subsection (placement: right after "Workspaces and
    ...
    `stored ?? env ?? default` rule, the `LiveSettings<T>` shape, and the "next beat, never
    mid-operation" guarantee, at the density of the existing sections (WHY, not a field list —
    the field list lives in code/schemars, not prose)
  - Modify: `tests/ws_e2e.sh` — a new assertion block
- **Interfaces:** none new; this task only proves what Tasks 1–7 built.

Steps:

- [ ] **Step 1: CLAUDE.md.** Two or three sentences: where the truth lives (object-store document
  / `ClusterSettings` CR), the merge order, and the one thing every future contributor must not
  do (read `std::env::var` directly for a knob that has a `Settings` field — go through
  `LiveSettings` instead, matching how the CLAUDE.md already tells contributors to route through
  `App::election_tick`/`OwnershipStore` rather than reinvent leader election).
...
spec names, each wired to a real call site in Task 3/4 or marked `ponytail:` inert where none
exists. §3 How a process reads them → Task 2 (`LiveSettings<T>`), Task 3 (agent reflector +
per-beat reads), Task 4 Step 3 (central refresh beat). §4 Validation, audit, safety → Task 1 RBAC,
Task 4 Step 2 / Task 5 Steps 1–3 (range 422s, annotations, ten-version history, revert, "last good
wins" in Task 3 Step 1 / Task 4 Step 3). §5 The admin UI → Task 6. §6 Read-only infrastructure view
→ Task 7. Rules → the Global Constraints block, enforced concretely by: env-then-stored-then-store
order (Task 2 Step 2/3's `merged_with`), next-beat-never-mid-operation (Task 3 Step 3's per-beat
reads, never a running operation's parameter), last-good-wins (Task 3 Step 1, Task 4 Step 3), range
validation (Task 4 Step 2, Task 5 Step 2), secrets/addresses excluded (the Global Constraints list,
checked against every field the knob-list table admits). Cases table → covered by the recorder
tests in Tasks 3/4/5 and the live proof in Task 8. Testing → Task 2 Step 4 (precedence unit test),
Task 3 Step 6 (agent recorder + fake-clock), Task 4 Step 7 / Task 5 Step 4 (admin write-path
recorder), Task 8 Step 2 (live).
