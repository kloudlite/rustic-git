...
the `kloudlite-admin` Deployment/Service/Ingress/ServiceAccount, `KLOUDLITE_API_ROLE`,
...
- **Secrets and process identity are never settings.** Nothing here touches
  `KLOUDLITE_PEER_SECRET`, `KLOUDLITE_JWT_SECRET`, any `AWS_*`/`AZURE_*`/`KLOUDLITE_S3_URL`,
  `KLOUDLITE_CACHE_DIR`, `KLOUDLITE_PEER_ADDR`/`_SVC`/`KLOUDLITE_SELF`,
...
pub const CENTRAL_SETTING_META: &[(&str, Mark, &[&str])] = &[
    ...
    ("cloneHost", Mark::Live, &[]),
    ("sshHost", Mark::Boot, &["kloudlite-gateway"]),
    ("sshPort", Mark::Boot, &["kloudlite-gateway"]),
    ("registryHost", Mark::Live, &[]),
    ("signupOpen", Mark::Live, &[]),
    ("logFormat", Mark::Boot, &["kloudlite-srv", "kloudlite-api", "kloudlite-worker", "kloudlite-gateway", "kloudlite-admin"]),
    ("workerLanes", Mark::Boot, &["kloudlite-worker"]),
];
```
...
| `default_image` | `WS_DEFAULT_IMAGE` | *(required today — no built-in default; ships with `""` = "keep the env value")* | none (image ref string) | **boot** — `kloudlite-agent` | `bins/agent/src/controller/mod.rs:232` |
| `git_init_image` | `WS_GIT_INIT_IMAGE` | `alpine/git:2.45.2` | none (image ref string) | **boot** — `kloudlite-agent` | `bins/agent/src/controller/mod.rs:263` |
| `runtime_class` | `WS_RUNTIME_CLASS` | `""` (empty = host kernel) | none (k8s runtimeClass name) | **boot** — `kloudlite-agent` | `bins/agent/src/controller/mod.rs:234` |

**`CentralSettings` (server/worker/gateway/api tunables), object-store document:**

| field | env var | default | range | mark | readers (if boot) |
|---|---|---|---|---|---|
| `max_body` | `KLOUDLITE_MAX_BODY` | 2147483648 | 1048576..=8589934592 | live | — |
| `max_layer` | `KLOUDLITE_MAX_LAYER` | 5368709120 | 1048576..=21474836480 | live | — |
...
| `upload_grace_secs` | `KLOUDLITE_UPLOAD_GRACE_SECS` | 86400 | 3600..=604800 | live | — |
...
| `clone_host` | `KLOUDLITE_CLONE_HOST` | `""` (prod requires) | none | live | — (web reads it live via Task 4 Step 5's route, no restart needed) |
| `ssh_host` | `KLOUDLITE_SSH_HOST` | `""` | none | **boot** | `kloudlite-gateway` — the init container that clones over SSH reads this at pod start via the gateway's own connection info, not per-request |
| `ssh_port` | `KLOUDLITE_SSH_PORT` | 22 | 1..=65535 | **boot** | `kloudlite-gateway` |
| `registry_host` | `KLOUDLITE_REGISTRY_HOST` | `""` | none | live | — |
...
| `log_format` | `KLOUDLITE_LOG_FORMAT` | `""` (text; `json` switches) | none (enum-shaped string, validated against `["", "json"]`) | **boot** | `kloudlite-srv`, `kloudlite-api`, `kloudlite-worker`, `kloudlite-gateway`, `kloudlite-admin` — `crates/core/src/log.rs:30` reads it once at `tracing_subscriber::init()`, before any settings handle exists |
| `worker_lanes` | `KLOUDLITE_WORKER_CONCURRENCY` | 4 | 1..=64 | **boot** | `kloudlite-worker` — `bins/worker/src/main.rs:76` spawns exactly `lanes` tasks at startup; changing the count needs a fresh process |
...
- **Files:**
  ...
  - Modify: `deploy/k3s/api-rbac.yaml` — admin ServiceAccount `create/patch` on `clustersettings`
    (the quotas plan's `kloudlite-admin` ClusterRole; the user-role ServiceAccount gets nothing)
...
- [ ] **Step 1: Define the CRD.** In `crates/workspaces/src/crd/mod.rs`, beside the other
  ...
  pub struct ClusterSettingsSpec {
      ...
      /// time, not per reconcile; a change rolls `kloudlite-agent` (Task 5). Empty means "keep
  ...
  pub const CLUSTER_SETTING_META: &[(&str, crate::settings_meta::Mark, &[&str])] = &[
      ...
      ("defaultImage", Mark::Boot, &["kloudlite-agent"]),
      ("gitInitImage", Mark::Boot, &["kloudlite-agent"]),
      ("runtimeClass", Mark::Boot, &["kloudlite-agent"]),
  ];
  ```
  ...
  `kloudlite_core::settings::Mark` re-exported — name it however the existing `crates/workspaces`
  import conventions do (check a sibling `use kloudlite_core::...` line before inventing a path).
...
- [ ] **Step 2: `crd_yaml` test.** Run `CRD_REGEN=1 cargo test --test crd_yaml -p
  kloudlite-workspaces` to regenerate the golden file the test compares against, then `cargo
...
- **Interfaces:** `kloudlite_core::settings::LiveSettings<T>::{new, load, store}`,
  `kloudlite_workspaces::settings::AgentSettings`, `kloudlite_core::settings::CentralSettings`
...
- **Files:**
  ...
  - Modify: `crates/core/src/metrics.rs` or wherever `/healthz` is served per binary — expose the
    ...
    `deploy/kloudlite.yaml:169/470/475` — no `/api/health` route exists anywhere in the Rust
...
- [ ] **Step 3: Refresh beat, all four binaries.** One function, `kloudlite_core::settings::
...
- [ ] **Step 3b: Boot fields read once, before the beat.** `log_format` and `worker_lanes` are
  ...
  `tracing_subscriber::init()` call (today reading `KLOUDLITE_LOG_FORMAT` directly) takes the
  resolved string instead, and `bins/worker/src/main.rs:76`'s `lanes` local takes the resolved
  count instead of `env("KLOUDLITE_WORKER_CONCURRENCY", "4")`. The refresh beat (Step 3) still
...
- [ ] **Step 4: Wire each "new" central field to a caller** where one plainly exists:
  ...
  `KLOUDLITE_CLONE_HOST` etc. directly from ITS OWN env today (`web/apps/web/src/lib/clone.ts`),
...
- **Files:**
  ...
  - Modify: `bins/api/src/main.rs` — construct the AKS in-cluster `kube::Client` the admin binary
    needs for Task 5/6's central-scope calls (see Step 1 — today `kloudlite-admin` only holds a
    k3s kubeconfig per region; it holds nothing for its OWN cluster)
  - Modify: `deploy/kloudlite.yaml` — `kloudlite-admin` ServiceAccount (new — today the
    ...
    `RoleBinding` in the `kloudlite` namespace scoped to exactly `get/list/patch` on
    `deployments`/`statefulsets` named `kloudlite-srv`/`kloudlite-api`/`kloudlite-worker`/
    `kloudlite-admin` (NOT `kloudlite-web`, which lives in a namespace `kloudlite-web.yaml`
  ...
  - Modify: `deploy/k3s/agent-rbac.yaml` — nothing (the agent is rolled BY the admin server's
    ...
    not, add it there, scoped to `kloudlite-agent` in `kube-system`)
  - Create/modify a per-region gateway RBAC file if `deploy/k3s/gateway.yaml`'s namespace
    (`kloudlite-system`) has no admin-writable Role yet — grep before assuming one exists
...
- [ ] **Step 1: Who's on AKS, who's per-region — confirm before coding.** `kloudlite-srv`
  (StatefulSet), `kloudlite-api`, `kloudlite-worker`, `kloudlite-admin` all live in namespace
  `kloudlite` on the AKS cluster (`deploy/kloudlite.yaml`, confirmed above). `kloudlite-web` is
  a separate Deployment in `deploy/kloudlite-web.yaml` — grep its `metadata.namespace`; if it is
  also `kloudlite` on AKS it needs the same Role's `resourceNames` list extended, if it is a
  DIFFERENT namespace or cluster it needs its own Role. **`kloudlite-gateway` is NOT a central
  AKS workload** — `deploy/k3s/gateway.yaml` deploys one per region, in that region's k3s, in
  namespace `kloudlite-system` (its own header comment: "One replica per pool node... the
  Cloudflare A records for `ws-<region>.khost.dev`"). The spec's §7 workload list (echoed
  verbatim into this plan's own task list) names `kloudlite-gateway` under "central" and then
  separately lists "the region gateway" under "per region" — this plan corrects that: there is
  only ONE gateway kind (`kloudlite-gateway`) and it is exclusively a per-region k3s Deployment;
  `admin::workloads::KNOWN`'s central half does NOT include a `kloudlite-gateway` entry, and its
  per-region half's "region gateway" row IS `kloudlite-gateway` in `kloudlite-system`, rolled
  through the same region `kube::Client` Task 6 already resolves for `ClusterSettings`. This also
  means the earlier "boot, readers: kloudlite-gateway" rows in this update's knob list
  ...
  pub const KNOWN_CENTRAL: &[(&str, Kind)] = &[
      ("kloudlite-srv", Kind::StatefulSet),
      ("kloudlite-api", Kind::Deployment),
      ("kloudlite-worker", Kind::Deployment),
      ("kloudlite-web", Kind::Deployment),
      ("kloudlite-admin", Kind::Deployment),
  ];
  /// Per region: resolved against that region's `kube::Client`, same as `ClusterSettings`.
  pub const KNOWN_PER_REGION: &[(&str, Kind)] = &[
      ("kloudlite-agent", Kind::DaemonSet), // namespace kube-system
      ("kloudlite-gateway", Kind::Deployment), // namespace kloudlite-system
  ];
  ```

- [ ] **Step 2: The AKS in-cluster client.** `kloudlite-admin` runs ON AKS (it is one of the
  Deployments in `deploy/kloudlite.yaml`), so unlike its region calls — which go out over a
  ...
  on the pod (Task 5's `deploy/kloudlite.yaml` change) — no kubeconfig, no Secret, nothing to
...
- [ ] **Step 3: `roll_readers`.** For each reader name in a boot field's `readers` list (or the
  ...
  `spec.template.metadata.annotations["kloudlite.io/restarted-at"]` to `Utc::now().to_rfc3339()`
  via `Patch::Merge` (exactly what `kubectl rollout restart` sends — never a pod `delete`), and
  the audit annotations `kloudlite.io/rolled-by` (the caller's name), `/rolled-at` (same
...
- [ ] **Step 6: RBAC.** `deploy/kloudlite.yaml`'s new `Role`/`RoleBinding` (Step 1's file list) —
  `get`/`list`/`patch` on `deployments.apps`/`statefulsets.apps`, `resourceNames` restricted to
  the four/five names in `KNOWN_CENTRAL`, namespace `kloudlite` (extended to `kloudlite-web`'s
  ...
  (`kloudlite-admin`'s, defined in `deploy/k3s/api-rbac.yaml` per the quotas plan), which needs a
  new row: `get/list/patch` on `daemonsets` (`apps`, `resourceNames: ["kloudlite-agent"]`,
  namespace `kube-system`) and on `deployments` (`resourceNames: ["kloudlite-gateway"]`,
  namespace `kloudlite-system`). No `delete`, no `create` — a roll only ever patches an existing
  object.
...
- [ ] **Step 1: `GET/PUT /admin/settings/central`.** `GET` proxies a read of `cluster/settings`
  ...
  1. Diff the incoming partial body against the current document to find which CHANGED fields are
     ...
     "every region's `kloudlite-gateway`" (Task 5 Step 1's note), resolved from `crd::Region`;
...
- [ ] **Step 2: `GET/PUT /admin/settings/clusters/{region}`.** `GET` reads
  ...
  (`default_image`/`git_init_image`/`runtime_class` → reader `kloudlite-agent` in THIS region);
  ...
  `kloudlite.io/updated-by`/`updated-at` annotations (per the spec's §4), server-side-applies the
...
- [ ] **Step 3: Ten-version history + revert for the cluster scope.** An annotation,
  `kloudlite.io/settings-history`, carrying the previous spec as JSON, list-capped at 10 —
...
- **Files:**
  ...
  - Modify: `web/apps/web/src/lib/api.ts` (or wherever the admin API base lives per the quotas
    plan's `KLOUDLITE_ADMIN_API_URL`/`adminCall`) — add `getCentralSettings`,
  ...
  /v1/settings/central` (Task 4 Step 5) instead of `process.env.KLOUDLITE_CLONE_HOST` etc.
...
- [ ] **Step 2: Save confirmation.** Before the diff is posted, if any changed row's `mark` is
  ...
  If `kloudlite-srv` is among the readers (only reachable from a central `PUT`, since no cluster
  ...
  desired}` body as a plain error — "kloudlite-worker is still rolling out (2/3 ready); try
...
- [ ] **Step 4: Clusters tab.** One region selector (reusing whatever region-list component the
  ...
  (no second-confirmation case here — no cluster-scope boot field's reader is `kloudlite-srv`).
...
- [ ] **Step 6: `lib/clone.ts` migration.** Replace the three `process.env.KLOUDLITE_*` reads
...
- [ ] **Step 7: `bun test`.** `SettingsTable`'s diff-computation (only changed rows are sent), the
  confirmation dialog's boot-vs-live branch and the second-confirmation-on-`kloudlite-srv` case,
...
- **Interfaces:** `GET /admin/infra` → `{ tier: string, image: string, replicas: u32 }[]` plus
  ingress hosts and each node's decommission status (`kloudlite.io/decommission-status`
...
- [ ] **Step 1: CLAUDE.md.** Three or four sentences: where the truth lives (object-store document
  ...
  `kloudlite.io/restarted-at` (never a pod delete), and a reader still mid-rollout blocks the
...
- [ ] **Step 3: e2e for a boot roll.** `kubectl patch clustersettings default --type merge -p
  ...
  `kubectl rollout status daemonset/kloudlite-agent -n kube-system` and assert it completes and
  the pod template's `kloudlite.io/restarted-at` annotation is fresh; assert the same patch sent
...
(confirmation, second confirmation on `kloudlite-srv`, roll progress) → Task 7 Steps 2/5.** Rules
...
- The spec's §7 workload list (and this update's own task instructions, which echo it) names
  `kloudlite-gateway` under "central" and separately lists "the region gateway" per region — read
  against `deploy/k3s/gateway.yaml` (one Deployment per region, namespace `kloudlite-system`,
  hostPort 80 per pool node) and `deploy/kloudlite.yaml` (which has no gateway Deployment at
  all), there is only ONE `kloudlite-gateway` kind and it is exclusively per-region. Task 5 Step 1
  resolves this: `admin::workloads::KNOWN_CENTRAL` has no gateway row; the per-region row IS
  `kloudlite-gateway`, and it is what `ssh_host`/`ssh_port`'s boot-reader entries in the central
...
- The spec's §5 says central binaries expose "the loaded version on their `/api/health`", but no
  ...
  `deploy/kloudlite.yaml`). Read this as describing the health-check concept, not a literal new
...
- `KLOUDLITE_CLONE_HOST`/`SSH_HOST`/`SSH_PORT`/`REGISTRY_HOST` are consumed by the Next.js web
