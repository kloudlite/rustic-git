...
fn an_incremental_hash_matches_the_one_shot_digest() {
    use rustic_git::registry::store::Hasher;
...
Create `tests/registry_limits.rs`. It is its own binary on purpose: `max_layer()` is a process-wide `OnceLock` read from `RUSTIC_GIT_MAX_LAYER`, so a tiny cap cannot share a process with the other registry tests.
...
async fn authed() -> (String, common::TestEnv, reqwest::Client, String) {
    std::env::set_var("RUSTIC_GIT_MAX_LAYER", CAP);
    let (base, e) = common::serve_public().await;
    let token = e.store.create_token("acme").await.unwrap();
