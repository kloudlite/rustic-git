...
Rust 2021 workspace (axum 0.8, tokio, serde, reqwest, chrono), `kloudlite_storage::store::valid_segment`
for the segment rule, `kloudlite_workspaces::store::MemStore` for tests, integration tests in
`tests/vol_agent.rs` (root package `kloudlite-tests`), unit tests in the crate modules.
...
**Interfaces:**
- Consumes: `kloudlite_storage::store::valid_segment` (already a dependency —
...
- [ ] **Step 2: See it fail.** `cargo test -p kloudlite-workspaces model::tests` — expected:
...
- [ ] **Step 4: See it pass.** `cargo test -p kloudlite-workspaces model::tests` — 2 passed.
...
- [ ] **Step 2: See it fail.** `cargo test -p kloudlite-workspaces api::tests` — expected:
...
- [ ] **Step 4: See it pass.** `cargo test -p kloudlite-workspaces api::tests` — 1 passed.
      Then `cargo clippy --workspace -- -D warnings`.
...
- [ ] **Step 2: See it fail.** `cargo test -p kloudlite-workspaces compose::tests` — expected:
...
```rust
            if seen.insert(m.folder.clone()) {
                ...
                kloudlite_workspaces::model::validate_mount(m)?;
...
- [ ] **Step 4: See it pass.** `cargo test -p kloudlite-workspaces compose::tests` — 1 passed.
      Then `cargo build -p kloudlite-agent && cargo clippy --workspace -- -D warnings`.
...
- [ ] **Step 4: See it pass.** `cargo test --test vol_agent` (all green, including the three
      amended tests) and `cargo build -p kloudlite-agent`. Then
      `cargo clippy --workspace -- -D warnings`.
...
- [ ] **Step 1: Write the failing test.** Add to the jobs module in `tests/vol_agent.rs` (needs
      `use kloudlite_workspaces::model::{Workspace, WsState};` added to the module's imports —
...
```rust
    let jobs = kloudlite_server::boot::build_jobs_state(app.is_leader()).await?;
```
...
- [ ] **Step 2: Verify.** `cargo build -p kloudlite-agent && cargo clippy --workspace -- -D warnings`.
