...
- [ ] **Step 1:** Write the pod, in an existing throwaway namespace on `centralindia-k3s` (the probe's `ws-slo-probe` is fine — it is swept by name prefix, so name it `run-spike-buildkit`):
