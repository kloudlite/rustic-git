...
**Tech Stack:** Rust 2021 workspace, axum 0.8, SlateDB + `object_store`, `metrics`/`metrics-exporter-prometheus`, tokio; integration tests in the root `rustic-git-tests` package under `tests/`, unit tests in-crate.
...
## Global Constraints
- Upload-pack negotiation cap: `8 * 1024 * 1024` bytes (8 MiB), a `const` in `bins/server/src/router/git.rs`, no env override — a `want`/`have` pkt-line is ~50 bytes, so 8 MiB is over 150 000 lines, far past any real negotiation, and one fewer knob to misconfigure than `RUSTIC_GIT_MAX_BODY` was.
- `max_body()` (2 GiB default, 512 MiB in `deploy/rustic-git.yaml:145`) stays exactly as it is on the receive-pack streamed path (`live_body`) — it is already correct there and its test at `tests/git_http_limits.rs:81` must keep passing unchanged.
...
- M3 option chosen: **its own listener/port** — `RUSTIC_GIT_METRICS_ADDR=0.0.0.0:9464` via the already-written `rustic_git_core::metrics::serve_if_configured()`, and `/metrics` removed from the peer router. Reason in Task 4.
- L3 single copy lives in `crates/core/src/sshkeys.rs` behind an optional `ssh` feature (optional `russh` dependency). `crates/storage` depends on `rustic-git-core` with no features, so its Cargo.toml keeps no ssh dependency.
...
**Interfaces:**
- Consumes: `axum::body::to_bytes`, `rustic_git_core::httpx::max_body` (unchanged, still used by `live_body`)
...
`use rustic_git_core::httpx::max_body;` at `git.rs:2` stays — `live_body` at `git.rs:139` still uses it.

- [ ] **Step 4: Run tests and clippy** — `cargo test --test git_http_limits && cargo test -p rustic-git-server && cargo clippy --workspace --all-targets --locked -- -D warnings`
...
- [ ] **Step 2: Run it, expect failure** — `cargo test -p rustic-git-api a_description_with_control_characters_is_refused` fails with `assertion failed: check_description("hi\ncreated_by=someone.else").is_err()`; `cargo test -p rustic-git-storage a_newline_in_a_description_cannot_forge_a_marker_field` fails with `assertion `left == right` failed\n  left: "someone.else"\n right: "alice@example.com"`.
...
- [ ] **Step 4: Run tests and clippy** — `cargo test -p rustic-git-api -p rustic-git-storage && cargo test --test browse && cargo clippy --workspace --all-targets --locked -- -D warnings`
...
**Interfaces:**
- Consumes: `rustic_git_storage::pool::Pool::exists(&self, owner: &str, name: &str) -> Result<bool>` (`crates/storage/src/pool/mod.rs:296`)
...
- [ ] **Step 2: Run it, expect failure** — `cargo test --test routing an_invented_repo_name_is_404_and_claims_nothing only_the_create_routes_may_claim_a_name_that_does_not_exist`. Expected: the second test fails to compile with ``error[E0432]: unresolved import `rustic_git_server::router_test` ``; after the module exists, the first fails with `assertion `left == right` failed\n  left: 404\n right: 404` passing but `assert_eq!(a.app.owner(...), None)` failing — `Some(Entry { node: "rustic-git-1", .. })`, the claim the fix removes.
...
- Modify `deploy/rustic-git.yaml:31-33` (scrape port), `:145-150` (env + containerPort), `:308-341` (NetworkPolicy ingress)
- Modify `deploy/alerts.md:3-4`
- Test `tests/metrics.rs` (rewrite of the single test there)

**Interfaces:**
- Consumes: `rustic_git_core::metrics::serve_if_configured()`, `rustic_git_core::metrics::routes()`
...
`bins/server/src/router/mod.rs`: delete lines 39-40 (the comment and `.merge(rustic_git_core::metrics::routes())`) from `peer_router`, and amend the `peer_router` doc comment's last sentence to read:
...
`bins/server/src/main.rs`: after `rustic_git_core::metrics::init();` at line 238, add:
...
`deploy/rustic-git.yaml`: at lines 31-33 change `prometheus.io/port: "8081"` to `"9464"`; in the server container's env block beside `RUSTIC_GIT_MAX_BODY` (line 145) add:
...
after line 150 add `            - { name: metrics, containerPort: 9464 }`; and in the `rustic-git-peers-only` NetworkPolicy add a final ingress rule (after the `8080`/`2222` block ending at line 341):
...
`deploy/alerts.md:3-4`: replace "the server tier serves `/metrics` on the peer port (8081)" with "every binary, the server tier included, serves `/metrics` on `RUSTIC_GIT_METRICS_ADDR` (9464)".
...
- [ ] **Step 5: Commit** — `git add bins/server/src/router/mod.rs bins/server/src/router/route.rs bins/server/src/main.rs deploy/rustic-git.yaml deploy/alerts.md tests/metrics.rs && git commit -m "Move server metrics off the peer listener onto their own port"`
...
- Produces: `fleet_store_ok(url: &str) -> Result<()>` (signature unchanged); env `RUSTIC_GIT_ALLOW_MEM_FLEET`
...
- [ ] **Step 2: Run it, expect failure** — `cargo test -p rustic-git-storage mem_is_not_a_fleet_store_unless_opted_into` fails with `assertion failed: super::fleet_store_ok("mem://").is_err()`.
...
- [ ] **Step 4: Run tests and clippy** — `cargo test -p rustic-git-storage && cargo test --test routing && cargo clippy --workspace --all-targets --locked -- -D warnings`
...
- [ ] **Step 2: Run it, expect failure** — `cargo build -p rustic-git-git` fails with ``error[E0624]: method `forget_pack` is private``.
...
- Produces: `rustic_git_core::sshkeys::ssh_fingerprint(line: &str) -> rustic_git_core::Result<String>`
...
- [ ] **Step 2: Run it, expect failure** — `cargo test -p rustic-git-core --features ssh a_public_key_line_fingerprints_and_anything_else_is_refused` fails to compile: ``error[E0425]: cannot find function `ssh_fingerprint` in module `super` ``.
...
`bins/server/Cargo.toml:18`: `rustic-git-core = { path = "../../crates/core", features = ["ssh"] }`.
`crates/api/Cargo.toml:12`: `rustic-git-core = { path = "../core", features = ["ssh"] }`.
...
- [ ] **Step 4: Run tests and clippy** — `cargo test -p rustic-git-core --features ssh && cargo test --workspace && cargo clippy --workspace --all-targets --locked -- -D warnings`, plus `cargo tree -p rustic-git-storage -i russh` to confirm storage still pulls no ssh parse of its own (it may appear transitively through a sibling; the manifest is what the rule is about).
...
- [ ] **Step 4: Run tests and clippy** — `cargo test -p rustic-git-server && cargo clippy --workspace --all-targets --locked -- -D warnings`
...
**Interfaces:**
- Consumes: `open()` at `bins/server/src/router/git.rs:44-46` and `rustic_git_core::httpx::user_names` (no code change — this is the missing test in L6)
