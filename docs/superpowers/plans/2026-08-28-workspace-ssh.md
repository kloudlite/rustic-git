...
- Session token: JWT `typ: "ssh-session"`, claims `{ sub: <owner username>, ws: <workspace id>, region, jti, iat, exp }`, `exp - iat = 60`. Single use per gateway replica (in-memory `jti` set with expiry). Signed with the same `KLOUDLITE_JWT_SECRET` the api uses; the gateway gets it from Secret `kloudlite-jwt` in `kube-system` (k3s).
...
- Pod: default image only; command `["/nix/profile/current/bin/sshd", "-D", "-e", "-f", "/etc/ssh/sshd_config"]`; `/etc/ssh` from Secret `ws-ssh-{id}` (`ssh_host_ed25519_key` 0400, `ssh_host_ed25519_key.pub`, `sshd_config`), `/root/.ssh/authorized_keys` from Secret `user-key` key `authorized_keys` (0600). `sshd_config` exactly as in the spec. NetworkPolicy: ingress 22 only from pods labelled `app=kloudlite-gateway` in namespace `kube-system`.
...
- `/v1` is public on `dev.kloudlite.io` via a path rule to `kloudlite-api`, rate-limited like the app.
...
```
- [ ] **Step 2: Run** `cargo test -p kloudlite-core jwt` → compile error.
...
- [ ] **Step 4: Run** `cargo test -p kloudlite-core && cargo clippy -p kloudlite-core -- -D warnings`.
...
- [ ] **Step 4: Run** `cargo test -p kloudlite-api && cargo clippy -p kloudlite-api -- -D warnings`.
...
- [ ] **Step 4: Run** `cargo test -p kloudlite-workspaces && cargo clippy -p kloudlite-workspaces -p kloudlite-api-bin -- -D warnings` (check the api bin's package name).
...
- Produces:
  ...
  NetworkPolicy `allow-gateway-ssh`: ingress port 22 from `namespaceSelector kubernetes.io/metadata.name=kube-system` + `podSelector app=kloudlite-gateway`.
...
- [ ] **Step 4: Run** `cargo test -p kloudlite-workspaces -p kloudlite-agent-bin && cargo clippy --workspace -- -D warnings && CRD_REGEN=1 cargo test -p kloudlite-workspaces --test crd_yaml`.
...
**Files:**
- Create: `bins/gateway/Cargo.toml` (package `kloudlite-gateway-bin`, bin `kloudlite-gateway`), `bins/gateway/src/main.rs`, `bins/gateway/src/lib.rs` (`tunnel.rs`: auth + pump; `resolve.rs`: pod IP), `bins/gateway/tests/tunnel.rs`
- Modify: root `Cargo.toml` members; `Dockerfile` (a `gateway` stage from the same builder: debian-slim, non-root uid 1001, `ENTRYPOINT ["kloudlite-gateway"]`); `.github/workflows/image.yml` (build/push `ghcr.io/kloudlite/kloudlite-gateway:<sha>`)
- Create: `deploy/k3s/gateway.yaml` (ServiceAccount + ClusterRole `get` workspaces, `get` pods; Deployment 2 replicas in `kube-system`, label `app=kloudlite-gateway`, env `KLOUDLITE_JWT_SECRET` from Secret `kloudlite-jwt`, `WS_REGION`; Service `kloudlite-gateway:8080`)
...
- [ ] **Step 4: Run** `cargo test -p kloudlite-gateway-bin && cargo clippy -p kloudlite-gateway-bin -- -D warnings`; `kubectl apply --dry-run=client -f deploy/k3s/gateway.yaml`.
...
- Modify: `web/apps/web/src/lib/api.ts` (`ApiWorkspace.ssh?: { gateway: string; host_key: string }`, `ApiCredential.material?: string`, cli token calls), `web/apps/web/src/app/(shell)/[owner]/(org)/settings/page.tsx` (CLI tokens section), `web/apps/web/src/components/app/workspace-list.tsx` (an `ssh` popover on the row: `kl ws ssh <name>` + "copy ssh config" of the Host block; hidden when `ssh` is absent), the keys settings component (badge "re-add to use for SSH" when `material` is empty), `deploy/kloudlite-web.yaml` (path rule `/v1/(.*)` → `kloudlite-api:80` before `/`, same rate-limit annotations)
- [ ] **Step 1**: tsc/lint-driven; a bun test for the ssh-config block renderer shared with the snippet.
- [ ] **Step 2–4**: implement; `bunx tsc --noEmit -p apps/web/tsconfig.json && bun run lint && bun test`; `kubectl apply --dry-run=client -f deploy/kloudlite-web.yaml`.
...
2. k3s: `kubectl apply -f crds.yaml -f agent-rbac.yaml`; copy `kloudlite-jwt` Secret from AKS to k3s `kube-system`; `-f gateway.yaml -f cloudflared.yaml`; agent DaemonSet repin.
...
- Types: `SshSessionClaims`/`CliClaims` (T1) used by T3/T5/T7; `Workspace.ssh` (T3) by T8; `ws-ssh-{id}` + `authorized_keys` (T3/T4) by T5's NetworkPolicy assumption (gateway label `app=kloudlite-gateway` in `kube-system`, T5/T4 agree).
