...
- *Ops P1 "server memory requests → 256Mi"*: stale. `deploy/kloudlite.yaml` already requests 384Mi on `kloudlite-srv` with a measured rationale in the comment above it, 96Mi on the leader (measured 8Mi steady), and 256Mi on api/worker. Nothing to change; tell the user to re-validate against live RSS if they still want to.
...
**Files:**
- Modify: `deploy/kloudlite.yaml` — the `env:` blocks of all four Rust workloads: `kloudlite-leader` (env starts ~line 56), `kloudlite-srv` (~line 288), `kloudlite-api` (~line 608), `kloudlite-worker` (~line 715). (Spec says "three workloads"; there are four — leader, srv, api, worker — all get the var.)
- Modify: `deploy/kloudlite-web.yaml` — web `env:` block (~line 34) and the readinessProbe `periodSeconds: 5` at line 90.
...
- [ ] **Step 1:** In `deploy/kloudlite.yaml`, add to each of the four containers' `env:` lists (comment once per workload, WHY-style):
...
- [ ] **Step 2:** In `deploy/kloudlite-web.yaml`, add to the web container's `env:`:
...
- [ ] **Step 4:** `kubectl apply --dry-run=client -f deploy/kloudlite.yaml -f deploy/kloudlite-web.yaml` to validate syntax (or `kubectl create --dry-run=client` if not connected — any yaml parse is enough).
...
async fn getting_one_repo_asks_the_directory_before_anything_else() {
    ...
    let up = upstream(axum::http::StatusCode::OK).await;
    let base = api_with_jwt(&e, &up, KEY).await;
    let token = kloudlite::jwt::Jwt::new(KEY).unwrap().mint("k@example.com", "K", Some("k")).unwrap();
