...
- The public listeners (`8080`, `2222`) must **never** honour `X-Kloudlite-Owner` or any identity claim. Only the peer listeners (`8081`, `8082`) do.
...
- A request is forwarded at most twice (`X-Kloudlite-Hops`, or the hop count in the stream header). A request out of hops is served where it lands. **The public listener strips this header** — a client must not be able to force a node to serve a repo it does not own.
- The peer listeners require `X-Kloudlite-Peer: <secret>` (HTTP) / the secret in the stream header, from a Kubernetes Secret. Wrong or missing secret → 403, close. **This is in addition to the separate port, not instead of it**: `kolomi-cluster` runs with `networkPolicy: none`, so a NetworkPolicy would be silently accepted and enforce nothing, and pod networking is flat.
...
mod tests {
    ...
    fn peers(n: usize) -> Vec<String> {
        (0..n).map(|i| format!("kloudlite-{i}")).collect()
    }
...
/// Every peer, best first. `peers` are stable pod names (`kloudlite-0`), never IPs — a restarted
...
```rust
    ...
    fn fleet(n: usize) -> Vec<Peer> {
        (0..n).map(|i| peer(&format!("kloudlite-{i}"))).collect()
    }
    ...
    async fn the_top_candidate_serves_without_probing() {
        let f = fleet(3);
        let repo = repo_where_i_rank(&f, "kloudlite-0", 0);
        let m = Membership::fixed(f, "kloudlite-0".into());
    ...
    async fn second_forwards_up_when_first_answers() {
        let f = fleet(3);
        let repo = repo_where_i_rank(&f, "kloudlite-1", 1);
        let n = names(&f);
        let first = rank(&repo, &n)[0].clone();
        let m = Membership::fixed(f.clone(), "kloudlite-1".into());
        let up: &'static [&str] = &["kloudlite-0", "kloudlite-1", "kloudlite-2"];
    ...
    async fn second_serves_when_first_is_down_from_two_vantages() {
        ...
        let up: &'static [&str] = if first == "kloudlite-0" { &["kloudlite-1", "kloudlite-2"] } else { &["kloudlite-0", "kloudlite-1"] };
        let m = Membership::fixed(f, "kloudlite-1".into());
    ...
    async fn third_defers_to_a_reachable_second() {
        let f = fleet(4);
        let repo = repo_where_i_rank(&f, "kloudlite-2", 2);
        let n = names(&f);
        let ranked = rank(&repo, &n);
        let (first, second) = (ranked[0].clone(), ranked[1].clone());
        let m = Membership::fixed(f.clone(), "kloudlite-2".into());
        let fc = first.clone();
        let fc2 = first.clone();
        let route = m
            .decide(&repo,
    ...
    async fn a_node_outside_the_candidates_never_serves() {
        let f = fleet(6);
        let repo = repo_where_i_rank(&f, "kloudlite-5", 5);
        let m = Membership::fixed(f.clone(), "kloudlite-5".into());
        let n = names(&f);
    ...
    async fn candidates_are_top_three_by_rank_not_top_three_that_are_up() {
        let f = fleet(5);
        let repo = repo_where_i_rank(&f, "kloudlite-3", 3);
        let m = Membership::fixed(f.clone(), "kloudlite-3".into());
        let r = m.decide(&repo, |_: &Peer| std::future::ready(false), |_: &Peer, _: &Peer| std::future::ready(Some(false))).await;
    ...
    async fn higher_ranks_are_probed_concurrently() {
        let f = fleet(4);
        let repo = repo_where_i_rank(&f, "kloudlite-3", 3);
        let m = std::sync::Arc::new(Membership::fixed(f.clone(), "kloudlite-3".into()));
    ...
    async fn self_is_a_member_only_when_dns_lists_it() {
        let f = fleet(3);
        let m = Membership::fixed(f.clone(), "kloudlite-9".into());
        assert!(!m.peers().await.iter().any(|p| p.name == "kloudlite-9"));
        let repo = "o/r";
...
pub struct Membership {
    /// SRV name to resolve, e.g. `_peer._tcp.kloudlite.kloudlite.svc.cluster.local`. Empty
...
**Interfaces:**
- Produces:
  - `pub const OWNER_HEADER: &str = "x-kloudlite-owner"`, `pub const HOPS_HEADER: &str = "x-kloudlite-hops"`, `pub const PEER_HEADER: &str = "x-kloudlite-peer"`, `pub const MAX_HOPS: u32 = 2`
...
use kloudlite::proxy::{Forwarder, HOPS_HEADER, OWNER_HEADER, PEER_HEADER};
...
async fn probe_via_distinguishes_no_from_could_not_ask() {
    let f = Forwarder::new(SECRET.into());
    assert_eq!(f.probe_via(&stub_peer(true).await, "kloudlite-0").await, Some(true));
    assert_eq!(f.probe_via(&stub_peer(false).await, "kloudlite-0").await, Some(false));
    assert_eq!(f.probe_via("127.0.0.1:1", "kloudlite-0").await, None);
}
```

- [ ] **Step 3: Run to verify it fails**

`cargo test --test proxy` → FAIL to compile, `unresolved import kloudlite::proxy`.
...
pub const OWNER_HEADER: &str = "x-kloudlite-owner";
/// How many times this request has been forwarded. Bounds re-forwarding.
pub const HOPS_HEADER: &str = "x-kloudlite-hops";
...
pub const PEER_HEADER: &str = "x-kloudlite-peer";
...
use kloudlite::peers::{Membership, Peer};
use kloudlite::App;
use std::sync::Arc;
...
struct Node {
    store: Arc<kloudlite::store::Store>,
    public: String,
    peer: String,
...
async fn node(os: Arc<dyn slatedb::object_store::ObjectStore>, name: &str, fleet: &[(String, String)]) -> Node {
    ...
    let store = Arc::new(kloudlite::store::Store::open(os, tmp.path().join("cache"), false).await.unwrap());
    ...
    let a2 = app.clone();
    tokio::spawn(async move { axum::serve(pub_l, kloudlite::http::router(a2)).await.unwrap() });
    tokio::spawn(async move { axum::serve(peer_l, kloudlite::http::peer_router(app)).await.unwrap() });
...
fn repo_owned_by(fleet: &[(String, String)], want: &str) -> String {
    ...
    (0..500).map(|i| format!("alice/w{i}")).find(|r| kloudlite::peers::rank(r, &names)[0] == want).unwrap()
}
...
fn repo_ranked(fleet: &[(String, String)], want: &[&str]) -> String {
    ...
    (0..5000).map(|i| format!("alice/w{i}"))
        .find(|r| kloudlite::peers::rank(r, &names).iter().take(want.len()).map(String::as_str).eq(want.iter().copied()))
...
async fn the_public_listener_ignores_a_claimed_identity() {
    ...
    let res = client().await
        ...
        .header(kloudlite::proxy::OWNER_HEADER, "alice")
        .send().await.unwrap();
...
async fn the_public_listener_ignores_a_claimed_hop_count() {
    ...
    let res = client().await
        ...
        .header(kloudlite::proxy::HOPS_HEADER, kloudlite::proxy::MAX_HOPS.to_string())
        .send().await.unwrap();
...
async fn the_peer_listener_requires_the_secret() {
    ...
    for wrong in [None, Some("nope")] {
        let mut r = client().await
            ...
            .header(kloudlite::proxy::OWNER_HEADER, "alice").header("git-protocol", "version=2");
        if let Some(w) = wrong { r = r.header(kloudlite::proxy::PEER_HEADER, w); }
...
async fn the_peer_listener_serves_probes_and_honours_identity() {
    ...
    assert_eq!(c.get(ok("/healthz")).header(kloudlite::proxy::PEER_HEADER, SECRET).send().await.unwrap().status(), 200);
    assert_eq!(c.get(ok("/healthz")).send().await.unwrap().status(), 403);
    let res = c.get(ok("/alice/web/info/refs?service=git-upload-pack"))
        .header(kloudlite::proxy::OWNER_HEADER, "alice").header(kloudlite::proxy::PEER_HEADER, SECRET)
        .header("git-protocol", "version=2").send().await.unwrap();
    assert_eq!(res.status(), 200);
    // /probe: a reports whether it can reach a named peer. It can reach itself.
    let body = c.get(ok("/probe")).query(&[("peer", "a")]).header(kloudlite::proxy::PEER_HEADER, SECRET)
...
async fn a_lower_ranked_node_forwards_up_when_the_owner_is_reachable() {
    ...
    let res = client().await
        ...
        .basic_auth("x", Some(&token)).header("git-protocol", "version=2")
        .header(kloudlite::proxy::HOPS_HEADER, "1").header(kloudlite::proxy::PEER_HEADER, SECRET)
        .send().await.unwrap();
    assert_eq!(res.status(), 200);
...
async fn a_request_out_of_hops_is_refused_unless_local() {
    ...
    let res = client().await
        ...
        .basic_auth("x", Some(&token)).header("git-protocol", "version=2")
        .header(kloudlite::proxy::HOPS_HEADER, kloudlite::proxy::MAX_HOPS.to_string())
        .header(kloudlite::proxy::PEER_HEADER, SECRET)
        .send().await.unwrap();
...
async fn a_confirmed_outage_lets_the_second_candidate_serve() {
    ...
    let second_name = kloudlite::peers::rank(&repo, &names)[1].clone();
...
async fn a_node_fenced_by_a_stray_process_reopens_when_it_is_still_the_owner() {
    ...
    let stray = slatedb::Db::builder(kloudlite::pool::path(o, n), e.store.os.clone()).build().await.unwrap();
...
async fn a_fenced_node_does_not_reopen_when_it_is_not_the_owner() {
    ...
    assert!(kloudlite::pool::is_fenced(&e2), "got: {e2}");
...
```rust
async fn stream_listener(store: Arc<kloudlite::store::Store>) -> String {
    let app = common::app(store);
    let l = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = l.local_addr().unwrap().to_string();
    tokio::spawn(async move { kloudlite::proxy::serve_peer_streams(app, l).await });
    addr
}
...
async fn a_two_hop_ssh_forward_relays_the_status_line() {
    ...
    let mut sock = tokio::net::TcpStream::connect(kloudlite::proxy::stream_addr(&c.peer)).await.unwrap();
...
async fn a_real_ssh_clone_works_through_a_forwarding_node() {
    ...
    // kloudlite::ssh::serve for b on a random port with a generated host key (see ssh_e2e.rs for
...
- [ ] **Step 1b: `node()` starts the stream listener too** — in `tests/routing.rs`'s `node()`, after binding the peer listener, also bind `kloudlite::proxy::stream_addr(&my_addr)` and spawn `serve_peer_streams(app.clone(), l)` on it, so two-hop SSH tests can reach a node's stream port.
    ...
    match app.route(&repo_path).await {
        ...
        crate::peers::Route::Peer(peer) => {
            ...
            let code = match &piped {
                Ok(()) => 0,
                Err(e) => { let _ = handle.extended_data(id, 1, format!("kloudlite: {e}\n").into_bytes()).await; 1 }
            };
...
async fn serve() -> Result<()> {
    let store = Arc::new(Store::open(object_store()?, env("KLOUDLITE_CACHE_DIR", "./cache").into(), true).await?);
    store.spawn_health_probe();

    let peer_addr = env("KLOUDLITE_PEER_ADDR", "0.0.0.0:8081");
    ...
    let (peers, peer_secret) = match std::env::var("KLOUDLITE_PEER_DNS") {
        Ok(dns) if !dns.is_empty() => {
            let me = std::env::var("KLOUDLITE_SELF").ok().filter(|s| !s.is_empty())
                .ok_or_else(|| kloudlite::err("KLOUDLITE_SELF (this pod's name) is required with KLOUDLITE_PEER_DNS"))?;
            let secret = std::env::var("KLOUDLITE_PEER_SECRET").ok().filter(|s| !s.is_empty())
                .ok_or_else(|| kloudlite::err("KLOUDLITE_PEER_SECRET is required with KLOUDLITE_PEER_DNS"))?;
            (kloudlite::peers::Membership::new(format!("_peer._tcp.{dns}:{peer_port}"), me), secret)
        }
        _ => {
            ...
            let solo = kloudlite::peers::Peer { name: "solo".into(), addr: format!("127.0.0.1:{peer_port}") };
            (kloudlite::peers::Membership::fixed(vec![solo], "solo".into()), secret)
        }
    };
    ...
    if std::env::var("KLOUDLITE_PEER_DNS").map(|d| !d.is_empty()).unwrap_or(false) {
        let p = peers.clone();
        let me = std::env::var("KLOUDLITE_SELF").unwrap_or_default();
        tokio::spawn(async move {
    ...
    let app = Arc::new(kloudlite::App::new(store.clone(), peers, peer_secret));
    ...
    let peer_stream = tokio::net::TcpListener::bind(kloudlite::proxy::stream_addr(&peer_addr)).await?;
    ...
    let http_srv = axum::serve(http, kloudlite::http::router(a2)).with_graceful_shutdown(wait(term_rx.clone()));
    let peer_srv = axum::serve(peer_http, kloudlite::http::peer_router(a3)).with_graceful_shutdown(wait(term_rx.clone()));
    ...
    tokio::select! {
        ...
        r = kloudlite::proxy::serve_peer_streams(a4, peer_stream) => { r?; }
...
**Files:** `deploy/kloudlite.yaml`, `README.md`
...
- [ ] **Step 1** — `kubectl -n kloudlite create secret generic kloudlite-peer --from-literal=secret="$(openssl rand -hex 32)"`.
- [ ] **Step 2** — Manifest. Delete the Ingress and `kloudlite-http`. On the container:

```yaml
          env:
            - name: KLOUDLITE_PEER_DNS
              value: kloudlite.kloudlite.svc.cluster.local
            - name: KLOUDLITE_SELF
              valueFrom: { fieldRef: { fieldPath: metadata.name } }   # the stable pod name — the hash key
            - name: KLOUDLITE_PEER_SECRET
              valueFrom: { secretKeyRef: { name: kloudlite-peer, key: secret } }
          ports:
      ...
      affinity:
        podAntiAffinity:
          preferredDuringSchedulingIgnoredDuringExecution:
            - weight: 100
              podAffinityTerm:
                labelSelector: { matchLabels: { app: kloudlite } }
...
```yaml
  ports:
    ...
    - { name: peer, port: 8081, targetPort: peer }   # SRV: _peer._tcp.kloudlite.kloudlite.svc
```
...
kind: NetworkPolicy
metadata:
  name: kloudlite-peers-only
  namespace: kloudlite
spec:
  podSelector:
    matchLabels: { app: kloudlite }
  policyTypes: [Ingress]
  ingress:
    - from:
        - podSelector:
...
- [ ] **Step 5** — Exactly one pod's `/healthz` (via `kubectl exec ... wget -qO- --header="X-Kloudlite-Peer: $SECRET" localhost:8081/healthz`) shows a non-zero warm count. Two → stop.
...
```bash
git add deploy/kloudlite.yaml README.md
