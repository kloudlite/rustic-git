...
**Goal:** Cut the single `rustic_git` library into a Cargo workspace (strategy A: bottom-up, crate-at-a-time behind a shrinking `rustic_git` facade) so each of the three binaries links only what it uses, and the eight files over 700 lines become focused module trees. Zero behavior change; binary names, image contents, deploy yamls, env vars unchanged.
...
- **The facade keeps every consumer compiling mid-migration:** after each extraction, `src/lib.rs` re-exports the moved modules (`pub use rustic_git_<crate>::…`) so `rustic_git::…` paths in `main.rs`, `src/bin/*`, and `tests/*.rs` are untouched until Task 11.
...
- **Binary names unchanged:** `rustic-git`, `rustic-git-api`, `rustic-git-worker` — `[[bin]] name` pins them regardless of package names.
...
```toml
[package]
name = "rustic-git-core"
version = "0.1.0"
edition = "2021"
license = "SSPL-1.0"

[lib]
name = "rustic_git_core"
...
- [ ] **Step 6:** Facade wiring in the root package:
  - root `Cargo.toml`: `members = [".", "crates/core"]`; `[dependencies] rustic-git-core = { path = "crates/core" }`.
  - `src/lib.rs`: delete the moved items and `pub mod jwt; pub mod pktline;`; add
    ...
    ```
  - `src/proxy.rs`: delete the moved items; add at top `pub use rustic_git_core::peer::*;` (keeps every `crate::proxy::X` and `rustic_git::proxy::X` path alive — tests import `rustic_git::proxy::{Forwarder, HOPS_HEADER, OWNER_HEADER, PEER_HEADER}`).
  - `src/http.rs`: delete `Trusted` and `max_body`; add `pub use rustic_git_core::httpx::{max_body, Trusted};` and fix the two `use crate::http::` sites if paths shifted (they should not).
- [ ] **Step 7:** Verify:

```sh
cargo test
cargo clippy --lib -- -D warnings
cargo clippy -p rustic-git-core -- -D warnings
! cargo tree -p rustic-git-core -e normal | grep -qE 'russh|gix-|pgp|slatedb|redis|mongodb'   # core is thin
```
...
```toml
[package]
name = "rustic-git-storage"
version = "0.1.0"
edition = "2021"
license = "SSPL-1.0"

[lib]
name = "rustic_git_storage"
...
```
  ...
  (If `cargo check -p rustic-git-storage` flags an unused entry, delete it; if it flags a missing one — e.g. `axum` should NOT be needed here; if it is, the offending item belongs in a higher crate, stop and re-read the seam.)
- [ ] **Step 2:** Move `store.rs`, `ownership.rs` (+ `ownership/tests.rs`), `index.rs`, `events.rs`, `auth.rs`, `config.rs` verbatim. Inside the crate, rewrite `crate::err`/`crate::Result`/`crate::hex` to `rustic_git_core::…` (or re-export them from `crates/storage/src/lib.rs` as `use rustic_git_core::{err, hex, Error, Result};` so `crate::err` keeps resolving — do the re-export, it is the smaller diff and every later crate repeats the pattern):
...
- [ ] **Step 5:** Facade: root `Cargo.toml` gains `rustic-git-storage = { path = "crates/storage" }` and the member entry. `src/lib.rs` replaces the eight `pub mod` lines with:
...
```sh
cargo test
cargo clippy --lib -- -D warnings && cargo clippy -p rustic-git-storage -- -D warnings
! cargo tree -p rustic-git-storage -e normal | grep -qE 'russh|pgp|gix-pack|gix-traverse|gix-object|axum|imara-diff'
```
...
- [ ] **Step 1:** `crates/gitbase/Cargo.toml` — deps: `rustic-git-core`, `rustic-git-storage`, `gix-odb`, `gix-hash`, `gix-object`, `gix-pack`, `gix-traverse`, `gix-features`, `gix-actor`, `flate2`, `tokio`, `serde` (all `{ workspace = true }` except the two path deps). Lib name `rustic_git_gitbase`.
- [ ] **Step 2:** Move `objects.rs` and `refs.rs` verbatim (`crate::store` → `rustic_git_storage::store`, or the same `pub(crate) use` re-export trick as Task 3). `crates/gitbase/src/lib.rs`:

```rust
pub(crate) use rustic_git_core::{err, Error, Result};
...
- [ ] **Step 3:** Move `merge_base` (the whole function, its doc comment, and any private helper only it uses — check `src/browse.rs` around line 442) into `crates/gitbase/src/merge_base.rs`. In `src/browse.rs` add `pub use rustic_git_gitbase::merge_base;` so `crate::browse::merge_base` (callers: `src/pulls.rs:462`, `src/http/browse_api/merge.rs:136`) and test imports keep resolving.
- [ ] **Step 4:** Facade: `src/lib.rs` → `pub use rustic_git_gitbase::{objects, refs};` replacing the two `pub mod` lines; root Cargo.toml member + dep.
- [ ] **Step 5:** Verify:

```sh
cargo test
cargo clippy --lib -- -D warnings && cargo clippy -p rustic-git-gitbase -- -D warnings
! cargo tree -p rustic-git-gitbase -e normal | grep -qE 'russh|pgp|axum|redis|mongodb'
```
...
- **`model.rs`** — everything the worker imports, zero gix: `pull_key`, `PullRequest`, `Mergeability`, `MergeJob`, `PullState`, `Comment`, the `ms`/`ms_opt` deserializers, `get`, `put`, `list`, `with_merge_jobs`, `open_only`, `next_number`, `Source`, `ensure_migrated`, `migrate_from`, `is_migrated`, and **`Deep`** (moved out of the check section — `src/bin/worker.rs` names `rustic_git::pulls::Deep` at lines 254/355, so it must not sit behind the gix gate).
- **`check.rs`** — gix, server-only, **feature-gated** (`#[cfg(feature = "check")] pub mod check;` with `pub use check::*` under the same cfg): `CHECK_LIMIT`, `check`, `check_with`, `check_repo`, `Checked` (its `Deep` payload comes from `model`). This is the only module that touches `gix_hash::ObjectId`, `store::Repo::odb()`, and `gitbase::merge_base` — the feature gates optional deps `gix-hash` and `rustic-git-gitbase`.
...
- The worker links `rustic-git-pulls` with default features (no `check`); the facade — and later `bins/server` — enables `features = ["check"]`. The worker_merges suite (37 tests) is the guard that no code moved across an await boundary.
...
```toml
[package]
name = "rustic-git-pulls"
version = "0.1.0"
edition = "2021"
license = "SSPL-1.0"

[lib]
name = "rustic_git_pulls"
...
check = ["dep:gix-hash", "dep:rustic-git-gitbase"]

[dependencies]
rustic-git-core = { path = "../core" }
rustic-git-storage = { path = "../storage" }
rustic-git-gitbase = { path = "../gitbase", optional = true }
...
- [ ] **Step 5:** Facade: `src/lib.rs` → `pub use rustic_git_pulls::{directory, merge_worker, pulls};` (root dep declared with `features = ["check"]`).
- [ ] **Step 6:** Verify:

```sh
cargo test        # includes the 37-test worker_merges guard inside tests/pulls.rs
cargo clippy --lib -- -D warnings && cargo clippy -p rustic-git-pulls --all-features -- -D warnings
! cargo tree -p rustic-git-pulls -e normal | grep -qE 'gix-|russh|pgp'        # default features: no gix
cargo tree -p rustic-git-pulls -e normal --features check | grep -q gix-hash  # check pulls it in
```
...
```
  ...
  In `src/main.rs` `spawn_lease_tasks`, change the three call sites `a.reconcile_owned_markers().await` → `rustic_git::reconcile_owned_markers(&a).await` (same for the other two). This is the plan's one sanctioned call-shape change; nothing about ordering, intervals, or bodies moves.
- [ ] **Step 4:** Verify:

```sh
cargo test
cargo clippy --lib -- -D warnings && cargo clippy -p rustic-git-app -- -D warnings
! cargo tree -p rustic-git-app -e normal | grep -qE 'gix-|russh|pgp|axum'   # app stays gix-free
```
...
- [ ] **Step 1:** `crates/git/Cargo.toml` — package `rustic-git-git`, lib `rustic_git_git`; deps: `rustic-git-core`, `rustic-git-storage`, `rustic-git-gitbase`, `rustic-git-app`, and workspace deps `tokio`, `tokio-util`, `axum`, `russh`, `gix-odb`, `gix-pack`, `gix-object`, `gix-hash`, `gix-traverse`, `gix-features`, `flate2`, `futures`, `imara-diff`, `serde`, `serde_json`, `base64`. (`imara-diff` is browse's; confirm with `cargo check -p` and prune unused.)
...
- [ ] **Step 3:** Move `protocol/mod.rs`, `protocol/receive.rs`, `browse.rs` (minus `merge_base`, already gone; keep its `pub use rustic_git_gitbase::merge_base;`), `ssh.rs`, `gc.rs` verbatim. `receive.rs`'s `crate::http::max_body` becomes `rustic_git_core::httpx::max_body`.
- [ ] **Step 4:** Move the server half of `src/proxy.rs` (`HEADER_MAX`, `HEADER_TIMEOUT`, `serve_peer_streams`, `serve_peer_stream`) to `crates/git/src/proxy.rs`, whose top reads `pub use rustic_git_core::peer::*;` so `proxy::Forwarder` etc. keep one canonical path. Delete `src/proxy.rs`.
...
```
  ...
  Facade `src/lib.rs`: `pub use rustic_git_git::{browse, gc, protocol, proxy, ssh};`
- [ ] **Step 6:** Verify:

```sh
cargo test        # protocol.rs, ssh_e2e.rs, pack_cap.rs, proxy.rs suites all still through the facade
cargo clippy --lib -- -D warnings && cargo clippy -p rustic-git-git -- -D warnings
! cargo tree -p rustic-git-git -e normal | grep -qE 'pgp|mongodb'
```
...
- [ ] **Step 1:** `crates/registry/Cargo.toml` — package `rustic-git-registry`, lib `rustic_git_registry`; deps: `rustic-git-core`, `rustic-git-storage`, `rustic-git-app`, and `tokio`, `axum`, `futures`, `serde`, `serde_json`, `base64`, `chrono`, `form_urlencoded`, `reqwest`. No gix, no russh, no pgp.
- [ ] **Step 2:** Move all nine files byte-for-byte; `src/registry/mod.rs` becomes `crates/registry/src/lib.rs` (prepend `#![allow(clippy::result_large_err)]` and the cross-crate `pub(crate) use` block: `rustic_git_core::{err, hex, jwt, httpx::Trusted}`, `rustic_git_storage::{auth, index, ownership, pool, store}`, `rustic_git_app::App`). The blob-deletion rule, verbatim manifests, `Digest::parse`, `oci_err` envelope: moved, not edited.
- [ ] **Step 3:** Facade: `src/lib.rs` → `pub use rustic_git_registry as registry;` (keeps `rustic_git::registry::…` and, inside the facade, `crate::registry::…`).
- [ ] **Step 4:** Verify:

```sh
cargo test        # registry_{blobs,http,gc,limits,manifests,store,uploads}.rs suites
cargo clippy --lib -- -D warnings && cargo clippy -p rustic-git-registry -- -D warnings
! cargo tree -p rustic-git-registry -e normal | grep -qE 'gix-|russh|pgp|mongodb|imara-diff'
```
...
[package]
name = "rustic-git-server"
version = "0.1.0"
edition = "2021"
license = "SSPL-1.0"

[lib]                      # thin lib so the workspace test host (Task 11) can drive the router
name = "rustic_git_server"
path = "src/lib.rs"

[[bin]]
name = "rustic-git"        # binary name unchanged
path = "src/main.rs"
...
rustic-git-core = { path = "../../crates/core" }
rustic-git-storage = { path = "../../crates/storage" }
rustic-git-pulls = { path = "../../crates/pulls" }     # directory; no check feature
...
rustic-git-core = { path = "../../crates/core" }
rustic-git-storage = { path = "../../crates/storage" }
rustic-git-pulls = { path = "../../crates/pulls" }     # model + jobs + merge_worker; NO check
rustic-git-registry = { path = "../../crates/registry" }
tokio = { workspace = true }
rustls = { workspace = true }
reqwest = { workspace = true }
...
```
  ...
  In each moved `main.rs`, rewrite `rustic_git::X` to the owning crate path (mechanical: `rustic_git::config` → `rustic_git_storage::config`, `rustic_git::merge_worker` → `rustic_git_pulls::merge_worker`, `rustic_git::proxy::PEER_HEADER` → `rustic_git_core::peer::PEER_HEADER`, `rustic_git::registry::gc` → `rustic_git_registry::gc`, …). Bins are the first consumers to leave the facade; tests stay on it until Task 11.
...
- [ ] **Step 3 (spec file split — http.rs → bins/server/src/router/):** `router/route.rs` — the routing middleware and ownership plumbing: `repo_of`, `api_prefixed`, `git_shape`, `api_route`, `is_git_route`, `route_public`, `route_peer`, `route_inner`, `trust_peer`, `trust_nobody`, `healthz`, `own_claim`, `own_renew`, `own_release`, `own_draining`, `two_lines`, `leader_only`, `BROWSE_TAILS`, and the `every_browse_route_is_routable` test beside it. `router/git.rs` — the git handlers: `open`, `body_reader`, `reopen_after_fence`, `info_refs`, `upload_pack`, `receive_pack`, `respond_first`, `is_client_fault`, `success`, `git_routes`. `router/limits.rs` — `max_decompressed`, `internal`, `client_err`, `bad_request`, `fenced_elsewhere` (plus a `pub use rustic_git_core::httpx::{max_body, Trusted};`). `router/mod.rs` — `router()`, `peer_router()`, re-exports. Move `src/http/browse_api/` to `bins/server/src/browse_api/` unchanged (its `crate::…` paths point at the re-export block in `bins/server/src/lib.rs`).
- [ ] **Step 4:** `bins/server/src/lib.rs` exposes what main.rs and the test host need:

```rust
#![allow(clippy::result_large_err)]
pub(crate) use rustic_git_core::{err, Error, Result};
pub(crate) use rustic_git_storage::{auth, cache, events, index, ownership, pool, store};
pub(crate) use rustic_git_gitbase::{objects, refs};
pub(crate) use rustic_git_pulls::{directory, merge_worker, pulls};
pub(crate) use rustic_git_app::App;
pub(crate) use rustic_git_git::{browse, protocol, proxy, ssh};
pub(crate) use rustic_git_registry as registry;
pub mod boot;
pub mod browse_api;
pub mod lanes;
pub mod listeners;
...
ls target/release/rustic-git target/release/rustic-git-api target/release/rustic-git-worker  # all three, same names
cargo test          # includes every_browse_route_is_routable in its new home
cargo clippy --workspace -- -D warnings
! cargo tree -p rustic-git-api-bin -e normal    | grep -qE 'russh|gix-pack|gix-odb|gix-traverse|imara-diff'
! cargo tree -p rustic-git-worker-bin -e normal | grep -qE 'russh|pgp|gix-pack|gix-traverse|imara-diff'
cargo tree -p rustic-git-worker-bin -e normal | grep -q slatedb    # worker keeps its store
```
...
**Tests decision, resolved by reading `tests/*.rs`:** the suites exercise the composed server (`rustic_git::App`, router, registry, protocol — imports span seven crates), so they stay together at the **workspace root**: the root package stops being a facade and becomes the integration-test host `rustic-git-tests` (near-empty lib, all dev-dependencies). This — rather than attaching to `bins/server` — keeps `./tests/registry_e2e.sh`, `./tests/compat-matrix.sh`, and `cargo test --test registry_blobs` working from the repo root exactly as CLAUDE.md documents them.
...
- [ ] **Step 1:** Root `Cargo.toml`: rename the package to `rustic-git-tests`, drop `[lib] name = "rustic_git"` (plain default lib), move every runtime dependency to `[dev-dependencies]` as path deps on all nine workspace crates (pulls with `features = ["check"]`, plus `rustic-git-server` for the router/boot helpers) plus the existing dev deps (`slatedb-common`, `tower`, `tempfile`, `async-trait`, `serde_json`, `mongodb`, `tokio`, `axum`, `futures`, `reqwest`, `russh` — prune to what the tests actually name). `src/lib.rs` shrinks to `//! Integration-test host; the code lives in crates/ and bins/.`
- [ ] **Step 2:** Rewrite test imports once, mechanically:

```sh
grep -rl 'rustic_git::' tests/ | xargs sed -i '' \
  -e 's/rustic_git::App/rustic_git_app::App/g' \
  -e 's/rustic_git::\(store\|pool\|ownership\|index\|cache\|events\|auth\|config\)/rustic_git_storage::\1/g' \
  -e 's/rustic_git::\(objects\|refs\)/rustic_git_gitbase::\1/g' \
  -e 's/rustic_git::\(pulls\|directory\|merge_worker\)/rustic_git_pulls::\1/g' \
  -e 's/rustic_git::\(protocol\|browse\|ssh\|gc\)/rustic_git_git::\1/g' \
  -e 's/rustic_git::proxy/rustic_git_core::peer/g' \
  -e 's/rustic_git::registry/rustic_git_registry::/g;s/registry::::/registry::/g' \
  -e 's/rustic_git::\(pktline\|jwt\|err\|hex\)/rustic_git_core::\1/g'
```

  then `cargo test --no-run` and fix the residue by hand (router/`http` references → `rustic_git_server::router`, `Trusted` → `rustic_git_core::httpx::Trusted`). `tests/common/` gets the same treatment.
...
cargo test
cargo clippy --workspace -- -D warnings
cargo build --release --locked && ls target/release/rustic-git{,-api,-worker}
./tests/registry_e2e.sh || test $? -eq 77          # 77 = docker half skipped, not a pass — note which
grep -rn 'rustic_git::' tests/ src/ | wc -l        # 0 — the facade name is gone
docker build . -t rustic-git-workspace-check       # if a daemon is available; otherwise flag for CI
```
