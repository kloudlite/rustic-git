...
worker) that runs `kloudlite-agent` as a privileged DaemonSet against a host btrfs pool at
...
control-plane node  (2 OCPU / 8 GB)   k3s server, SQLite datastore, taint node-role.kubernetes.io/control-plane=true:NoSchedule
        ...
        +-- session node (32 OCPU / 128 GB)  label kloudlite.io/role=session  taint kloudlite.io/role=session:NoSchedule
        |     /wspool-prod  (dedicated data disk, btrfs)  -> agent DaemonSet pod (privileged, hostPath)
        +-- env node     (16 OCPU / 128 GB)  label kloudlite.io/role=env      taint kloudlite.io/role=env:NoSchedule
              /wspool-prod  (dedicated data disk, btrfs)  -> agent DaemonSet pod (privileged, hostPath)
```

The agent is a **node-level Kubernetes controller**: it watches kloudlite CRDs filtered to its own
...
server. This cluster holds no kloudlite server pods, no SlateDB, and no
...
- Every manifest is committed under `deploy/` in this repo's existing style: one file per concern,
  comments explain WHY only, matching the density of `deploy/kloudlite.yaml`.
...
| `deploy/k3s/agent-rbac.yaml` | Namespace `kloudlite-ws`, ServiceAccount `kloudlite-agent`, ClusterRole/Role + bindings: watch/list/get on the CRDs, update on their `/status`, and the pods/namespaces it manages. |
...
- [ ] **Step 2:** Create the container and a write-only SAS scoped to it (permissions `cwr`, no
  ...
  control plane at `/etc/kloudlite/k3s-backup.sas`, mode 600, root-owned. Never `cat` it.
...
- [ ] **Step 4:** Run it once and verify the artifact is genuinely restorable, not merely uploaded:
  ...
  tar -xzf /tmp/r.tgz -C /tmp && sqlite3 /tmp/state.db "select count(*) from kine where name like '%kloudlite.io%';"
  ```
...
- [ ] **Step 1:** State the verification first — show the target does not exist yet.
  `az group show -n kloudlite-k3s -o tsv --query name`
...
- [ ] **Step 3:** Join the session node (`K3S_TOKEN_FILE` keeps the token out of the process table
  ...
  curl -sfL https://get.k3s.io | INSTALL_K3S_VERSION=v1.33.5+k3s1 \
    ...
    --node-label kloudlite.io/role=session \
    --node-taint kloudlite.io/role=session:NoSchedule
  ```
- [ ] **Step 4:** Same for the env node with `kloudlite.io/role=env` in both flags.
- [ ] **Step 5:** Verify labels and taints landed (a label without its taint is the dangerous half —
  ...
  `kubectl get nodes -L kloudlite.io/role -o custom-columns=N:.metadata.name,ROLE:.metadata.labels.kloudlite\\.io/role,TAINTS:.spec.taints[*].key`
  Expected: three rows; `k3s-cp <none> node-role.kubernetes.io/control-plane`,
  `k3s-session session kloudlite.io/role`, `k3s-env env kloudlite.io/role`.
...
- [ ] **Step 1:** Show absence: `kubectl get crd -o name | grep kloudlite.io || echo NONE`
  Expected: `NONE`.
...
- [ ] **Step 4:** Verify the API really serves them, not just that the CRD object exists:
  `kubectl api-resources --api-group=kloudlite.io`
  Expected: a row per kind with its short name and `NAMESPACED` value; and
  `kubectl get --raw /apis/kloudlite.io/v1alpha1 | head -c 200` returns an `APIResourceList`
...
- [ ] **Step 1:** Show absence: `kubectl -n kloudlite-ws get sa kloudlite-agent`
  Expected: `Error from server (NotFound)`.
- [ ] **Step 2:** Write `agent-rbac.yaml`: `Namespace kloudlite-ws`, `ServiceAccount
  kloudlite-agent`, and:
  - a **ClusterRole `kloudlite-agent-crd`** — `apiGroups: [kloudlite.io]`, `resources: [<kinds>]`,
  ...
  - a **Role in `kloudlite-ws`** granting `pods`, `pods/log`, `pods/exec`, `services`,
...
- [ ] **Step 3:** Create the secrets from files, never from literals on the command line
  ...
  kubectl -n kloudlite-ws create secret generic kloudlite-agent \
    --from-file=token=/tmp/tok --from-file=azure-key=/tmp/azkey --from-file=cosmos-key=/tmp/ckey
  shred -u /tmp/tok /tmp/azkey /tmp/ckey
  ```
  Verify without revealing: `kubectl -n kloudlite-ws get secret kloudlite-agent -o jsonpath='{range $.data.*}{@}{"\n"}{end}' | wc -l`
  Expected: `3`.
- [ ] **Step 4:** Apply and verify the roles permit exactly what is intended, negatives included:
  ```sh
  SA=system:serviceaccount:kloudlite-ws:kloudlite-agent
  kubectl auth can-i watch <kind>.kloudlite.io --as=$SA -A          # yes
  kubectl auth can-i update <kind>.kloudlite.io/status --as=$SA -A  # yes
  kubectl auth can-i delete <kind>.kloudlite.io --as=$SA -A         # no
  kubectl auth can-i create pods --as=$SA -n kloudlite-ws           # yes
...
- [ ] **Step 1:** Show absence: `kubectl -n kloudlite-ws get ds`
  Expected: `No resources found`.
- [ ] **Step 2:** Write the DaemonSet — two DaemonSets in one file (`kloudlite-agent-session`,
  `kloudlite-agent-env`) rather than one with a broad selector, because the two roles get
  different `WS_CPU`/`WS_MEM_MB` and different tolerations. Each carries:
  - `nodeSelector: { kloudlite.io/role: session }` (resp. `env`) **and** a matching
  ...
  - `serviceAccountName: kloudlite-agent` — without it the pod gets `default`, whose token can
...
- [ ] **Step 3:** Apply and verify placement — exactly one pod per worker, none on the control plane:
  `kubectl -n kloudlite-ws get pod -o custom-columns=N:.metadata.name,NODE:.spec.nodeName,S:.status.phase`
...
- [ ] **Step 4:** Verify the pool is genuinely the host's, not an empty ephemeral dir:
  `kubectl -n kloudlite-ws exec ds/kloudlite-agent-session -- btrfs subvolume list /wspool-prod`
...
- [ ] **Step 5:** **Verify each pod can reach the API server AND actually establish its watch.** A
  ...
  kubectl -n kloudlite-ws exec ds/kloudlite-agent-session -- \
    sh -c 'wget -qO- --no-check-certificate --header="Authorization: Bearer $(cat /var/run/secrets/kubernetes.io/serviceaccount/token)" \
      https://kubernetes.default.svc/apis/kloudlite.io/v1alpha1 | head -c 120'
  # b) the controller says so itself
  kubectl -n kloudlite-ws logs ds/kloudlite-agent-session --tail=30
  ```
...
- [ ] **Step 5b:** Prove the watch is **scoped to its own node** — otherwise two agents race the
  ...
  `kubectl -n kloudlite-ws logs ds/kloudlite-agent-env --tail=10`
...
- [ ] **Step 6:** Namespace convention + default deny. Environments get one namespace each,
  `env-{id}`; workspaces stay in `kloudlite-ws`. Write `netpol-env-namespace.yaml` as a template
...
- [ ] **Step 7:** Prove the template denies, in a throwaway namespace, the same before/after shape
  as Task 5 Step 4: create `env-probe`, run a pod, `wget` a pod in `kloudlite-ws` (succeeds),
...
- [ ] **Step 1:** Confirm the old agent is still healthy and still serving its own region:
  `ssh 20.219.39.174 'systemctl is-active kloudlite-agent && journalctl -u kloudlite-agent --since -5m --no-pager | tail -5'`
...
- [ ] **Step 2:** Rehearse the rollback for real: scale both DaemonSets to zero by adding a
  ...
  `kubectl -n kloudlite-ws patch ds kloudlite-agent-session -p '{"spec":{"template":{"spec":{"nodeSelector":{"kloudlite.io/role":"disabled"}}}}}'`
  (and the same for `-env`).
  Expected: `kubectl -n kloudlite-ws get pod` → `No resources found` within ~30s.
