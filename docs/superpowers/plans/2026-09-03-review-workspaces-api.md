...
- **Manifest is generated:** any `crd.rs` change to a spec/status struct or a `#[kube(...)]` attribute requires `CRD_REGEN=1 cargo test -p rustic-git-workspaces --test crd_yaml` and the regenerated `deploy/k3s/crds.yaml` in the same commit.
...
Run: `cargo test -p rustic-git-workspaces --test api_volumes -- --test-threads=1; echo exit=$?`
...
Run: `cargo test -p rustic-git-workspaces -p rustic-git-agent-bin -- --test-threads=1; echo exit=$?` then `cargo clippy --workspace --all-targets --locked -- -D warnings; echo exit=$?`
...
Run: `cargo test -p rustic-git-workspaces --test api_user creating_past_the_per_owner_cap -- --test-threads=1; echo exit=$?`
...
Run: `cargo test -p rustic-git-workspaces --lib the_namespace_caps_aggregate -- --test-threads=1; echo exit=$?`
...
```rust
            ...
            &k8s::resource_quota(
                ...
                rustic_git_workspaces::model::max_per_owner(),
            ),
            ctx,
        )
        .await?;
...
Run: `cargo test -p rustic-git-workspaces -p rustic-git-agent-bin -- --test-threads=1; echo exit=$?` and `cargo clippy --workspace --all-targets --locked -- -D warnings; echo exit=$?`
Expected: exit=0 both.
...
Run: `cargo test -p rustic-git-workspaces --test api_user --test api_volumes -- --test-threads=1; echo exit=$?`
...
Run: `cargo test -p rustic-git-workspaces -p rustic-git-agent-bin -- --test-threads=1; echo exit=$?` and the clippy gate.
Expected: exit=0 both.
...
Run: `cargo test -p rustic-git-workspaces --test api_volumes a_restore_that_has_not_checked_out -- --test-threads=1; echo exit=$?`
...
Run: `cargo test -p rustic-git-workspaces --test api_user listing_regions_never_names -- --test-threads=1; echo exit=$?`
...
Run: `cargo test -p rustic-git-workspaces --test crd_yaml --test api_volumes -- --test-threads=1; echo exit=$?`
...
Run: `cargo test -p rustic-git-workspaces --test api_user restore_refuses -- --test-threads=1; echo exit=$?`
...
- Modify: `deploy/k3s/api-rbac.yaml`, `deploy/k3s/README.md`, `deploy/rustic-git.yaml:132-136,428-432`, `tests/ws_e2e.sh:69,105-108,146-151,222-256`, `.local/run-api.sh`, `CLAUDE.md`
...
- Produces:
  ...
  #[kube(group = "rustic-git.io", version = "v1alpha1", kind = "Region", plural = "regions",
...
Run: `cargo test -p rustic-git-workspaces --test api_user --test crd_yaml -- --test-threads=1; echo exit=$?`
...
`deploy/rustic-git.yaml`: delete both `COSMOS_ENDPOINT`/`COSMOS_KEY`/`COSMOS_DB` env blocks (`:132-136`, `:428-432`).
...
`.local/run-api.sh`: no change is needed for regions — it never set `COSMOS_*`. Verify the rest of what it needs is still true: `RUSTIC_GIT_MONGO_URI` (directory), `RUSTIC_GIT_JWT_SECRET`, `RUSTIC_GIT_S3_URL=mem://`, `RUSTIC_GIT_PEER_SECRET`, `KUBECONFIG=.local/k3s.yaml`. Add one line to its header comment: regions now live in the k3s cluster the `KUBECONFIG` points at, so a local API sees whatever regions that cluster holds.
...
cargo test -p rustic-git-workspaces -p rustic-git-agent-bin -- --test-threads=1; echo exit=$?
cargo clippy --workspace --all-targets --locked -- -D warnings; echo exit=$?
cargo tree -p rustic-git-workspaces | grep -c azure; echo "azure deps above must be 0"
```
...
- Produces: no public API change. `rustic_git_workspaces::api::{router, ApiState, Directory, OwnerMaterial, refresh_user_keys, owner_set_selector, ATTACHED_ENV_LABEL, KIND_LABEL, OWNER_LABEL, TEAM_LABEL}` all still resolve — `mod.rs` re-exports whatever moved.
...
Run: `cargo test -p rustic-git-workspaces --test api_user every_v1_route_is_still_mounted -- --test-threads=1; echo exit=$?`
...
Run: `cargo test -p rustic-git-workspaces --test crd_yaml restore_of_is_gone -- --test-threads=1; echo exit=$?`
...
Run: `cargo test -p rustic-git-workspaces --test api_user a_workspace_doc_reports_its_volume -- --test-threads=1; echo exit=$?`
...
- `bins/api/src/main.rs`: delete the `with_upstream` call. `RUSTIC_GIT_UPSTREAM`/`RUSTIC_GIT_PEER_SECRET` are still needed by `rustic_git_api::serve` — leave them.
...
Run after each: `cargo check -p rustic-git-workspaces -p rustic-git-agent-bin; echo exit=$?`
...
Run: `cargo check -p rustic-git-workspaces -p rustic-git-agent-bin; echo exit=$?`
