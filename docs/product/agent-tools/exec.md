...
An agent drives a workspace over ssh. There is no exec API and no agent daemon inside the pod: the same ssh a person uses is the transport, which means the same keys, the same audit path, and no second permission model.
