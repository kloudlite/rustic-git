 ...
 *  manual write — a required reason, with a second confirmation for `kloudlite-srv` since
...
export function RollTable({
  ...
  function submit() {
    ...
    const needsSecond = target.name === "kloudlite-srv";
    ...
    <div className="space-y-4">
      ...
      <Dialog open={target !== null} onOpenChange={(open) => !open && setTarget(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{confirmedFirst ? "This rolls kloudlite-srv" : `Roll ${target?.name}`}</DialogTitle>
            <DialogDescription>
              {confirmedFirst
                ? "Rolling kloudlite-srv moves database ownership between nodes (CLAUDE.md, “Deploying”) — a brief window where the first registry request to a moved image can fail once."
