 ...
 *  its action settles, success or not, so without them a rejected 50-line key paste is blank.
 *  The dialog feeds them back as `defaultValue`. */
export type AddKeyState = { ok?: true; error?: string; values?: Record<string, string> } | null;
...
export async function addSshKey(_prev: AddKeyState, formData: FormData): Promise<AddKeyState> {
  ...
  if (!r.ok) {
    ...
    if (r.kind === "invalid") return { error: r.message, values };
    return { error: r.message || "Could not add the key.", values };
  }
...
export type CreateTokenState = { token?: string; name?: string; error?: string; values?: Record<string, string> } | null;
...
export async function createToken(_prev: CreateTokenState, formData: FormData): Promise<CreateTokenState> {
  ...
  const values = { owner, name };
  if (!owner) return { error: "Pick which namespace this token is for.", values };
  if (!name) return { error: "Give the token a name.", values };
  ...
  if (!r.ok) {
    if (r.kind === "invalid") return { error: r.message, values };
    return { error: r.message || "Could not create the token.", values };
  }
  revalidatePath("/settings");
