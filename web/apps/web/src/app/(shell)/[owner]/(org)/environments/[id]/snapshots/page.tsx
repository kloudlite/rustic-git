...
/** One environment's snapshot lineage, newest first — live or archived, the same records.
 *
...
export default async function Page({ params }: { params: Promise<{ owner: string; id: string }> }) {
  ...
  return (
    <EnvSnapshots
      ...
      history={history.map((c) => ({ id: c.id, message: c.message, created_at: c.created_at }))}
      ...
      currentId={env?.restored_to ?? null}
    />
  );
}
