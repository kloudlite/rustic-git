...
export type WorkspaceSession = {
  ...
  repo: string;
  ref: string;
  /** A person, an agent working for a person, or CI/CD — a workspace brought up
   *  to run a trigger, owned by the pipeline and torn down when it finishes. */
  owner: { kind: "user"; login: string; name: string } | { kind: "agent"; name: string; for: string } | { kind: "ci"; trigger: string; run: number };
