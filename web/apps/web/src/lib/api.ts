...
const PEER_SECRET = process.env.KLOUDLITE_PEER_SECRET ?? "";
...
export type ApiWorkspace = {
   ...
   *  `packages`. `source` is where the answer came from: `"nixhub"` or `"mirror"`. */
  locks?: { entry: string; version: string; rev: string; source: string }[];
