 ...
 *
 *  The bench's chat transcripts and sessions are named out loud: deleting the Bench now takes its
 *  folder with it (`kloudlite.io/bench-folder`, `bins/agent/src/controller/bench.rs`), and that is
 *  the one thing here a person cannot get back from a snapshot. */
export function removalConfirm(name: string, team: string, now: number): string {
  const date = ABSOLUTE.format(now + REMOVAL_GRACE_MS);
  return `Remove ${name} from ${team}? Their bench — with its chat transcripts and sessions — their workspaces and their space choice are deleted on ${date} (7 days). Pushed snapshots, repos, images and team environments stay with the team. Takes effect within about 5 minutes.`;
}
...
export function deleteNowConfirm(owner: string, team: string): string {
  return `Type ${owner} to delete their bench — chat transcripts and sessions included — their workspaces and their space choice in ${team} now`;
}
