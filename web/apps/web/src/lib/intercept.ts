 ...
 *  `intercepts` while nothing is in force, which means the workspace is stopped or unreachable
 *  and the REAL service is the one answering. `heldBy` here is the wish's workspace; whether
 *  traffic actually goes there is `ApiEnvironment.service_status`, matched by name and read directly.
 *
