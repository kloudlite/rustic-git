...
No DeepSeek / `kloudlite-ai` key, secret, or reference exists anywhere in this repo (grepped
...
- **Views, never authorization:** `index/` markers, the `kloudlite.io/owner` and `/kind` labels
...
`git.khost.dev:22` → `kloudlite-lb`. The routing middleware derives `{owner}/{name}` from the URL,
...
`KLOUDLITE_MAX_BODY`, 512 MiB in prod), writes objects, and updates refs in its own SlateDB. It
...
fleet-wide `KLOUDLITE_JWT_SECRET`. The manifest request routes on `img/{owner}/{name}` to the node
