...
cargo test                                   # workspace: every crate's unit tests plus tests/*.rs
                                             # integration suite (the root package, `kloudlite-tests`,
...
./tests/ws_e2e.sh                            # real server+api+agent+Azure+btrfs workspaces
                                             ...
                                             # poller — and kloudlite-api serves /v1/*);
...
Run a server locally without S3: `KLOUDLITE_S3_URL=file://./x` (or `mem://`, lost on exit).
...
happened). The ownership map has one writer, **elected**: every `kloudlite-srv` pod runs `App::election_tick`
...
(`announce_stranded_merges` in `bins/server/src/lanes.rs`). The actual merge runs in `kloudlite-worker` using the real
...
`crates/workspaces` + `bins/agent` (`kloudlite-agent`) + `bins/api` (`kloudlite-api`) add a
...
`kloudlite.io/v1alpha1`, all cluster-scoped: `/v1` on `bins/api` writes **spec** (desired), each
...
from a `superadmins` collection in the directory that `KLOUDLITE_WORKSPACES_ADMINS` merely
...
process, `bins/api` started with `KLOUDLITE_API_ROLE=admin` instead of the default `user`, which
...
RBAC mirrors the split: `api-rbac.yaml` gives the `kloudlite-admin` ServiceAccount write on
`Quota`/`QuotaRequest`/`Region` and keeps the ordinary `kloudlite-api` ServiceAccount to reads,
so a bug that mounted the wrong router would still 403 at the Kubernetes layer. The web's
`/superadmin` area calls this second process at `KLOUDLITE_ADMIN_API_URL`, never the ordinary API
...
something this console does. `nodes: patch` on the `kloudlite-admin` ClusterRole
(`deploy/k3s/api-rbac.yaml`) and `pods: get`/`list` on the admin Role (`deploy/kloudlite.yaml`,
...
one `Unavailable`), and stamps `kloudlite.io/decommission-status: draining running=N owned=N
...
started by the pod prelude as `kl` before sshd, on port 7788 of the pod IP, fenced by the
namespace: only the person's own bench (`allow-bench-tools`) and the ssh tunnel (`kl-connect ws ide
<ws>`) reach it, an intercepting environment only on its intercepted ports (never 7788 itself —
refused at `/v1` and by the controller), and `/v1/workspaces/{id}/tools` hands the address to the
owner alone — so there is no second credential and no auth code in the crate. A plain HTTP tool API (`GET /tools` with schemas,
...
(`kloudlite-agent`, privileged, one pod per btrfs-capable node) is a controller, not a worker:
...
that, every team Bench whose team the directory answers does not exist is deleted
(`prune_team_benches` — `delete_team` lives in the directory binary with no kubeconfig), since a
bench is what would otherwise spare a deleted team's namespace. The
...
(`crates/workspaces/src/crd/mod.rs`), a cluster-scoped CR every `kloudlite-agent` watches through
...
`kloudlite.io/restarted-at` on the pod template, never a pod delete — and only after prechecking
...
revert to any of them. The admin API (`bins/api` with `KLOUDLITE_API_ROLE=admin`) exposes
...
pair in every cluster (`deploy/k3s/otel-agent.yaml`, copied into `deploy/kloudlite.yaml` for AKS
...
Snapshot, Volume, QuotaRequest, Request, Node per region; Region centrally) turning transitions — `created`, `started`, `stopped`, and every change of a
parent's `Ready` status or reason as `{kind}.condition` — into rows
...
password, minting its own tokens from `kloudlite-jwt`. The catalogue is Rust
...
placeholder. `KLOUDLITE_HYPERDX_URL` is optional the same way — unset means no "Open in HyperDX"
...
console works with no ClickHouse deployed. `KLOUDLITE_ADMIN_FIXTURES=1` answers admin GETs from
...
`kloudlite-jwt` Secret is required (pods fail closed without it), and Rust pods run as uid 1001
