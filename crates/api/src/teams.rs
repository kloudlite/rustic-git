...
pub(crate) async fn create_team(
    ...
    match db.create(body.slug.trim(), &body.name, &user).await {
        ...
        Err(e) => {
            ...
            if e.downcast_ref::<kloudlite_pulls::directory::Invalid>().is_some() {
...
pub(crate) struct SignIn {
    user: kloudlite_pulls::directory::User,
...
pub(crate) async fn upsert_user(
    ...
    match db.upsert_user(&body.email, &body.name).await {
        Ok(u) => {
            ...
            axum::Json(SignIn { user: u, token, expires_in: kloudlite_core::jwt::TTL_SECS }).into_response()
        }
        Err(e) => {
            let msg = e.to_string();
            if e.downcast_ref::<kloudlite_pulls::directory::Invalid>().is_some() {
...
async fn team_for<'a>(
    ...
    slug: &str,
    min: Option<Role>,
) -> std::result::Result<(String, Team, Role, &'a kloudlite_pulls::directory::Directory), Response> {
...
pub(crate) async fn update_team(
    ...
    let checked = match &body.profile {
        ...
        Some(p) => {
            ...
            let pins = match kloudlite_pulls::directory::check_pins(&p.pins, &names) {
                Ok(v) => v,
    ...
    match db.update_team(&slug, &body.name, &body.description).await {
        ...
        Err(e) if e.downcast_ref::<kloudlite_pulls::directory::Invalid>().is_some() => {
...
fn invite_id(token: &str) -> String {
    kloudlite_core::hex(&sha2::Sha256::digest(token.as_bytes()))
}
...
pub(crate) async fn create_invite(
    ...
    if kloudlite_pulls::directory::Directory::role_of(&team, &email).is_some() {
...
pub(crate) async fn preview_invite(
    ...
    if caller(&api, &headers).is_err() {
        return kloudlite_core::httpx::unauthorized();
    }
    let db = match directory(&api) {
        Ok(d) => d,
        Err(r) => return r,
...
pub(crate) async fn create_signin_link(
    ...
    let now = mongodb::bson::DateTime::now();
    let link = kloudlite_pulls::directory::SignInLink {
        id: invite_id(&token),
        email: email.clone(),
        created_at: now,
...
fn is_last_superadmin(admins: &[kloudlite_pulls::directory::SuperAdmin], target: &str) -> bool {
...
fn superadmin_doc(a: &kloudlite_pulls::directory::SuperAdmin) -> serde_json::Value {
    serde_json::json!({
        "_id": a.user,
...
mod role_tests {
    use super::{rank, Role};
    use kloudlite_pulls::directory::{Directory, Member, Team};
...
mod superadmin_rule_tests {
    ...
    use mongodb::bson::DateTime;
    use kloudlite_pulls::directory::SuperAdmin;
