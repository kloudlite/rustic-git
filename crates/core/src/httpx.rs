...
///
/// The one definition: `storage` re-exports this one rather than keeping a twin, because a call
/// site that spells the match itself is a call site that spells it case-sensitively.
...
/// The judgement half of `basic_user_names` — the header decode needs `axum::http::HeaderMap`,
/// this half is pure. `storage` re-exports it.
