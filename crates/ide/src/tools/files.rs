...
use crate::paths::{confine, relative};
...
pub fn read(t: &TreeCtx, args: &Value) -> Result<Value, ToolError> {
...
fn read_one(t: &TreeCtx, args: &Value) -> Result<Value, ToolError> {
    let p = confine(t, str_arg(args, "path")?)?;
    let meta = std::fs::metadata(&p).map_err(|e| io(e, &p))?;
    if meta.len() > MAX_BYTES {
        return Err(ToolError::Failed(format!("{}: {} bytes, over the {} byte read limit", relative(t, &p), meta.len(), MAX_BYTES)));
    }
    let bytes = std::fs::read(&p).map_err(|e| io(e, &p))?;
    if looks_binary(&bytes) {
        return Ok(json!({ "path": relative(t, &p), "binary": true, "size": meta.len(), "mime": "application/octet-stream" }));
    }
    ...
    Ok(json!({ "path": relative(t, &p), "content": slice.join("\n"), "total_lines": lines.len(), "truncated": offset - 1 + slice.len() < lines.len() }))
}

pub fn write(t: &TreeCtx, args: &Value) -> Result<Value, ToolError> {
    let p = confine(t, str_arg(args, "path")?)?;
    ...
    Ok(json!({ "path": relative(t, &p), "bytes": content.len(), "created": created }))
}
...
pub fn edit(t: &TreeCtx, args: &Value) -> Result<Value, ToolError> {
    ...
    for (fi, f) in files.iter().enumerate() {
        ...
        let p = confine(t, str_arg(f, "path").map_err(|_| ToolError::Invalid(at("`path` (string) is required")))?)?;
        ...
        for (i, e) in edits.iter().enumerate() {
            ...
            match (n, opt_bool(e, "replace_all")) {
                (0, _) => return Err(ToolError::Failed(at(&format!("edit {i}: `old` not found in {}", relative(t, &p))))),
                (1, _) | (_, true) => text = text.replace(old, new),
                (n, false) => return Err(ToolError::Failed(at(&format!("edit {i}: `old` matches {n} places in {}; add context or set replace_all", relative(t, &p))))),
            }
        }
    ...
    if staged.len() == 1 {
        return Ok(json!({ "path": relative(t, &staged[0].0), "applied": staged[0].3 }));
    }
...
pub fn glob(t: &TreeCtx, args: &Value) -> Result<Value, ToolError> {
    ...
    Ok(json!({ "cwd": relative(t, &cwd), "paths": hits.into_iter().map(|(_, p)| p).collect::<Vec<_>>(), "truncated": truncated }))
}

pub fn grep(t: &TreeCtx, args: &Value) -> Result<Value, ToolError> {
    let pattern = str_arg(args, "pattern")?;
    let cwd = confine(t, opt_str(args, "cwd").unwrap_or("."))?;
    ...
    Ok(match mode {
        "files" => json!({ "cwd": relative(t, &cwd), "files": files, "truncated": truncated }),
        "count" => json!({ "cwd": relative(t, &cwd), "counts": counts, "truncated": truncated }),
        _ => json!({ "cwd": relative(t, &cwd), "matches": matches, "truncated": truncated }),
    })
}
...
impl ToolSet for Files {
    fn tools(&self) -> Vec<Tool> {
        vec![
            Tool { name: "read", description: "Read text files with line numbers. path for one file, or paths (up to 50 strings or {path, offset, limit} objects) for several in one call, each answered on its own. offset (1-based line) and limit (lines) page; a binary file answers its size and mime, no bytes.", schema: obj(json!({ "tree": {"type":"string"}, "path": {"type":"string"}, "paths": {"type":"array"}, "offset": {"type":"integer"}, "limit": {"type":"integer"} }), &[]) },
            Tool { name: "write", description: "Create or overwrite a file atomically; parent directories are created.", schema: obj(json!({ "tree": {"type":"string"}, "path": {"type":"string"}, "content": {"type":"string"} }), &["path","content"]) },
            Tool { name: "edit", description: "Exact string replacements, all or nothing across the whole call. {path, edits} for one file or files: [{path, edits}] (up to 50) for several. Each edit's `old` must occur exactly once unless replace_all.", schema: obj(json!({ "tree": {"type":"string"}, "path": {"type":"string"}, "edits": {"type":"array","items":{"type":"object","properties":{"old":{"type":"string"},"new":{"type":"string"},"replace_all":{"type":"boolean"}},"required":["old","new"]}}, "files": {"type":"array","items":{"type":"object","properties":{"path":{"type":"string"},"edits":{"type":"array"}},"required":["path","edits"]}} }), &[]) },
            Tool { name: "patch", description: "Apply a unified diff (git or plain -p1 format) under cwd (default: your working directory); checked first, so nothing is half-applied. Answers the files it touched. Fewer tokens than edit for a large rewrite.", schema: obj(json!({ "tree": {"type":"string"}, "diff": {"type":"string"}, "cwd": {"type":"string"} }), &["diff"]) },
            Tool { name: "glob", description: "Files matching a glob under cwd (default: your working directory), gitignore-aware, newest first.", schema: obj(json!({ "tree": {"type":"string"}, "pattern": {"type":"string"}, "cwd": {"type":"string"} }), &["pattern"]) },
            Tool { name: "grep", description: "Regex search under cwd, gitignore-aware. mode: content (default) | files | count; glob narrows files; context adds lines around a match.", schema: obj(json!({ "tree": {"type":"string"}, "pattern": {"type":"string"}, "cwd": {"type":"string"}, "glob": {"type":"string"}, "mode": {"type":"string","enum":["content","files","count"]}, "context": {"type":"integer"}, "ignore_case": {"type":"boolean"}, "max": {"type":"integer"} }), &["pattern"]) },
        ]
    }

    fn call<'a>(&'a self, name: &'a str, args: Value) -> BoxFuture<'a, Result<Value, ToolError>> {
        async move {
            // Resolved BEFORE the work, on the reactor: an unknown tree is a 400 the caller
            // gets at once, never a blocking task that starts and then discovers it has no root.
            let t = self.trees.resolve(super::opt_str(&args, "tree"))?;
            ...
            // The TREE's graph, not the server's one: a subagent's edit refreshes its own.
            if changes && r.is_ok() {
                after.refresh_soon();
            }
            r
...
mod tests {
    ...
    fn tree() -> (tempfile::TempDir, Arc<crate::trees::TreeCtx>, PathBuf) {
        ...
        let t = Trees::new(root.clone(), None).resolve(None).unwrap();
        (tmp, t, root)
    }
    ...
    fn read_pages_by_line_and_sniffs_binary() {
        let (_t, t, _root) = tree();
        let v = read(&t, &json!({ "path": "src/main.rs", "offset": 2, "limit": 1 })).unwrap();
        assert_eq!(v["content"], "     2\t    println!(\"hi\");");
        assert_eq!(v["total_lines"], 3);
        assert_eq!(v["truncated"], true);
        let b = read(&t, &json!({ "path": "blob.bin" })).unwrap();
        assert_eq!(b["binary"], true);
        assert!(read(&t, &json!({ "path": "../../etc/passwd" })).is_err());
    }

    #[test]
    fn read_takes_several_paths_and_answers_each_on_its_own() {
        let (_t, t, _root) = tree();
        let v = read(&t, &json!({ "paths": ["README.md", { "path": "src/main.rs", "limit": 1 }, "missing.txt"], "limit": 1 })).unwrap();
        ...
        assert!(read(&t, &json!({ "paths": [] })).is_err());
    }

    #[test]
    fn edit_across_files_is_atomic() {
        let (_t, t, root) = tree();
        let e = edit(&t, &json!({ "files": [
    ...
    fn write_creates_parents_and_reports_created() {
        let (_t, t, root) = tree();
        let v = write(&t, &json!({ "path": "deep/er/new.txt", "content": "x" })).unwrap();
        assert_eq!(v["created"], true);
        let v = write(&t, &json!({ "path": "deep/er/new.txt", "content": "xy" })).unwrap();
    ...
    fn edit_is_all_or_nothing_and_names_the_failing_index() {
        let (_t, t, root) = tree();
        let e = edit(&t, &json!({ "path": "src/main.rs", "edits": [{ "old": "hi", "new": "hello" }, { "old": "nope", "new": "x" }] })).unwrap_err();
        ...
        let e = edit(&t, &json!({ "path": "README.md", "edits": [{ "old": "l", "new": "L" }] })).unwrap_err();
        assert!(e.to_string().contains("matches"), "{e}");
        let v = edit(&t, &json!({ "path": "README.md", "edits": [{ "old": "l", "new": "L", "replace_all": true }] })).unwrap();
    ...
    fn two_temp_names_never_collide_and_grep_skips_a_file_over_the_limit() {
        ...
        let v = grep(&t, &json!({ "pattern": "qqq", "mode": "files" })).unwrap();
    ...
    fn glob_and_grep_respect_gitignore_and_modes() {
        let (_t, t, _root) = tree();
        let v = glob(&t, &json!({ "pattern": "**/*.rs" })).unwrap();
        ...
        let v = grep(&t, &json!({ "pattern": "fn main", "mode": "files" })).unwrap();
        assert_eq!(v["files"].as_array().unwrap().len(), 1);
        let v = grep(&t, &json!({ "pattern": "HELLO", "ignore_case": true })).unwrap();
        assert_eq!(v["matches"][0]["line"], 3);
        let v = grep(&t, &json!({ "pattern": "println", "context": 1 })).unwrap();
        assert!(v["matches"][0]["context"].as_str().unwrap().contains("fn main()"));
        let v = grep(&t, &json!({ "pattern": "a", "mode": "count", "glob": "*.md" })).unwrap();
