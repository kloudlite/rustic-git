...
use super::{check_handle, is_duplicate_key, AddMember, Backend, Directory, HandleKind, Member, MemberState, Role, User};
...
impl Directory {
    ...
    pub async fn is_member(&self, email: &str, slug: &str) -> Result<bool> {
        Ok(self.get(slug).await?.is_some_and(|t| Self::active_role_of(&t, email).is_some()))
    }
    ...
    /// `role_of` for ACCESS: a paused member holds no role until unpaused. `role_of` stays the
    /// data-side answer (they are still in the team, their row and role are kept).
    pub fn active_role_of(team: &Team, email: &str) -> Option<Role> {
        team.members.iter().find(|m| m.user.eq_ignore_ascii_case(email) && m.state == MemberState::Active).map(|m| m.role)
    }
