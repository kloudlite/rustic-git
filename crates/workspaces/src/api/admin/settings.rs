...
pub struct PeerClient {
    ...
    /// e.g. `http://rustic-git:8081` — the peer Service, never the public one.
    pub upstream: String,
    pub secret: String,
}
...
async fn central_boot_readers(s: &ApiState, field: &str) -> Result<Vec<(Scope, &'static str)>, Response> {
    match field {
        "sshHost" | "sshPort" => {
            ...
            Ok(regions.into_iter().map(|r| (Scope::Region(r), "rustic-git-gateway")).collect())
        }
