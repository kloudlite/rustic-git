...
fn placed_ws(name: &str, owner: &str) -> Value {
    json!({
        ...
        "metadata": {"name": name, "uid": format!("uid-{name}"), "labels": {"kloudlite.io/owner": owner}},
        "spec": {
...
async fn history_lists_snapshot_crs_in_parent_order() {
    ...
    let routes = vec![get(
        ...
        json!({"apiVersion": "kloudlite.io/v1alpha1", "kind": "SnapshotList", "metadata": {}, "items": [tip, root]}),
    )];
    let s = server(routes).await;
    let tok = token(&s.jwt, "karthik");
    let resp = reqwest::Client::new()
...
async fn history_rows_carry_the_frozen_state_or_null() {
    ...
    let routes = vec![get(
        format!("{API}/snapshots"),
        json!({"apiVersion": "kloudlite.io/v1alpha1", "kind": "SnapshotList", "metadata": {}, "items": [with_state, without_state]}),
    )];
    let s = server(routes).await;
    let tok = token(&s.jwt, "karthik");
    let resp = reqwest::Client::new()
...
async fn refs_names_the_newest_snapshot_as_main() {
    ...
    let routes = vec![get(
        format!("{API}/snapshots"),
        json!({"apiVersion": "kloudlite.io/v1alpha1", "kind": "SnapshotList", "metadata": {}, "items": [root, tip]}),
    )];
    let s = server(routes).await;
    let tok = token(&s.jwt, "karthik");
    let resp = reqwest::Client::new()
...
async fn history_stays_newest_first_across_a_three_snapshot_chain() {
    ...
    let routes = vec![get(
        format!("{API}/snapshots"),
        json!({"apiVersion": "kloudlite.io/v1alpha1", "kind": "SnapshotList", "metadata": {}, "items": [mid, root, tip]}),
    )];
    let s = server(routes).await;
    let tok = token(&s.jwt, "karthik");
    let resp = reqwest::Client::new()
...
async fn history_created_at_is_rfc3339() {
    ...
    let routes = vec![get(
        format!("{API}/snapshots"),
        json!({"apiVersion": "kloudlite.io/v1alpha1", "kind": "SnapshotList", "metadata": {}, "items": [root]}),
    )];
    let s = server(routes).await;
    let tok = token(&s.jwt, "karthik");
    let resp = reqwest::Client::new()
...
fn replicas(items: Vec<Value>) -> Route {
    get(format!("{API}/volumereplicas"), json!({"apiVersion": "kloudlite.io/v1alpha1", "kind": "VolumeReplicaList", "metadata": {}, "items": items}))
}
