...
async fn admin_server(routes: Vec<Route>, keys: Option<Arc<rustic_git_storage::store::Store>>) -> Server {
...
fn req_obj(name: &str, owner: &str, created: &str) -> Value {
    json!({"apiVersion": "rustic-git.io/v1alpha1", "kind": "QuotaRequest",
           "metadata": {"name": name, "labels": {"rustic-git.io/owner": owner}, "creationTimestamp": created},
...
async fn overview_with_nothing_pending_still_returns_fleet_numbers() {
    ...
    // itself still flags zero-agents — no `rustic-git-agent` workload is mocked here).
    let attention = body["attention"].as_array().unwrap();
...
async fn audit_row(keys: &Arc<rustic_git_storage::store::Store>, actor: &str, action: &str, target: &str) {
