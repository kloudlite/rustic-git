...
fn every_crd_has_a_status_subresource_and_the_right_node_selector() {
    ...
    for crd in all_crds() {
        ...
        let want: &[&str] = match crd.spec.names.kind.as_str() {
            ...
            // Per space, read from a cluster-wide cache on every node: no node axis.
            "SpaceEnvironment" => &[],
    ...
    for crd in all_crds() {
        ...
        if matches!(
            ...
            "OwnerBinding" | "OwnerKeys" | "VolumeReplica" | "Region" | "Quota" | "QuotaRequest" | "Request"
                | "ClusterSettings" | "SpaceEnvironment"
        ) {
            continue;
        }
...
fn the_bootstrap_defaults_are_the_specs_table() {
    ...
    assert_eq!((u.workspaces, u.environments, u.snapshots, u.disk_gb, u.cpu, u.memory_gb), (5, 2, 20, 100, 40, 80));
