...
use rustic_git_workspaces::api::{admin::router, admin::PeerClient, ApiState};
...
async fn admin_server(routes: Vec<Route>, keys: Option<Arc<rustic_git_storage::store::Store>>, peer: Option<PeerClient>) -> Server {
    let jwt = jwt();
...
fn region(id: &str) -> Value {
    json!({"apiVersion": "rustic-git.io/v1alpha1", "kind": "Region", "metadata": {"name": id}, "spec": {"name": id, "status": "active"}})
}

fn cluster_settings(annotations: Value) -> Value {
    json!({
        "apiVersion": "rustic-git.io/v1alpha1", "kind": "ClusterSettings",
...
async fn put_cluster_writes_annotations_and_history() {
    let updated = cluster_settings(json!({"rustic-git.io/updated-by": CALLER}));
    let s = admin_server(
        vec![
    ...
    assert_eq!(sent[0]["metadata"]["annotations"]["rustic-git.io/updated-by"], CALLER);
    let hist = &sent[0]["metadata"]["annotations"]["rustic-git.io/settings-history"];
    assert!(hist.as_str().unwrap().contains("syncSecs"), "the OLD spec must be pushed onto history");
}

/// A `Mark::Boot` field (`defaultImage` → `rustic-git-agent`) mid-rollout is a 409, `{ready,
...
async fn revert_cluster_restores_the_named_entry_and_grows_history_again() {
    ...
    let current = cluster_settings(json!({"rustic-git.io/settings-history": hist.to_string()}));
    let s = admin_server(
        vec![
            get(format!("{API}/regions/us"), region("us")),
