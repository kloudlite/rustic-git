...
async fn clusters_list_composes_agents_nodes_and_hosted_counts() {
    let s = admin_server(vec![
        ...
        get(format!("{API}/clustersettings/default"), json!({"apiVersion": "kloudlite.io/v1alpha1", "kind": "ClusterSettings", "metadata": {"name": "default", "generation": 4}, "spec": {}, "status": {"observedGeneration": 3}})),
    ])
    .await;
...
async fn drain_sets_the_label_and_undrain_clears_label_and_status() {
    ...
    assert_eq!(sent[0]["metadata"]["labels"]["kloudlite.io/decommission"], "true", "{:?}", sent);

    let r = post(&s, "/admin/clusters/r1/nodes/n1/undrain", json!({"reason": "changed my mind"})).await;
    assert_eq!(r.status(), 200);
    let sent = s.rec.sent("PATCH", &format!("{NODES}/n1"));
    let undrain = &sent[1];
    assert!(undrain["metadata"]["labels"]["kloudlite.io/decommission"].is_null(), "{undrain}");
    assert!(undrain["metadata"]["annotations"]["kloudlite.io/decommission-status"].is_null(), "{undrain}");
}

/// Decommission refuses a node that has not reached `drained` yet.
#[tokio::test]
async fn decommission_refuses_before_drained() {
    let s = admin_server(node_routes(json!({"kloudlite.io/decommission-status": "draining running=1 owned=2 copies=0 thin=0"}))).await;
...
async fn decommission_cordons_a_drained_node_and_deletes_nothing() {
    let s = admin_server(node_routes(json!({"kloudlite.io/decommission-status": "drained 2026-09-04T00:00:00Z"}))).await;
    let r = post(&s, "/admin/clusters/r1/nodes/n1/decommission", json!({"reason": "vm going away"})).await;
    assert_eq!(r.status(), 200);
    let sent = s.rec.sent("PATCH", &format!("{NODES}/n1"));
...
async fn cluster_detail_counts_hosted_copies_per_node() {
    let vol = json!({"apiVersion": "kloudlite.io/v1alpha1", "kind": "Volume", "metadata": {"name": "v1"},
                     "spec": {"owner": "ann", "team": "", "nodeName": "n1", "region": "r1", "quotaGb": 20, "replicas": 2}});
    let rep = json!({"apiVersion": "kloudlite.io/v1alpha1", "kind": "VolumeReplica", "metadata": {"name": "v1-n1"},
...
async fn drain_works_on_an_inactive_region() {
    let inactive = json!({"apiVersion": "kloudlite.io/v1alpha1", "kind": "Region",
    ...
    assert_eq!(r.status(), 200);
    assert_eq!(s.rec.sent("PATCH", &format!("{NODES}/n1"))[0]["metadata"]["labels"]["kloudlite.io/decommission"], "true");
}
