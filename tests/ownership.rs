...
use kloudlite_storage::ownership::{Entry, OwnershipStore};
...
async fn leader_put_then_get() {
    ...
    leader.put("alice/web", &entry("kloudlite-1", 1_000)).await.unwrap();
    let got = leader.get("alice/web").await.unwrap();
    assert_eq!(got, Some(entry("kloudlite-1", 1_000)));
}
...
async fn follower_eventually_sees_leader_write() {
    ...
    assert_eq!(seen, Some(entry("kloudlite-1", 1_000)), "follower never saw the leader's write");
}
...
async fn follower_put_errors() {
    ...
    let follower = OwnershipStore::open(os);
    let res = follower.put("alice/web", &entry("kloudlite-1", 1_000)).await;
    assert!(res.is_err());
}
...
async fn all_returns_everything_written() {
    ...
    let leader = leader(os).await;
    leader.put("alice/web", &entry("kloudlite-1", 1_000)).await.unwrap();
    leader.put("bob/app", &entry("kloudlite-2", 2_000)).await.unwrap();
    ...
    assert_eq!(
        all,
        vec![
            ("alice/web".to_string(), entry("kloudlite-1", 1_000)),
            ("bob/app".to_string(), entry("kloudlite-2", 2_000)),
        ]
    );
}
...
async fn follower_opened_first_converges_once_the_leader_writes() {
    ...
    assert_eq!(seen, Some(entry("kloudlite-1", 1_000)), "follower never converged");
}
