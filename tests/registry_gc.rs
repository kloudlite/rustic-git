...
async fn an_unreferenced_blob_is_swept_and_a_referenced_one_is_not() {
    ...
    e.store.os
        .put(&kloudlite_registry::store::manifest_path("acme", "nginx", &md), PutPayload::from(manifest))
        .await.unwrap();
...
async fn a_layer_two_images_share_survives_one_of_them_being_emptied() {
    ...
    for image in ["nginx", "api"] {
        ...
        let md = Digest::of(&m);
        e.store.os
            .put(&kloudlite_registry::store::manifest_path("acme", image, &md), PutPayload::from(m))
            .await.unwrap();
...
async fn a_blob_referenced_only_via_an_index_entry_or_subject_survives() {
    ...
    e.store.os
        .put(&kloudlite_registry::store::manifest_path("acme", "multi", &ixd), PutPayload::from(index))
        .await.unwrap();
    ...
    e.store.os
        .put(&kloudlite_registry::store::manifest_path("acme", "artifact", &atd), PutPayload::from(attached))
        .await.unwrap();
...
async fn a_manifest_that_is_not_valid_json_aborts_the_sweep_and_deletes_nothing() {
    ...
    e.store.os
        .put(&kloudlite_registry::store::manifest_path("acme", "broken", &gd), PutPayload::from(garbage))
        .await.unwrap();
...
async fn a_blob_referenced_between_the_two_manifest_reads_survives_the_mount_race() {
    ...
    let md = Digest::of(&m);
    e.store.os
        .put(&kloudlite_registry::store::manifest_path("acme", "mounted", &md), PutPayload::from(m))
        .await.unwrap();
...
async fn worker_lanes_are_inert_and_gc_still_sweeps_with_redis_down() {
    let cache = kloudlite_storage::cache::Cache::connect(Some("redis://127.0.0.1:1")).await;
...
async fn a_sweep_with_nothing_old_enough_reads_no_manifests() {
    ...
    let garbage = b"not json at all".to_vec();
    e.store.os
        .put(&kloudlite_registry::store::manifest_path("acme", "broken", &Digest::of(&garbage)), PutPayload::from(garbage))
        .await.unwrap();
...
async fn reconciling_an_unchanged_marker_writes_nothing() {
    ...
    let before = kloudlite_storage::index::read(&e.store.os, kloudlite_storage::index::Kind::Img, "acme", "nginx").await.unwrap();
    assert_eq!(gc::reconcile_owner(&e.store, "acme").await.unwrap(), 0);
    assert_eq!(gc::reconcile_owner(&e.store, "acme").await.unwrap(), 0);
    let after = kloudlite_storage::index::read(&e.store.os, kloudlite_storage::index::Kind::Img, "acme", "nginx").await.unwrap();
