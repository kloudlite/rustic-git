...
/// `KLOUDLITE_GIT_S3_URL=file://` gives, and object_store has no `MultipartStore` for it. The harness
...
pub async fn serve_public_file() -> (String, TestEnv) {
    ...
    tokio::spawn(async move {
        axum::serve(l, kloudlite_git_server::router::router(app)).await.unwrap();
    });
    (base, e)
}
...
pub async fn env_cached() -> TestEnv {
    ...
    store.cache = Arc::new(kloudlite_git_storage::cache::Cache::memory());
...
pub async fn app(store: Arc<Store>) -> Arc<kloudlite_git_app::App> {
...
/// silent green on a CI runner missing the tool. CI sets `KLOUDLITE_GIT_REQUIRE_GIT=1`, which turns
...
fn have_tool(bin: &str, probe: &str) -> bool {
    ...
    assert!(
        ok || std::env::var_os("KLOUDLITE_GIT_REQUIRE_GIT").is_none(),
        "{bin} is not installed and KLOUDLITE_GIT_REQUIRE_GIT is set: this test must not skip here"
    );
    ok
}
...
pub async fn serve(app: Arc<kloudlite_git_app::App>) -> u16 {
    let l = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
...
pub async fn serve_peer() -> (String, TestEnv) {
    ...
    let base = format!("http://{}", l.local_addr().unwrap());
    tokio::spawn(async move {
        axum::serve(l, kloudlite_git_server::router::peer_router(app)).await.unwrap();
    });
    (base, e)
}
...
pub async fn serve_public_and_peer() -> (String, String, TestEnv) {
    ...
    let app2 = app.clone();
    tokio::spawn(async move {
        axum::serve(pub_l, kloudlite_git_server::router::router(app2)).await.unwrap();
    });
    tokio::spawn(async move {
        axum::serve(peer_l, kloudlite_git_server::router::peer_router(app)).await.unwrap();
    });
...
pub async fn push_fixture(e: &TestEnv, owner: &str, name: &str) -> kloudlite_git_storage::store::Repo {
...
) -> kloudlite_git_storage::store::Repo {
    let s = e.store.clone();
...
pub async fn seed_blobs(e: &TestEnv, owner: &str, contents: &[&[u8]]) {
    ...
    for c in contents {
        let d = kloudlite_git_registry::Digest::of(c);
        e.store
            .os
            .put(&kloudlite_git_registry::store::blob_path(owner, &d), PutPayload::from(c.to_vec()))
            .await
            .unwrap();
    }
}
...
// one (`image.yml`'s `test` job), and a laptop without `KLOUDLITE_GIT_TEST_MONGO_URI` skips the
...
pub struct TestDirectory {
    pub dir: Arc<kloudlite_git_pulls::directory::Directory>,
    client: mongodb::Client,
    name: String,
}

/// `Some` when `KLOUDLITE_GIT_TEST_MONGO_URI` names a Mongo, `None` — with a printed reason — when
...
pub async fn mongo(what: &str) -> Option<TestDirectory> {
    let uri = match std::env::var("KLOUDLITE_GIT_TEST_MONGO_URI") {
        Ok(u) if !u.is_empty() => u,
        _ => {
            eprintln!("skipping the handler half of {what}: KLOUDLITE_GIT_TEST_MONGO_URI is unset");
            return None;
        }
    };
    ...
    let dir = kloudlite_git_pulls::directory::Directory::connect(&uri, &name).await.unwrap();
