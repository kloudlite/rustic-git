...
async fn api_with_jwt(e: &common::TestEnv, up: &Upstream, secret: &str) -> String {
    ...
    tokio::spawn(async move {
        kloudlite_api::serve(store, cache, None, Some(jwt), upstream, "s".into(), l, None, None, None, None, false)
            .await
            .unwrap()
    });
    format!("http://{addr}")
...
async fn api_with_dir(e: &common::TestEnv, up: &Upstream, d: &common::TestDirectory) -> String {
    ...
    let dir = d.dir.clone();
    tokio::spawn(async move {
        kloudlite_api::serve(store, cache, Some(dir), Some(jwt), upstream, "s".into(), l, None, None, None, None, false)
            .await
            .unwrap()
    });
    format!("http://{addr}")
...
async fn api_with(
    ...
    let (store, upstream) = (e.store.clone(), format!("http://{}", up.addr));
    tokio::spawn(async move {
        kloudlite_api::serve(store, cache, None, None, upstream, "s".into(), l, None, None, None, None, false)
            .await
            .unwrap()
    });
    format!("http://{addr}")
...
async fn a_team_is_created_in_a_known_region() {
    ...
    tokio::spawn(async move {
        kloudlite_api::serve(store, cache, Some(dir), Some(jwt), upstream, "s".into(), l, None, None, Some(check), None, false)
            .await
            .unwrap()
    });
    d.dir.upsert_user("boss@example.com", "Boss").await.unwrap();
