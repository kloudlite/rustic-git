...
use kloudlite_vcs::gc::RepackExt;
use kloudlite_gitbase::refs::UpdateRefsExt;
use kloudlite_vcs::protocol::{receive, upload};
...
async fn head_falls_back_to_existing_branch() {
    ...
    let ls_refs = |s: std::sync::Arc<kloudlite_storage::store::Store>| async move {
        let repo = s.open_repo("a", "r").await.unwrap().unwrap();
        let mut req = Vec::new();
        pktline::write_text(&mut req, "command=ls-refs").unwrap();
        pktline::write_delim(&mut req).unwrap();
...
async fn squash_and_merge_commit_land_the_right_shape() {
    ...
    for (strategy, want_parents) in [("squash", 1usize), ("merge", 2usize)] {
        ...
        let log = kloudlite_vcs::browse::log(&odb, head, 3).unwrap();
        ...
        let landed = kloudlite_gitbase::objects::write_commit(
            &s,
            &repo,
...
fn fetch_pack_bytes(
    s: &std::sync::Arc<kloudlite_storage::store::Store>,
    repo: &kloudlite_storage::store::Repo,
    lines: &[String],
) -> Vec<u8> {
    let mut req = Vec::new();
    pktline::write_text(&mut req, "command=fetch").unwrap();
