...
async fn clone_push_fetch() {
    ...
    assert!(
        r.to_lowercase()
            .contains("www-authenticate: basic realm=\"kloudlite\""),
        "{r}"
    );
...
async fn lane_consolidates_packs_past_threshold() {
    ...
    kloudlite_server::lanes::consolidate_owned_packs(&app, 4).await;
    assert_eq!(packs().await, 4);
    kloudlite_server::lanes::consolidate_owned_packs(&app, 3).await;
...
async fn consolidation_crash_before_retire_leaves_duplicates_not_holes() {
    ...
    use kloudlite_server::gc::RepackExt;
    let e = common::env().await;
    let s = e.store.clone();
    ...
    let old = kloudlite_server::gc::rebuild(&s, &repo, false).await.unwrap().unwrap();
